...
typedef enum { FB30_INC, FB30_DEC, FB30_ASL, FB30_LSR, FB30_ROL } Fb30Change;
...
static void cycle(NbaSetupFb30Work *s, NbaCodecBusKind kind, uint32_t address,
                  uint8_t value, int8_t read_shift, bool changed_value)
{
    NbaCodecPendingCycle *p = &B.pending[B.pending_count];
    s->indirect_cycle[B.pending_count++] = 0;
...
static void idle(NbaSetupFb30Work *s)
{
    cycle(s, NBA_CODEC_IDLE, 0, 0, -1, false);
}

static void instruction(NbaSetupFb30Work *s, uint16_t pc, uint8_t bytes)
{
    uint8_t i;
...
static void implied(NbaSetupFb30Work *s, uint16_t pc, uint8_t bytes, uint8_t idles)
{
    instruction(s, pc, bytes);
    while (idles-- != 0) idle(s);
}

static bool index_idle(const NbaSetupFb30Work *s, uint16_t base, uint16_t index)
{
    return (P & 0x10u) == 0 || (base & 255u) + index > 255u;
}

static void memory(NbaSetupFb30Work *s, uint16_t pc, uint8_t bytes,
...
static void indirect_read(NbaSetupFb30Work *s, uint16_t pc, uint8_t dp, uint8_t width)
{
    uint8_t i;
    instruction(s, pc, 2);
    ...
    for (i = 0; i < width; ++i) {
        ...
        s->indirect_cycle[B.pending_count - 1u] = 1;
    }
}

static uint16_t changed(uint16_t old, uint8_t width, Fb30Change kind, uint8_t carry)
{
    uint16_t mask = width == 1 ? 255u : 65535u;
    switch (kind) {
    case FB30_INC: return (uint16_t)((old + 1u) & mask);
    case FB30_DEC: return (uint16_t)((old - 1u) & mask);
    case FB30_ASL: return (uint16_t)((old << 1) & mask);
    case FB30_LSR: return (uint16_t)((old & mask) >> 1);
    case FB30_ROL: return (uint16_t)(((old << 1) | carry) & mask);
    }
    return 0;
}

static void nz(NbaSetupFb30Work *s, uint16_t value, uint8_t width)
{
    uint16_t mask = width == 1 ? 255u : 65535u;
...
static void change_flags(NbaSetupFb30Work *s, uint16_t old, uint16_t value,
                         uint8_t width, Fb30Change kind)
{
    if (kind == FB30_ASL || kind == FB30_ROL)
        P = (uint8_t)((P & 0xfeu) | ((old & (width == 1 ? 128u : 32768u)) != 0 ? 1u : 0u));
    else if (kind == FB30_LSR) P = (uint8_t)((P & 0xfeu) | (old & 1u));
    nz(s, value, width);
}

static void rmw(NbaSetupFb30Work *s, uint16_t pc, uint8_t bytes, uint32_t address,
                uint8_t width, bool indexed, Fb30Change kind)
{
...
static void rmw_commit(NbaSetupFb30Work *s)
{
    uint16_t result = changed(B.read_value, s->rmw_width, (Fb30Change)s->change_kind, s->carry_in);
    change_flags(s, B.read_value, result, s->rmw_width, (Fb30Change)s->change_kind);
}

static void set_a(NbaSetupFb30Work *s, uint32_t value)
{
...
static void set_x(NbaSetupFb30Work *s, uint32_t value)
{
...
static void set_y(NbaSetupFb30Work *s, uint32_t value)
{
    uint8_t width = (P & 0x10u) != 0 ? 1u : 2u;
...
static void status_set(NbaSetupFb30Work *s, uint32_t value)
{
    P = (uint8_t)value;
    if ((P & 0x10u) != 0) { X &= 255u; Y &= 255u; }
}

static void accumulator_change(NbaSetupFb30Work *s, Fb30Change kind)
{
    uint8_t width = (P & 0x20u) != 0 ? 1u : 2u;
    uint16_t old = A;
...
static void compare(NbaSetupFb30Work *s, uint16_t value, uint16_t operand, uint8_t width)
{
    uint16_t mask = width == 1 ? 255u : 65535u;
...
static void add_sub(NbaSetupFb30Work *s, uint16_t operand, bool subtract)
{
...
static void bit_test(NbaSetupFb30Work *s, uint16_t operand)
{
    uint16_t sign = (P & 0x20u) != 0 ? 128u : 32768u;
    uint16_t mask = (P & 0x20u) != 0 ? 255u : 65535u;
...
static void push(NbaSetupFb30Work *s, uint16_t pc, uint8_t bytes, uint16_t value, uint8_t width)
{
    uint8_t i;
    instruction(s, pc, bytes);
    idle(s);
...
static void pull(NbaSetupFb30Work *s, uint16_t pc, uint8_t width, bool rts)
{
    uint8_t i;
    instruction(s, pc, 1);
    idle(s); idle(s);
...
static void jsl_queue(NbaSetupFb30Work *s)
{
    ...
    cycle(s, NBA_CODEC_WRITE, (uint16_t)(B.registers.stack_pointer - 1u), 0xc6, -1, false);
    cycle(s, NBA_CODEC_WRITE, (uint16_t)(B.registers.stack_pointer - 2u), 0x32, -1, false);
}

static bool unsupported(NbaSetupFb30Work *s)
{
...
/* Preserve the original $C44D/$C44F unconsumed terminating-bit peek, the
 * $BEB5 zero-count threshold STZ, and $BEC0/$BEC1 zero-shift carry behavior.
 * The last edge fails to close a synthetic depth-16 tree; do not replace it
 * with mathematically normalized Huffman construction. Native AEA0AF uses
 * depth ten. Source-work and independent semantic proof document this scope. */
#include "nba_setup_fb30_program.inc"

static bool prepare(NbaSetupFb30Work *s)
{
...
bool nba_setup_fb30_work_begin(NbaSetupFb30Work *s, const NbaCodecWorkEntry *entry,
...
bool nba_setup_fb30_work_peek(const NbaSetupFb30Work *s, NbaCodecBusCycle *out)
{
    const NbaCodecPendingCycle *p;
    if (s == NULL || out == NULL || B.status != NBA_CODEC_WORK_RUNNING ||
        B.pending_index >= B.pending_count) return false;
    p = &B.pending[B.pending_index];
    *out = p->cycle;
    if (s->indirect_cycle[B.pending_index]) {
        out->address = DB(s->pointer_read + out->address);
        out->master_clocks = bus_clocks(out->address);
    }
    if (p->rmw_value)
        out->value = (uint8_t)(changed(B.read_value, s->rmw_width, (Fb30Change)s->change_kind, s->carry_in) >> p->read_shift);
    return true;
}

bool nba_setup_fb30_work_accept(NbaSetupFb30Work *s, uint8_t value)
{
    const NbaCodecPendingCycle *p;
    if (s == NULL || B.status != NBA_CODEC_WORK_RUNNING || B.pending_index >= B.pending_count) return false;
    p = &B.pending[B.pending_index++];
    if (p->cycle.kind == NBA_CODEC_READ) {
        if (p->read_shift >= 32) s->pointer_read |= (uint16_t)((uint16_t)value << (p->read_shift - 32));
