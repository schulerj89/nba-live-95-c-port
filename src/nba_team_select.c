...
bool nba_team_select_init(NbaTeamSelect *screen, const NbaAssetPack *assets,
    ...
    screen->category = NBA_TEAM_RANK_SCORING;
...
NbaTeamSelectSound nba_team_select_update(NbaTeamSelect *screen,
    ...
    if (input->pressed & NBA_BTN_UP) {
        screen->category = (NbaTeamRankCategory)(
            (screen->category + NBA_TEAM_RANK_COUNT - 1) % NBA_TEAM_RANK_COUNT);
        return NBA_TEAM_SOUND_CATEGORY;
    }
    if (input->pressed & NBA_BTN_DOWN) {
        screen->category = (NbaTeamRankCategory)(
            (screen->category + 1) % NBA_TEAM_RANK_COUNT);
        return NBA_TEAM_SOUND_CATEGORY;
    }
    if (input->pressed & (NBA_BTN_LEFT | NBA_BTN_RIGHT)) {
        ...
        uint8_t rank = nba_team_records[*selected].rank[screen->category];
        if (input->pressed & NBA_BTN_LEFT)
            rank = rank <= 1 ? NBA_TEAM_COUNT : (uint8_t)(rank - 1);
        else
            rank = rank >= NBA_TEAM_COUNT ? 1 : (uint8_t)(rank + 1);
        *selected = team_for_rank(screen->category, rank);
...
static bool team_dynamic_bg3_pixel(int x, int y) {
    if (y >= 60 && y < 77 && x >= 76 && x < 130) return true;
    if (y >= 94 && y < 112 && x >= 125 && x < 231) return true;
...
static void team_render_layers(NbaRenderer *renderer, const uint8_t *vram,
    ...
    int category_top = 116 + (int)category * 18;
    ...
    for (int py = 0; py < size; ++py) for (int px = 0; px < size; ++px) {
        ...
        if (color && x + px >= 0 && x + px < NBA_SNES_WIDTH && y + py < NBA_SNES_HEIGHT)
            renderer->pixels[(y + py) * NBA_SNES_WIDTH + x + px] =
                nba_snes_cgram_color(cgram, 128 + forced_palette * 16 + color,
                                     15, 0, 0, 0);
    }
}
...
void nba_team_select_render(const NbaTeamSelect *screen, NbaRenderer *renderer) {
    ...
    uint8_t active_team = screen->active_side == NBA_TEAM_SIDE_LEFT ?
                          screen->session->left_team : screen->session->right_team;
    const NbaAssetItem *active_vram = team_asset(screen, NBA_ASSET_TEAM_VRAM_BASE, active_team);
    const NbaAssetItem *active_cgram = team_asset(screen, NBA_ASSET_TEAM_CGRAM_BASE, active_team);
    ...
    team_render_layers(renderer, active_vram->data, active_cgram->data,
                       bg1_h, bg2_h, bg2_v,
                       bg3_v, brightness, show_bg3, screen->category);
    if (!show_bg3) return;
    ...
    for (int side = 0; side < 2; ++side) {
        ...
        int name_x = side == 0 ? 79 : 130;
        int name_y = (side == 0 ? 63 : 95) - bg3_v;
        team_draw_bg3_crop(renderer, vram->data, cgram->data,
                           125, 95, 106, 16, name_x, name_y, brightness,
                           side == (int)screen->active_side);
        for (int rank = 0; rank < NBA_TEAM_RANK_COUNT; ++rank) {
            int source_y = 116 + rank * 18;
            int y = source_y - bg3_v;
            team_draw_bg3_crop(renderer, vram->data, cgram->data,
                               188, source_y, 44, 16, side == 0 ? 36 : 188, y,
                               brightness, side == (int)screen->active_side &&
                               rank == (int)screen->category);
        }
    }
    if (!show_objects) return;
    const NbaAssetItem *oam = nba_assets_get(screen->assets, NBA_ASSET_TEAM_SELECT_OAM);
    if (oam && oam->size == 0x220u) {
        int left_palette = screen->active_side == NBA_TEAM_SIDE_LEFT ? 2 : 1;
        int right_palette = screen->active_side == NBA_TEAM_SIDE_RIGHT ? 2 : 1;
        for (int i = 20; i >= 6; --i)
            team_draw_panel_sprite(renderer, active_vram->data, active_cgram->data,
