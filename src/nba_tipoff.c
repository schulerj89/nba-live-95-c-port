...
static bool cpu_free_throw_shooter_caller(NbaTipoff *tipoff,
                                          uint8_t shooter,
...
/* `$86:9846-$986C`, CPU logic: restore an actor from a transient receiver or
 * action mode and synchronize both host projections of native actor `+$60`. */
static void cpu_restore_normal_mode(NbaTipoff *tipoff, unsigned slot) {
    ...
    actor->contact_action_timer_raw_60=action.timer;
...
/* `$87:B3BD-$B479`, animation system: apply a command to the actor's queued
 * upper/lower channels without resolving the later render resource phase. */
static bool actor_queue_animation_command(NbaTipoff *tipoff,
    NbaTipoffActor *actor, NbaPlayerAnimationCommand command, uint16_t state) {
    ...
    if (!nba_player_animation_command_scratch(tipoff->assets, &channels, command,
            ...
            actor->free_throw_launch_half_raw_a8 != 0,&tipoff->scratch_0047)) return false;
...
/* `$87:B3BD-$B479` plus selected `$87:AEC3` integration, animation system:
 * queue a command and immediately publish resources only for adopted action
 * paths whose native caller resolves them at the same boundary. */
static void actor_animation_command(NbaTipoff *tipoff, NbaTipoffActor *actor,
    ...
    if (!actor_queue_animation_command(tipoff, actor, command, state)) return;
...
/* Host-only shared helper; no direct native address. Implements the animation
 * sequences at `$86:C223-$C22F/$86:CB6A-$CB76` through
 * `$87:B538/$B555/$B3BD`: cancel both channels, then install knockdown. */
static void actor_install_knockdown_animation(NbaTipoff *tipoff,
                                              NbaTipoffActor *actor,
                                              uint16_t action) {
    (void)actor_queue_animation_command(
        tipoff, actor, NBA_ANIMATION_CANCEL_UPPER, 0u);
    (void)actor_queue_animation_command(
        tipoff, actor, NBA_ANIMATION_CANCEL_LOWER, 0u);
    (void)actor_queue_animation_command(
        tipoff, actor, NBA_ANIMATION_INSTALL_BOTH, action);
}
...
/* `$86:AB2D-$AF65`, CPU logic: grounded mode-15 setup. This includes the
 * aligned `$2A-$2C` selector at `$86:AE52-$AED7`; airborne `$AFC4` and catch-preinit
...
bool nba_tipoff_begin_rom_pass(NbaTipoff *tipoff, unsigned passer_slot,
    ...
    /* Native `$86:AB2D-$AF65` has one actor +$60 word. Keep the host's
     * mode-specialized aliases synchronized before a selected receiver can
     * be promoted to mode 14 by the owner-flow caller. */
    receiver->contact_action_timer_raw_60 = receiver_timer;
...
static bool cpu_refresh_defense_target(NbaTipoff *tipoff, unsigned slot,
    ...
    if (paired_slot >= NBA_GAMEPLAY_ACTOR_COUNT ||
        ...
        if (base_assignment_parent) return false;
...
/* `$86:994C-$99C3`, CPU logic: execute mode seven's dead-ball hold. The
 * caller supplies the scheduler-selected actor; native DP $C6 is two for the
 * production actor pass represented here. */
static void cpu_apply_mode_seven(NbaTipoff *tipoff, unsigned slot) {
    NbaTipoffActor *actor = &tipoff->actors[slot];
    /* `$994C-$9961`: only the exact full-word dead-ball state continues.
     * Both this path and timer expiry install state three before `$9846`. */
    if (tipoff->live_state_raw != 0x0082u) {
        (void)actor_queue_animation_command(
            tipoff, actor, NBA_ANIMATION_INSTALL_BOTH, 3u);
        cpu_restore_normal_mode(tipoff, slot);
        return;
    }
    /* `$9962-$9967` reads integer Z +$0C. A grounded actor may retain a
     * nonzero fractional byte in +$0B and must still run the hold. */
    if (fp_integer_word(actor->z_fp) != 0) return;
    ...
    /* `$9974-$997D`: CPU actors steer toward world origin through the
     * already translated `$85:B3AA -> $85:A82C` movement child. Human actors
     * preserve velocity and boost exactly. */
    if (actor->controller_assignment_raw < 0) {
        uint8_t direction = nba_gameplay_target_direction(
            (int16_t)(0u - (uint16_t)fp_integer_word(actor->x_fp)),
            (int16_t)(0u - (uint16_t)fp_integer_word(actor->y_fp)), NULL);
        uint8_t profile_42 = 0x58u;
        ...
        nba_gameplay_velocity_step(
            &actor->velocity_x, &actor->velocity_y,
            &actor->movement_boost_timer, direction, profile_42, 2u, false,
            (int16_t)tipoff->possession_actor);
    }

    if ((uint16_t)actor->velocity_x == 0u &&
        (uint16_t)actor->velocity_y == 0u) {
        (void)actor_queue_animation_command(
            tipoff, actor, NBA_ANIMATION_INSTALL_BOTH, 0x10u);
    ...
    (void)actor_queue_animation_command(
        tipoff, actor, NBA_ANIMATION_INSTALL_BOTH, 0x0Eu);
    if (actor->pass_direction_raw != 0u) {
        actor->movement_direction ^= 4u;
        actor->requested_direction = actor->movement_direction;
    }
    actor->pass_direction_raw = 1u;
}

/* `$86:F0B7-$F0FC`, CPU logic: execute mode nine's timed target override,
 * steering to +$56/+$58 until its final +$A2-directed movement window. */
static void cpu_apply_mode_nine(NbaTipoff *tipoff, unsigned slot) {
    NbaTipoffActor *actor = &tipoff->actors[slot];
        ...
        if ((int16_t)remaining >= 0) {
            ...
            if (remaining >= 10u) {
                direction = nba_gameplay_target_direction(
                    (int16_t)(actor->target_x - fp_integer_word(actor->x_fp)),
                    (int16_t)(actor->target_y - fp_integer_word(actor->y_fp)),
                    NULL);
            } else {
                direction = actor->velocity_direction_raw_a2;
            }
            uint8_t profile_42 = 0x58u;
            uint8_t team = team_id_for_context(tipoff, slot / 5u);
            (void)nba_player_gameplay_movement_profile(
 ...
 * passive actor modes, including exact modes seven through ten and twelve through fourteen,
...
static bool cpu_apply_passive_mode(NbaTipoff *tipoff, unsigned slot) {
    ...
    if (actor->control_mode == 9u) {
        cpu_apply_mode_nine(tipoff, slot);
        return true;
    }
    ...
    if (actor->control_mode == 12u)
        return cpu_update_rom_shooter(tipoff, slot);
    if (actor->control_mode == 13u)
        return cpu_update_rom_layup(tipoff, slot);
    if (actor->control_mode == 14u)
        return cpu_update_rom_special_receiver(tipoff, slot);
    ...
    if (actor->control_mode == 7u) {
        cpu_apply_mode_seven(tipoff, slot);
        return true;
    }
    return false;
}

/* `$87:9244/$87:9BD3`, CPU logic: expose one passive-mode dispatch for
 * focused production replay without actor-common physics. */
...
/* `$86:B769-$B978`, CPU logic: expose the shooter parent for strict native
 * replay and its direct `$87:9F50` free-throw caller. */
bool nba_tipoff_replay_mode_twelve(NbaTipoff *tipoff, uint8_t actor) {
    return tipoff && actor < NBA_GAMEPLAY_ACTOR_COUNT &&
           cpu_update_rom_shooter(tipoff, actor);
}

/* `$87:9F11-$9F75`, free-throw CPU logic: replay the exact prep/state-nine
 * caller around `$86:B769-$B978` without the preceding actor-common pass. */
bool nba_tipoff_replay_free_throw_shooter(NbaTipoff *tipoff,
                                          uint8_t actor,
                                          bool enter_state_nine) {
    if (!tipoff || !tipoff->session || actor >= NBA_GAMEPLAY_ACTOR_COUNT)
        return false;
    tipoff->handler_actor=actor;
    tipoff->offense_side=actor/5u;
    tipoff->possession_team=(int8_t)tipoff->offense_side;
    return cpu_free_throw_shooter_caller(
        tipoff,actor,enter_state_nine);
}
...
/* `$87:9C67 -> $86:C6AD-$C758`, CPU logic: execute mode-eight knockdown
 * landing, presentation-bit cadence, and timer-expiry recovery. */
static bool cpu_update_knockdown_actor(NbaTipoff *tipoff, unsigned slot) {
    NbaTipoffActor *actor = &tipoff->actors[slot];
    if (actor->control_mode != 8u) return false;
    actor->behavior_flags_raw |= 6u;
    if (actor->special_contact_raw_56 >= 0 &&
        actor->velocity_z == 0 && fp_integer_word(actor->z_fp) == 0) {
    ...
    /* The host has two projections of native actor +$60. Keep them identical
     * so the restored ordinary mode observes `$86:9846`'s zero immediately. */
    actor->reaction_threshold = actor->contact_action_timer_raw_60;
    uint16_t presentation_phase = (uint16_t)(
        actor->contact_action_timer_raw_60 - 0x36u);
    actor->actor_status_raw_28 &= 0xFFE7u;
    if ((presentation_phase & 0x8000u) == 0u &&
        presentation_phase >= 10u && presentation_phase < 20u)
        actor->actor_status_raw_28 |= 0x0010u;
    else if ((presentation_phase & 0x8000u) == 0u &&
             presentation_phase >= 20u && presentation_phase < 30u)
        actor->actor_status_raw_28 |= 0x0008u;
    else if ((presentation_phase & 0x8000u) == 0u &&
             presentation_phase >= 30u && presentation_phase < 40u)
        actor->actor_status_raw_28 |= 0x0010u;
    if ((actor->contact_action_timer_raw_60 & 0x8000u) == 0u) return true;
    cpu_restore_normal_mode(tipoff, slot);
...
/* Host callback bridge for the native mode-11 owner flow; no single direct
 * native address. Dispatch pose, action, formation and selected-pass calls,
 * including the mode-14 receiver transition after `$86:AB2D-$AF65`. */
    ...
    if(call==NBA_OWNER_CALL_POSE)returns=cpu_owner_pose_call(t,c->slot,paired);
        ...
        if(selected>=0) {
            ...
            if(nba_tipoff_begin_rom_pass(t,c->slot,(unsigned)selected)) {
                ...
                if(special) {
                    receiver->contact_action_timer_raw_60=
...
static void cpu_dispatch_normal_actor_behavior(NbaTipoff *tipoff,
     ...
     * repair on the role pass's `$09D8` result. */
    bool repair_locomotion_base;
    if (actor->control_mode == 2u)
        repair_locomotion_base =
            tipoff->pass_receiver_raw < 0 && tipoff->possession_actor < 0;
    else if (actor->control_mode == 4u || actor->control_mode == 6u)
        repair_locomotion_base = tipoff->role_ownerless_raw_09d8 != 0u;
    else
        repair_locomotion_base = true;
     ...
     * `$86:F7F2-$F7F9`, and `$86:F920-$F930` use that same flag to gate
     * modes two, four, and six.
    ...
    if(decision_due && actor->controller_assignment_raw<0 &&
       (actor->control_mode==2 ||
        ((actor->control_mode==4 || actor->control_mode==6 ||
          mode_four_anticipation_override) &&
         !actor->movement_boost_timer)))
    ...
    if (mode_four_anticipation_override && actor->movement_magnitude_raw == 0u)
        actor->requested_direction = actor->assignment_direction;
    if (decision_due &&
        (actor->control_mode == 2u || mode_four_anticipation_override))
        actor->movement_direction = actor->requested_direction;
}
...
/* `$87:B572/$85:963D-$985F`, gameplay and CPU logic: resolve the prior pose,
 * then commit one actor common-physics step before the late mode dispatcher. */
static void cpu_commit_actor_common(NbaTipoff *tipoff, unsigned slot) {
    ...
    NbaGameplayActorCommit commit = {
        ...
        .reaction_timer_raw_60 = (actor->control_mode == 13u ||
                                  actor->control_mode == 14u) ?
            actor->contact_action_timer_raw_60 : actor->reaction_threshold,
    ...
    if (actor->control_mode == 13u || actor->control_mode == 14u) {
        actor->contact_action_timer_raw_60 = commit.reaction_timer_raw_60;
 ...
 * physics prefix; modes seven through ten and twelve through fourteen defer `$87:9244` behavior until
...
static bool cpu_move_actor(NbaTipoff *tipoff, unsigned slot) {
    ...
    if (actor->control_mode == 7u || actor->control_mode == 8u ||
        ...
        actor->control_mode == 12u || actor->control_mode == 13u ||
        actor->control_mode == 14u) {
...
/* `$87:B649/$87:B66A/$86:B7AF-$B7CA`, gameplay: project the current actor
 * pose into the ball's integer coordinates while preserving fractions. */
...
/* Host attachment adapter; no direct native address. Project native pose
 * coordinates and publish the host ownership/render mirror. */
static void ball_attach_to_actor(NbaTipoff *tipoff, unsigned owner) {
    ...
    /* Modes 12/13 own their wind-up/activity words; attachment must preserve
     * positive `$0948` and mode 13's disrupted $FFFF marker. */
    if (actor->control_mode != 12u && actor->control_mode != 13u &&
        actor->control_mode != 17u) tipoff->ball_activity_raw = 0u;
}
...
static bool cpu_try_player_knockdown_contact(NbaTipoff *tipoff,
    ...
    victim->reaction_threshold = victim->contact_action_timer_raw_60;
    ...
    actor_install_knockdown_animation(tipoff, victim, action);
...
static void cpu_apply_pose_special_impulse(NbaTipoff *tipoff,
    ...
    t->reaction_threshold = t->contact_action_timer_raw_60;
    ...
    actor_install_knockdown_animation(tipoff, t, action);
}

/* Host-only gameplay attachment adapter; no direct native address. Support
 * mode twelve's `$87:B832` by consuming raw +$2A/+$2C pose resources and
 * composing its DP $47/$48 overlap while preserving byte $46. */
static void ball_position_at_mode_twelve_actor(NbaTipoff *tipoff,
                                               unsigned owner) {
    NbaTipoffActor *actor = &tipoff->actors[owner];
    uint16_t flags = actor->actor_status_raw_28;
    int16_t offset_x=0,offset_y=0,offset_z=0;
    (void)nba_player_ball_attachment_offsets(
        tipoff->assets, actor->upper_animation_resource_raw_2a,
        actor->lower_animation_resource_raw_2c, flags,
        &offset_x, &offset_y, &offset_z);
    tipoff->ball.x_fp = fp_replace_integer_word(tipoff->ball.x_fp,
        (int16_t)(fp_integer_word(actor->x_fp) + offset_x));
    tipoff->ball.y_fp = fp_replace_integer_word(tipoff->ball.y_fp,
    ...
    tipoff->scratch_0047 = (flags & 2u) != 0u ? 0xFFFFu : 0u;
    tipoff->scratch_0046 = (uint16_t)((tipoff->scratch_0046 & 0x00FFu) |
        ((tipoff->scratch_0047 & 0x00FFu) << 8));
}
...
/* Host-only self-test; no direct native address. Covers CPU contact ranges
 * `$86:BFBA-$C34B/$86:C91E-$CCCC`, including knockdown publication. */
static bool cpu_player_contact_self_test(const NbaAssetPack *assets) {
    NbaTipoff state;
    ...
    bool high_speed_foul = cpu_try_player_knockdown_contact(&state, 0u, 5u) &&
        ...
        state.actors[0].control_mode == 8u &&
        state.actors[0].animation_state == 0x35u &&
        state.actors[0].lower_animation_state == 0x35u;
    ...
    return state.rng.state == 32u &&
        ...
        state.actors[5].animation_state == 0x35u &&
        state.actors[5].lower_animation_state == 0x35u &&
...
/* Host-only shot-gameplay adapter; no direct native address. Project actor and
 * global fields for the Bank $86 shooting parents and their typed children. */
...
/* Host-only animation adapter; no direct native address. Publish the channel
 * and gameplay words returned by mode twelve's native child calls while
 * leaving +$2A/+$2C resources for the next animation pass. */
static void actor_store_shot_action_raw(NbaTipoff *tipoff,
                                        NbaTipoffActor *actor,
...
/* Host-only animation adapter; no direct native address. Publish a shot
 * action and resolve the same-boundary pose for established non-mode-twelve
 * callers that already depend on eager resources. */
static void actor_store_shot_action(NbaTipoff *tipoff, NbaTipoffActor *actor,
                                     const NbaShotAction *s) {
    actor_store_shot_action_raw(tipoff, actor, s);
    NbaPlayerResolvedPose pose={0};
    pose.direction=actor->direction;
    actor->animation_resources_valid=nba_player_resolve_pose(
...
/* `$86:9D6E/$9DA6-$A476`, CPU logic: publish the complete launch state. Full
 * `$9D6E` callers bind launch facing to actor +$4E, install upper 17 and use
 * authoritative `$093E`; `$9DA6` callers skip that facing/animation prefix. */
static void cpu_release_rom_shot(NbaTipoff *tipoff, unsigned slot,
                                 bool native_full_entry_contract) {
    NbaTipoffActor *shooter=&tipoff->actors[slot];
    ...
    in.basket_fraction=context->anchor_x_fraction_raw_08;
    ...
    in.shot_control_17c3=tipoff->shot_control_raw_17c3;
    ...
    s.actor=actor_shot_action(tipoff,shooter);
    s.facing=native_full_entry_contract ? shooter->movement_direction : shooter->direction;
    s.scratch_47=tipoff->scratch_0047;
    ...
    /* `$86:9DBC-$9DC2` copies authoritative owner `$093E` to `$09C8/$493B`
     * before clearing it. Host ball ownership is only an integration view. */
    s.owner=(uint16_t)(int16_t)(native_full_entry_contract ?
    ...
    if (native_full_entry_contract)
        memcpy(s.player_stats,
               tipoff->roster_shot_statistics[persistent_stat],
               sizeof(s.player_stats));
    else
        memcpy(s.player_stats,shooter->shot_statistics,
               sizeof(s.player_stats));
    if (shooter->controller_assignment_raw >= 0 &&
        shooter->controller_assignment_raw < NBA_CONTROLLER_COUNT) {
        NbaControllerRecord *record=&tipoff->controllers.record[
            shooter->controller_assignment_raw];
        memcpy(s.controller_stats,record->reserved,sizeof(s.controller_stats));
    }
    s.rng=tipoff->rng;
    ...
    if (native_full_entry_contract) shooter->movement_direction=(uint8_t)s.facing;
    else shooter->direction=(uint8_t)s.facing;
    if (native_full_entry_contract) actor_store_shot_action_raw(tipoff,shooter,&s.actor);
    else actor_store_shot_action(tipoff,shooter,&s.actor);
    shooter->contact_inhibit_raw_5a=s.contact_inhibit;
    memcpy(tipoff->roster_shot_statistics[persistent_stat],
           s.player_stats, sizeof(s.player_stats));
    memcpy(shooter->shot_statistics,
           tipoff->roster_shot_statistics[persistent_stat],
           sizeof(shooter->shot_statistics));
    if (shooter->controller_assignment_raw >= 0 &&
        shooter->controller_assignment_raw < NBA_CONTROLLER_COUNT) {
        NbaControllerRecord *record=&tipoff->controllers.record[
            shooter->controller_assignment_raw];
        memcpy(record->reserved,s.controller_stats,sizeof(s.controller_stats));
    }
    context->previous_dead_ball_actor_raw_43=s.assist_43;
    context->previous_controller_actor_raw_45=(int16_t)s.assist_45;
    tipoff->rng=s.rng;
    if (native_full_entry_contract) {
        tipoff->scratch_0047=s.scratch_47;
        tipoff->scratch_0046=(uint16_t)((tipoff->scratch_0046&0x00ffu)|
            ((s.scratch_47&0x00ffu)<<8));
    }
    ...
    tipoff->shot_roster_low_raw_0914=s.roster_low;
    tipoff->shot_roster_bank_raw_0916=s.roster_bank;
    tipoff->shot_display_actor_raw_493b=s.display_shooter;
    if(tipoff->fouls.shooting_foul_raw_09bc && !in.free_throw_0978)
        tipoff->deferred_shot_foul_phase_raw_0a02=1;
}

/* `$87:AEC3`, animation system: resolve a close-finish pose from raw actor
 * +$4E without consuming an animation cadence tick. */
static bool cpu_resolve_close_finish_pose(NbaTipoff *tipoff,
                                          NbaTipoffActor *actor) {
    NbaPlayerAnimationChannels channels=actor_animation_channels(actor);
    NbaPlayerResolvedPose pose={0};
    pose.direction=actor->direction;
    pose.mirror_flags=actor->actor_status_raw_28;
    actor->animation_resources_valid=nba_player_resolve_pose(
        tipoff->assets,&channels,actor->movement_direction,
        actor->free_throw_launch_half_raw_a8!=0u,
        actor->animation_variant_raw_6c,&pose);
    if(!actor->animation_resources_valid)return false;
    actor->upper_animation_resource_raw_2a=pose.upper_resource;
    actor->lower_animation_resource_raw_2c=pose.lower_resource;
    actor->upper_state_snapshot_raw_34=actor->animation_state;
    actor->lower_state_snapshot_raw_36=actor->lower_animation_state;
    actor->upper_phase_snapshot_raw_3e=actor->rom_upper_animation_phase_raw_3a;
    actor->lower_phase_snapshot_raw_40=actor->rom_lower_animation_phase_raw_3c;
    actor->direction=(uint8_t)pose.direction;
    actor->actor_status_raw_28=pose.mirror_flags;
    return true;
}

/* `$87:B649/$B66A/$B832`, animation attachment: compose a selected native
 * pose point into integer coordinates while preserving every fraction word. */
static bool cpu_close_finish_attachment(NbaTipoff *tipoff,unsigned slot,
        uint8_t point,bool write_xy,bool write_z,bool remember_x,
        int16_t *offset_x_out,int16_t *offset_y_out,int16_t *offset_z_out) {
    NbaTipoffActor *actor=&tipoff->actors[slot];
    int16_t ox=0,oy=0,oz=0;
    if(!actor->animation_resources_valid ||
       !nba_player_ball_attachment_point_offsets(tipoff->assets,
            actor->upper_animation_resource_raw_2a,
            actor->lower_animation_resource_raw_2c,
            actor->actor_status_raw_28,point,&ox,&oy,&oz))return false;
    uint16_t flags=actor->actor_status_raw_28;
    if((int16_t)flags<0)flags^=3u;
    tipoff->scratch_0047=(flags&2u)!=0u?0xffffu:0u;
    tipoff->scratch_0046=(uint16_t)((tipoff->scratch_0046&0x00ffu)|
        ((tipoff->scratch_0047&0x00ffu)<<8));
    if(remember_x)tipoff->shot_previous_actor_x_raw_0922=
        (uint16_t)fp_integer_word(tipoff->ball.x_fp);
    if(write_xy){
        tipoff->ball.x_fp=fp_replace_integer_word(tipoff->ball.x_fp,
            (int16_t)(fp_integer_word(actor->x_fp)+ox));
        tipoff->ball.y_fp=fp_replace_integer_word(tipoff->ball.y_fp,
            (int16_t)(fp_integer_word(actor->y_fp)+oy));
    }
    if(write_z)tipoff->ball.z_fp=fp_replace_integer_word(tipoff->ball.z_fp,
        (int16_t)(fp_integer_word(actor->z_fp)+oz));
    if(offset_x_out)*offset_x_out=ox;
    if(offset_y_out)*offset_y_out=oy;
    if(offset_z_out)*offset_z_out=oz;
    return true;
}

/* `$86:A9D0-$AA69`, CPU logic/gameplay: shared terminal close-finish release used by behavior
 * modes 13 and 14. Publish its exact two-point shot globals and attempt stat. */
static void cpu_finish_rom_close_shot(NbaTipoff *tipoff, unsigned slot) {
    NbaTipoffActor *actor = &tipoff->actors[slot];
    tipoff->shot_actor_raw_09c8 = tipoff->possession_actor;
    tipoff->shot_display_actor_raw_493b=(uint16_t)(int16_t)tipoff->possession_actor;
    tipoff->possession_actor=-1;
    tipoff->dead_ball_raw_0966=0xffffu;
    tipoff->shot_value_raw = 2u;
    tipoff->hud.shot_category_raw_4939=2u;
    tipoff->rim_raw_096a = 2u;
    tipoff->dead_ball_raw_0968=0u;
    tipoff->rim_raw_0920=0u;
    ...
    actor->contact_action_timer_raw_60=0u;
    actor->reaction_threshold=0u;
    tipoff->free_throw_flight_timer_raw_0930=0x04b0u;
    tipoff->catch_actor_record_raw_0910=0x3eebu;
    ...
    unsigned persistent=(slot/NBA_MATCH_LINEUP_SIZE)*NBA_MATCH_ROSTER_SIZE+
        actor->roster_slot;
    ++tipoff->roster_shot_statistics[persistent][0];
    memcpy(actor->shot_statistics,tipoff->roster_shot_statistics[persistent],
           sizeof(actor->shot_statistics));
    if(actor->controller_assignment_raw>=0 &&
       actor->controller_assignment_raw<NBA_CONTROLLER_COUNT)
        ++tipoff->controllers.record[actor->controller_assignment_raw].reserved[0];
}

/* `$86:986D-$994B`, CPU logic/gameplay close-finish continuation: select the close landing or
 * hold branch after `$86:A9D0`, including pose-point-one placement/effect. */
static void cpu_finish_rom_close_shot_landing(NbaTipoff *tipoff,unsigned slot) {
    NbaTipoffActor *actor=&tipoff->actors[slot];
    int16_t basket_x=(int16_t)tipoff->court_presentation.basket_x_3fef;
    int16_t basket_y=tipoff->rim_effect.reference_y_raw_3ff3;
    uint16_t dx=(uint16_t)(fp_integer_word(tipoff->ball.x_fp)-basket_x);
    uint16_t dy=(uint16_t)(fp_integer_word(tipoff->ball.y_fp)-basket_y);
    if((int16_t)dx<0)dx=(uint16_t)(0u-dx);
    if((int16_t)dy<0)dy=(uint16_t)(0u-dy);
    if((int16_t)(uint16_t)(dx-dy)<0){uint16_t swap=dx;dx=dy;dy=swap;}
    if((uint16_t)(dx+(dy>>2))>=8u)return;
    tipoff->rim_raw_13e7|=0x20u;
    if(actor->pass_direction_raw>=0x1bu){
        actor->control_mode=7u;
        actor->contact_action_timer_raw_60=0x00b4u;
        actor->reaction_threshold=0x00b4u;
        return;
    }
    uint16_t animation=0u;
    if(!nba_shot_close_finish_landing(tipoff->assets,
            actor->mode13_variant_raw_58,&animation))return;
    (void)actor_queue_animation_command(tipoff,actor,
        NBA_ANIMATION_INSTALL_BOTH,animation);
    actor->animation_upper_queue_raw_1c[0]=0x24u;
    actor->animation_lower_queue_raw_22[0]=0x24u;
    actor->animation_upper_queue_cursor_raw_18=0u;
    actor->animation_lower_queue_cursor_raw_1a=0u;
    actor->rom_upper_animation_phase_raw_3a=0u;
    actor->rom_lower_animation_phase_raw_3c=0u;
    actor->upper_animation_accumulator_raw_42=0u;
    actor->lower_animation_accumulator_raw_44=0u;
    ...
    actor->contact_action_timer_raw_60=0x18u;
    actor->reaction_threshold=0x18u;
    if(cpu_resolve_close_finish_pose(tipoff,actor)){
        int16_t ox=0,oy=0,oz=0;
        if(cpu_close_finish_attachment(tipoff,slot,1u,false,false,false,
                &ox,&oy,&oz)){
            actor->x_fp=fp_replace_integer_word(actor->x_fp,
                (int16_t)(basket_x-ox));
            actor->y_fp=fp_replace_integer_word(actor->y_fp,
                (int16_t)(basket_y-oy));
            actor->z_fp=fp_replace_integer_word(actor->z_fp,(int16_t)(80-oz));
        }
    }
    (void)nba_gameplay_effect_start(&tipoff->rim_effect,4u);
}

/* `$87:9BD3[13] -> $87:9C49 -> $86:A7DA-$A993`, CPU logic: carried-ball
 * close finish after common physics. Inline `$A994-$A9CF` is data only. */
static bool cpu_update_rom_layup(NbaTipoff *tipoff, unsigned slot) {
    NbaTipoffActor *actor = &tipoff->actors[slot];
    if (tipoff->possession_actor != (int8_t)slot) {
    ...
    int16_t dx=(int16_t)(uint16_t)((uint16_t)actor->velocity_x-
        (uint16_t)actor->mode13_baseline_velocity_x);
    int16_t dy=(int16_t)(uint16_t)((uint16_t)actor->velocity_y-
        (uint16_t)actor->mode13_baseline_velocity_y);
    bool disturbed = dx < -0x50 || dx > 0x50 || dy < -0x50 || dy > 0x50;
    ...
    if (airborne && (wrong_upper ||
        ...
        actor->movement_direction=(uint8_t)(actor->anchor_direction_raw>>1);
        (void)actor_queue_animation_command(tipoff,actor,
            NBA_ANIMATION_CANCEL_LOWER,0u);
        (void)actor_queue_animation_command(tipoff,actor,
            NBA_ANIMATION_INSTALL_UPPER,0x17u);
        if(cpu_resolve_close_finish_pose(tipoff,actor))
            (void)cpu_close_finish_attachment(tipoff,slot,0u,true,true,true,
                NULL,NULL,NULL);
        cpu_release_rom_shot(tipoff, slot, true);
        return true;
    }

    tipoff->close_finish_timing_raw_094e=0x1eu;
    if(actor->special_contact_raw_56!=6)
        tipoff->rim_force_raw_1866=(uint16_t)(0x34ebu+slot*0x100u);
    actor->movement_speed_raw_4a=0u;
    uint16_t timer = actor->contact_action_timer_raw_60;
    if (timer == 0x24u || (timer < 0x24u && !airborne &&
                          actor->lower_animation_state!=0x25u)) {
        actor->velocity_z =
            (actor->special_contact_raw_56 == 0 ||
             (actor->special_contact_raw_56 == 6 && actor->movement_direction == 3u)) ?
            0x0270 : 0x0264;
    }
    timer=(uint16_t)(timer-2u);
    ...
    if(timer==0u || (timer&0x8000u)!=0u){
        ...
        cpu_finish_rom_close_shot_landing(tipoff,slot);
        return true;
    }
    if(cpu_resolve_close_finish_pose(tipoff,actor)){
        int16_t ox=0,oy=0,oz=0;
        if(cpu_close_finish_attachment(tipoff,slot,0u,true,false,true,
                &ox,&oy,&oz))
            tipoff->ball.z_fp=fp_replace_integer_word(tipoff->ball.z_fp,
                (int16_t)(fp_integer_word(actor->z_fp)+oz));
    }
    tipoff->ball.velocity_z=(int16_t)0xfe98u;
    if(timer>0x24u)return true;
    if(actor->special_contact_raw_56<=0){
        uint16_t offset=(uint16_t)(timer-8u);
        if((int16_t)offset<=0)
            actor->movement_direction=actor->special_contact_raw_56<0 ?
                actor->requested_direction:(uint8_t)(actor->requested_direction^4u);
        else {
            uint8_t turn=0u;
            if(!nba_shot_close_finish_turn(tipoff->assets,
                    actor->special_contact_raw_56==0,offset,&turn))return true;
            actor->movement_direction=(uint8_t)((actor->requested_direction+turn)&7u);
        }
    }
    if(cpu_resolve_close_finish_pose(tipoff,actor))
        if(cpu_close_finish_attachment(tipoff,slot,0u,true,true,false,
                NULL,NULL,NULL))
            tipoff->shot_previous_actor_x_raw_0922=
                (uint16_t)fp_integer_word(actor->x_fp);
    return true;
}

/* `$87:9BD3[14] -> $87:9C4E -> $86:B154-$B334`, CPU logic: execute the
 * special receiver parent after common physics and global work. It preserves
 * a valid pass relationship, installs the grounded jump, attaches an owned
 * airborne ball, and uses the shared close-finish terminal chain. */
static bool cpu_update_rom_special_receiver(NbaTipoff *tipoff, unsigned slot) {
    NbaTipoffActor *actor = &tipoff->actors[slot];
    ...
    uint16_t timer = (uint16_t)(actor->contact_action_timer_raw_60 - 2u);
    if ((int16_t)(uint16_t)(timer - 1u) < 0) {
        if (tipoff->possession_actor == (int8_t)slot) {
            tipoff->ball_activity_raw = 1u;
            cpu_finish_rom_close_shot(tipoff, slot);
            cpu_finish_rom_close_shot_landing(tipoff, slot);
    ...
    if (!airborne) {
        ...
        if (actor->animation_state < 0x18u || actor->animation_state >= 0x1Fu) {
            (void)actor_queue_animation_command(tipoff,actor,
                NBA_ANIMATION_CANCEL_UPPER,0u);
            (void)actor_queue_animation_command(tipoff,actor,
                NBA_ANIMATION_CANCEL_LOWER,0u);
            (void)actor_queue_animation_command(tipoff,actor,
                NBA_ANIMATION_INSTALL_UPPER,actor->pass_direction_raw);
            actor->animation_upper_queue_raw_1c[0]=0x24u;
            actor->animation_upper_queue_cursor_raw_18=0u;
            (void)actor_queue_animation_command(tipoff,actor,
                NBA_ANIMATION_INSTALL_LOWER,0x1fu);
            uint16_t lower_queue=0u;
            if(!nba_shot_special_receiver_lower_queue(tipoff->assets,
                    actor->mode13_variant_raw_58,&lower_queue))return true;
            actor->animation_lower_queue_raw_22[1]=lower_queue;
            actor->animation_lower_queue_raw_22[0]=0x24u;
            actor->animation_lower_queue_cursor_raw_1a=1u;
            return true;
        }
            ...
            actor->velocity_z =
                (actor->special_contact_raw_56 == 0 ||
                 (actor->special_contact_raw_56 == 6 &&
                  actor->movement_direction == 3u)) ? 0x0270 : 0x0264;
        }
        return true;
    }
    ...
    if (actor->special_contact_raw_56 != 0) {
        ...
        uint16_t offset=(uint16_t)(timer-8u);
        if((int16_t)offset<=0)
            actor->movement_direction=(uint8_t)(actor->requested_direction^4u);
        else {
            uint8_t turn=0u;
            if(!nba_shot_close_finish_turn(tipoff->assets,true,offset,&turn))
                return true;
            actor->movement_direction=(uint8_t)((actor->requested_direction+turn)&7u);
        }
    }
...
/* Host-only CPU replay boundary; no direct native address. Invoke only the
 * `$86:A7DA-$A993` parent after a caller has prepared its entry state. */
bool nba_tipoff_replay_mode13_close_finish(NbaTipoff *tipoff, uint8_t actor) {
    return tipoff && actor < NBA_GAMEPLAY_ACTOR_COUNT &&
           cpu_update_rom_layup(tipoff, actor);
}

/* Host-only CPU replay boundary; no direct native address. Retain the existing
 * `$86:B154-$B334` mode-fourteen parent for shared-child regression checks. */
bool nba_tipoff_replay_mode14_close_finish(NbaTipoff *tipoff, uint8_t actor) {
    return tipoff && actor < NBA_GAMEPLAY_ACTOR_COUNT &&
           cpu_update_rom_special_receiver(tipoff, actor);
}

/* Host-only CPU replay boundary; no direct native address. Invoke the existing
 * `$86:B34F-$B624` close-finish initializer without the production scheduler. */
bool nba_tipoff_replay_close_finish_start(NbaTipoff *tipoff, uint8_t actor) {
    return tipoff && actor < NBA_GAMEPLAY_ACTOR_COUNT &&
           cpu_start_rom_layup(tipoff, actor);
}

/* Host-only CPU-logic special-receiver test; no direct native address. Check
 * the `$86:B154-$B334` caller's shared launch, terminal landing and restore. */
static bool cpu_special_receiver_self_test(const NbaAssetPack *assets) {
    ...
    seed = state.rng.state;
    if (!cpu_update_rom_special_receiver(&state, 0u) ||
        state.actors[0].control_mode != 16u ||
        state.possession_actor != -1 || state.ball.owner_actor != -1 ||
    ...
    state.actors[0].control_mode = 14u;
    state.actors[0].contact_action_timer_raw_60 = 40u;
    state.actors[0].reaction_threshold = 0xDEADu;
    state.actors[0].animation_state = 0x18u;
    state.actors[0].special_contact_raw_56 = 0;
    if (!cpu_update_rom_special_receiver(&state, 0u) ||
        state.actors[0].contact_action_timer_raw_60 != 38u ||
    ...
    state.pass_receiver_raw = 0;
    state.actors[0].control_mode = 14u;
    state.actors[0].contact_action_timer_raw_60 = 22u;
    state.actors[0].animation_state = 0x18u;
...
/* `$86:B979-$BAA1`, CPU logic: execute the special-shot parent and enter the
 * shared launch child. */
static bool cpu_update_rom_special_shooter(NbaTipoff *tipoff,unsigned slot) {
    ...
    if(result==NBA_SPECIAL_SHOT_RELEASE) {
        cpu_release_rom_shot(tipoff,slot,false);
...
/* `$86:B769-$B978`, CPU logic: execute mode twelve's ownership/pump gate,
 * pose attachment, wind-up, sidestep/jump and human/CPU release branches.
 * `$85:963D` physics belongs to the preceding actor-common caller. */
static bool cpu_update_rom_shooter(NbaTipoff *tipoff, unsigned slot) {
    NbaTipoffActor *shooter = &tipoff->actors[slot];
    ...
    if (owner == NBA_SHOT_PUMP_WAIT) return true;
    if (owner == NBA_SHOT_PUMP_CANCEL) {
        if (actor_queue_animation_command(tipoff, shooter,
                NBA_ANIMATION_CANCEL_UPPER, 0u) &&
            actor_queue_animation_command(tipoff, shooter,
                NBA_ANIMATION_CANCEL_LOWER, 0u)) {
            tipoff->ball_activity_raw=0u;
            ...
            tipoff->live_state_raw=0u;
            tipoff->ball.z_fp=(int32_t)0x28*256 +
                              (tipoff->ball.z_fp & 255);
            tipoff->ball.velocity_z=0;
            if (tipoff->attached_ball_state_raw_09f6 != 0u)
                tipoff->attached_ball_state_raw_09f6=2u;
            tipoff->dead_ball_raw_0968=0x28u;
            shooter->exact_shot_animation=false;
    ...
    tipoff->live_state_raw = 2u;
    ball_position_at_mode_twelve_actor(tipoff, slot);
    tipoff->shot_previous_actor_x_raw_0922 =
        (uint16_t)fp_integer_word(shooter->x_fp);
    int controller = shooter->controller_assignment_raw;
    uint16_t buttons = controller >= 0 && controller < NBA_CONTROLLER_COUNT ?
        tipoff->controllers.record[controller].held : 0u;
    ...
    if (stage == NBA_SHOT_DELAY) {
        ...
        nba_shot_action_windup_button(&action,shooter->controller_assignment_raw,
            tipoff->fouls.free_throw_state_raw_0978,buttons);
        shooter->behavior_flags_raw=action.flags;
        return true;
    }
    if (stage == NBA_SHOT_JUMP) {
        int16_t basket_x=tipoff->team_context[slot/5u].anchor_x_raw_0a;
        NbaShotSidestepInput step={fp_integer_word(shooter->x_fp),
            fp_integer_word(shooter->y_fp),basket_x,
            shooter->movement_magnitude_raw,shooter->anchor_distance_raw,
            tipoff->fouls.free_throw_state_raw_0978,tipoff->rng.state};
        nba_shot_action_sidestep(&action,&step);
        shooter->velocity_x=action.velocity_x;
        shooter->velocity_y=action.velocity_y;
        if (actor_queue_animation_command(tipoff, shooter,
                NBA_ANIMATION_CANCEL_LOWER, 0u)) {
            shooter->velocity_z=0x0210;
            (void)actor_queue_animation_command(tipoff, shooter,
                NBA_ANIMATION_INSTALL_LOWER, 0x32u);
        }
        return true;
    }
    /* `$86:B8CA-$B978` turns only once per call once lower +$44 >= $600,
     * then invokes the fully retained `$86:9D6E` child on release. */
    int16_t basket_x=tipoff->team_context[slot/5u].anchor_x_raw_0a;
    NbaShotGateInput gate={
        fp_integer_word(shooter->x_fp),fp_integer_word(shooter->y_fp),
        fp_integer_word(shooter->z_fp),shooter->velocity_z,
        shooter->controller_assignment_raw,
        shooter->lower_animation_accumulator_raw_44,
        tipoff->fouls.free_throw_state_raw_0978,tipoff->rng.state,buttons,
        shooter->movement_direction,basket_x
    };
    NbaShotGate decision=nba_shot_action_gate(&gate);
    shooter->movement_direction=(uint8_t)gate.facing;
...
/* Host-only CPU-logic mode-17 caller test; no direct native address. Exercise
 * the rare facing-dependent selector, jump, and `$86:9DA6` launch path. */
...
/* Host-only CPU-logic integration test; no direct native address. Exercise
 * `$86:B769-$B978` directly for branch and persistent-ball invariants; the
 * vector probe separately verifies its public production scheduler caller. */
static bool cpu_shot_branches_self_test(const NbaAssetPack *assets,
    ...
    s.ball.owner_actor=-1;s.ball.state=NBA_BALL_LOOSE;
    ...
    int32_t old_x=a->x_fp,old_y=a->y_fp,old_z=a->z_fp;
    int16_t old_vx=a->velocity_x,old_vy=a->velocity_y,old_vz=a->velocity_z;
    if (!cpu_update_rom_shooter(&s,0) || s.ball_activity_raw!=3u ||
        a->x_fp!=old_x || a->y_fp!=old_y || a->z_fp!=old_z ||
        a->velocity_x!=old_vx || a->velocity_y!=old_vy ||
        a->velocity_z!=old_vz || s.ball.owner_actor!=0 ||
        s.ball.state!=NBA_BALL_ATTACHED || (s.ball.x_fp&255)!=37 ||
        (s.ball.y_fp&255)!=83 || (s.ball.z_fp&255)!=171) return false;
    s.ball_activity_raw=0x1cu;a->movement_magnitude_raw=1u;
 ...
 * scheduler/physics: run one 30-Hz pass; modes seven through ten and twelve through fourteen advance their
...
static void cpu_update_all_actors(NbaTipoff *tipoff) {
    ...
    for (unsigned actor = 0; actor < NBA_GAMEPLAY_ACTOR_COUNT; ++actor) {
        ...
        bool deferred_behavior = state->control_mode == 7u ||
                                 ...
                                 state->control_mode == 12u ||
                                 state->control_mode == 13u ||
         ...
         * for modes seven through ten/twelve/thirteen/fourteen without changing other modes. */
        if (deferred_behavior)
            cpu_advance_actor_animation(tipoff, state);
        if (state->control_mode != 8u &&
            state->contact_inhibit_raw_5a != 0u)
            state->contact_inhibit_raw_5a =
                state->contact_inhibit_raw_5a > 2u ?
                (uint16_t)(state->contact_inhibit_raw_5a - 2u) : 0u;
        if (state->control_mode != 8u && state->control_mode != 9u &&
            state->recovery_inhibit_raw != 0u)
...
/* `$87:90A5-$90C2`, actor scheduler: apply mode-eight/mode-nine signed
 * inhibit countdowns after contacts and before `$87:9244` dispatch. */
static void cpu_prepare_mode_eight_nine_behavior(NbaTipoff *tipoff) {
    for (unsigned actor = 0; actor < NBA_GAMEPLAY_ACTOR_COUNT; ++actor) {
        NbaTipoffActor *state = &tipoff->actors[actor];
        if ((state->control_mode == 8u || state->control_mode == 9u) &&
            state->recovery_inhibit_raw != 0u) {
            uint16_t remaining =
                (uint16_t)(state->recovery_inhibit_raw - 2u);
            state->recovery_inhibit_raw =
                (remaining & 0x8000u) == 0u ? remaining : 0u;
        }
        if (state->control_mode == 8u &&
            state->contact_inhibit_raw_5a != 0u) {
            uint16_t remaining =
                (uint16_t)(state->contact_inhibit_raw_5a - 2u);
            state->contact_inhibit_raw_5a =
                (remaining & 0x8000u) == 0u ? remaining : 0u;
        }
    }
 ...
 * sweep latches, then dispatch modes seven through ten/twelve/thirteen/fourteen and normal actors after
...
static void cpu_update_actor_behaviors(NbaTipoff *tipoff) {
    ...
    for (unsigned actor = 0; actor < NBA_GAMEPLAY_ACTOR_COUNT; ++actor) {
        ...
        if (tipoff->actors[actor].control_mode == 7u ||
            ...
            tipoff->actors[actor].control_mode == 13u ||
            tipoff->actors[actor].control_mode == 14u)
 ...
 * behavior pass immediately or after the retained acquisition deferral. */
...
/* `$87:9F11-$9F3D`, free-throw gameplay: enter state nine, clear +$4A,
 * cancel both animation channels and publish command $16 before the caller
 * falls through to `$87:9F3E` in the same actor pass. */
static bool free_throw_enter_launch_state(NbaTipoff *tipoff,
                                          NbaTipoffActor *actor) {
    tipoff->fouls.free_throw_state_raw_0978 = 9u;
    actor->movement_speed_raw_4a = 0u;
    if (!actor_queue_animation_command(
            tipoff, actor, NBA_ANIMATION_CANCEL_UPPER, 0u) ||
        !actor_queue_animation_command(
            tipoff, actor, NBA_ANIMATION_CANCEL_LOWER, 0u)) return false;
    tipoff->ball_activity_raw = 1u;
    if (!actor_queue_animation_command(
            tipoff, actor, NBA_ANIMATION_INSTALL_BOTH, 0x16u)) return false;
    actor->behavior_flags_raw |= 4u;
    actor->exact_shot_animation = true;
    return true;
}

/* `$87:9CBF/$87:AAB2/$85:963D`, free-throw gameplay: execute the one common
 * animation/physics prefix that precedes the aiming and state-nine caller. */
static void cpu_free_throw_actor_common(NbaTipoff *tipoff,
                                        unsigned shooter) {
    NbaTipoffActor *actor=&tipoff->actors[shooter];
    cpu_ease_display_direction(actor);
    cpu_advance_actor_animation(tipoff, actor);
    cpu_commit_actor_common(tipoff, shooter);
}

/* `$87:9F76-$A017`, free-throw gameplay: apply the state-ten resolution
 * predicates. The launch-frame caller passes false because `$85:963D` has
 * already run and native does not integrate the new ball again at 9F76. */
static void cpu_free_throw_resolution_dispatch(NbaTipoff *tipoff,
                                                uint8_t shooter,
                                                bool advance_ball) {
    if (advance_ball) {
    ...
    completion.state_raw_0978 = tipoff->fouls.free_throw_state_raw_0978;
    ...
    if (!nba_gameplay_free_throw_resolution_step(
            &completion, true, tipoff->possession_actor < 0,
            tipoff->shot_value_raw, tipoff->rim_raw_097c,
            tipoff->free_throw_resolution_raw_0972,
            fp_integer_word(tipoff->actors[shooter].x_fp),
            fp_integer_word(tipoff->actors[shooter].y_fp))) return;
    tipoff->fouls.free_throw_state_raw_0978 = completion.state_raw_0978;
    if (tipoff->fouls.free_throw_sequence_raw_097a == 0u ||
        completion.state_raw_0978 != 11u) return;
    /* `$3EEF/$3EF3/$3EF7/$3EF9/$3EFB/$3EFD` are the gameplay-ball integer
     * position and velocity words. Preserve fractions between attempts. */
...
/* `$87:9F11-$9F75`, free-throw CPU logic: optionally publish the state-nine
 * pose, force shot assistance only around `$86:B769`, restore the full word,
 * and perform the ownership-gated release completion and state-ten handoff. */
static bool cpu_free_throw_shooter_caller(NbaTipoff *tipoff,
                                          uint8_t shooter,
                                          bool enter_state_nine) {
    NbaTipoffActor *actor=&tipoff->actors[shooter];
    if(enter_state_nine && !free_throw_enter_launch_state(tipoff,actor))
        return false;
    uint16_t saved_assistance=tipoff->session->config.options[5];
    tipoff->session->config.options[5]=1u;
    (void)cpu_update_rom_shooter(tipoff,shooter);
    tipoff->session->config.options[5]=saved_assistance;
    if(tipoff->possession_actor>=0)return true;
    tipoff->shot_result_resolved=false;
    ...
    tipoff->fouls.free_throw_state_raw_0978=completion.state_raw_0978;
    tipoff->fouls.free_throw_sequence_raw_097a=completion.attempts_raw_097a;
    tipoff->free_throw_upload_raw_180b=completion.upload_raw_180b;
    tipoff->free_throw_upload_raw_180c=completion.upload_raw_180c;
    cpu_free_throw_resolution_dispatch(tipoff,shooter,false);
    return true;
}

/* `$87:9CBF-$A017`, free-throw gameplay: retain CPU and controller aiming,
 * with `$87:9F11-$9F75` calling the converted mode-twelve shooter directly.
 * Aim artwork `$0988-$098E` and PPU scheduling remain outside this gameplay
 * state adapter. */
static bool cpu_update_free_throw_scene(NbaTipoff *tipoff) {
    ...
    tipoff->possession_team = (int8_t)tipoff->offense_side;
    NbaTipoffActor *actor = &tipoff->actors[shooter];
    if (*state == 3u || *state == 4u || *state == 5u || *state == 9u)
        cpu_free_throw_actor_common(tipoff, shooter);
    ...
    if (*state == 1u) {
        ...
        bool ready = true;
        for (unsigned index = 0; index < NBA_GAMEPLAY_ACTOR_COUNT; ++index) {
            CpuFreeThrowTarget target = targets[index];
            if (index == shooter && tipoff->ball.owner_actor < 0) {
                target.x = fp_round(tipoff->ball.x_fp);
                target.y = fp_round(tipoff->ball.y_fp);
            }
            if (!cpu_free_throw_move_actor(&tipoff->actors[index], &target))
                ready = false;
            cpu_advance_actor_animation(tipoff, &tipoff->actors[index]);
        }
        if (tipoff->ball.owner_actor < 0) {
            (void)cpu_update_live_ball(tipoff);
        ...
        if (*state == 4u || *state == 5u || human_path) {
            ...
            *state = human.state_raw_0978;
            ...
            if (result == NBA_HUMAN_FREE_THROW_LAUNCH) {
                (void)cpu_free_throw_shooter_caller(tipoff,shooter,true);
                return true;
            }
    ...
    if (*state == 9u) {
        (void)cpu_free_throw_shooter_caller(tipoff,shooter,false);
        return true;
    }
    if (*state == 10u) {
        cpu_free_throw_resolution_dispatch(tipoff, shooter, true);
        return true;
    }
    if (*state >= 11u) {
        NbaGameplayFreeThrowCompletion completion = {0};
...
/* Host-only CPU-logic integration test; no direct native address. Exercise the
 * `$87:9F11-$9F75` mode-twelve caller through the production stripe scene. */
static bool cpu_free_throw_scene_self_test(const NbaAssetPack *assets,
                                           NbaSession *session) {
    NbaSession fixture=*session;
    uint16_t supplied_assistance=session->config.options[5];
    NbaTipoff state;
    memset(&state, 0, sizeof(state));
    state.assets=assets;state.session=&fixture;publish_exhibition_team_ids(&state);
    for (unsigned i = 0; i < NBA_GAMEPLAY_ACTOR_COUNT; ++i)
    ...
    NbaTipoff live = {0};
    live.assets=assets;live.session=&fixture;publish_exhibition_team_ids(&live);
    ...
    live.possession_actor=0;live.ball.owner_actor=-1;
    live.ball.state=NBA_BALL_LOOSE;
    live.actors[0].control_mode=2u;
    live.actors[0].roster_slot=0u;
    live.actors[0].team_group_raw_6e=0u;
    live.actors[0].shot_stamina_raw_18=0x7fffu;
    live.team_context[0].anchor_x_raw_0a=-336;
    fixture.config.options[5]=0x3456u;
    if (!cpu_update_free_throw_scene(&live) ||
        live.fouls.free_throw_state_raw_0978 != 9u ||
        live.free_throw_aim_x_raw_0980 != 10u ||
        live.ball_activity_raw != 3u || live.possession_actor != 0 ||
        live.ball.owner_actor != -1 || live.ball.state != NBA_BALL_LOOSE ||
        live.actors[0].control_mode != 2u ||
        live.actors[0].animation_state != 0x16u ||
        live.actors[0].lower_animation_state != 0x16u ||
        live.actors[0].movement_speed_raw_4a != 0u ||
        (live.actors[0].behavior_flags_raw & 6u) != 6u ||
        fixture.config.options[5] != 0x3456u ||
        live.shot_launch_serial != 0u) return false;

    /* Directly distinguish the raw pre-command pose from command $16. B832
     * must consume +$2A/+$2C even though 9F11 invalidated host resources. */
    NbaTipoff prep={0};prep.assets=assets;prep.session=&fixture;
    publish_exhibition_team_ids(&prep);prep.possession_actor=0;
    prep.ball.owner_actor=-1;prep.ball.state=NBA_BALL_LOOSE;
    prep.ball.x_fp=0x35;prep.ball.y_fp=0x53;prep.ball.z_fp=0x75;
    prep.team_context[0].anchor_x_raw_0a=-336;
    NbaTipoffActor *prepared=&prep.actors[0];
    prepared->control_mode=2u;prepared->roster_slot=0u;
    prepared->team_group_raw_6e=0u;prepared->x_fp=-210*256;
    prepared->movement_speed_raw_4a=0x1234u;
    prepared->animation_state=0x0cu;prepared->lower_animation_state=0x0cu;
    prepared->base_animation_state_raw_38=0x0cu;
    prepared->upper_animation_lock_raw_46=1u;
    prepared->lower_animation_lock_raw_48=1u;
    prepared->upper_animation_resource_raw_2a=0x0094u;
    prepared->lower_animation_resource_raw_2c=0x073bu;
    prepared->animation_resources_valid=true;
    prepared->shot_stamina_raw_18=0x7fffu;
    int16_t ox=0,oy=0,oz=0;
    if(!nba_player_ball_attachment_offsets(assets,0x0094u,0x073bu,0u,
            &ox,&oy,&oz) || !free_throw_enter_launch_state(&prep,prepared) ||
       prepared->upper_animation_resource_raw_2a!=0x0094u ||
       prepared->lower_animation_resource_raw_2c!=0x073bu ||
       prepared->animation_resources_valid || prepared->control_mode!=2u ||
       prepared->movement_speed_raw_4a!=0u ||
       !cpu_update_rom_shooter(&prep,0u) || prep.ball_activity_raw!=3u ||
       fp_integer_word(prep.ball.x_fp)!=(int16_t)(-210+ox) ||
       fp_integer_word(prep.ball.y_fp)!=oy ||
       fp_integer_word(prep.ball.z_fp)!=oz ||
       (prep.ball.x_fp&255)!=0x35 || (prep.ball.y_fp&255)!=0x53 ||
       (prep.ball.z_fp&255)!=0x75 || prep.ball.owner_actor!=-1 ||
       prep.ball.state!=NBA_BALL_LOOSE || prep.shot_launch_serial!=0u)
        return false;

    /* A later genuine state-nine call forces assistance only around B769,
     * completes solely after raw ownership becomes negative, and reaches the
     * state-ten predicates without advancing the newly launched ball. */
    prep.fouls.free_throw_state_raw_0978=9u;
    prep.fouls.free_throw_sequence_raw_097a=2u;
    prep.ball_activity_raw=0xffffu;prep.possession_actor=0;
    prepared->controller_assignment_raw=-1;
    prepared->lower_animation_accumulator_raw_44=0x0600u;
    prepared->velocity_x=0x0100;prepared->velocity_y=0;
    prepared->velocity_z=0;prepared->x_fp=-210*256;
    fixture.config.options[5]=0x4567u;
    int32_t old_x=prepared->x_fp;
    if(!cpu_update_free_throw_scene(&prep) ||
       fixture.config.options[5]!=0x4567u ||
       prep.fouls.free_throw_state_raw_0978!=10u ||
       prep.fouls.free_throw_sequence_raw_097a!=1u ||
       prep.possession_actor!=-1 || prep.ball.owner_actor!=-1 ||
       prep.ball.state!=NBA_BALL_SHOT || prep.shot_launch_serial!=1u ||
       prep.free_throw_upload_raw_180b!=0x800cu ||
       prep.free_throw_upload_raw_180c!=0x8080u ||
       prepared->x_fp!=old_x+0x0200 ||
       prep.ball.x_fp!=(int32_t)(int16_t)prep.last_shot_launch.x*256+
                          (prep.last_shot_launch.x_fraction>>8) ||
       prep.ball.y_fp!=(int32_t)(int16_t)prep.last_shot_launch.y*256+
                          (prep.last_shot_launch.y_fraction>>8) ||
       prep.ball.z_fp!=(int32_t)(int16_t)prep.last_shot_launch.z*256+
                          (prep.last_shot_launch.z_fraction>>8) ||
       prep.ball.velocity_x!=prep.last_shot_launch.velocity_x ||
       prep.ball.velocity_y!=prep.last_shot_launch.velocity_y ||
       prep.ball.velocity_z!=prep.last_shot_launch.velocity_z)
        return false;
...
/* `$87:923D-$9298` and dispatched gameplay children: update possession,
 * ball/contact globals, team roles, and the later actor behavior sweep. */
...
/* Host-only gameplay initializer; no single direct native address. Validate
 * subsystem contracts, including mode-twelve parent/caller invariants, then
 * construct the initial tipoff state. */
...
#define NBA_TIPOFF_REQUIRE(label, expression) do { \
    ...
    NBA_TIPOFF_REQUIRE("free throw scene",
                       cpu_free_throw_scene_self_test(assets, session));
    ...
    NBA_TIPOFF_REQUIRE("player contact", cpu_player_contact_self_test(assets));
...
/* Host gameplay orchestration; no single direct native address. Coordinate
 * the tipoff, clock, possession, actor, ball, presentation, and audio passes. */
...
/* Host-only CPU telemetry projection; no direct native address. Export the
 * current gameplay state, including canonical native actor `+$60`. */
void nba_tipoff_capture_telemetry(const NbaTipoff *tipoff,
    ...
    for (unsigned actor = 0; actor < NBA_GAMEPLAY_ACTOR_COUNT; ++actor) {
        ...
        bool close_finish = state->control_mode == 13u ||
                            state->control_mode == 14u;
        out->mode13_timer_60_raw = close_finish ?
