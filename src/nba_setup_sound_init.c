...
static void cycle(NbaSetupSoundInit *s, NbaCodecBusKind kind, uint32_t address,
...
static void idle(NbaSetupSoundInit *s)
{
    cycle(s, NBA_CODEC_IDLE, 0, 0, -1, false);
}

static void instruction(NbaSetupSoundInit *s, uint16_t pc, uint8_t bytes)
{
    uint8_t i;
...
static void implied(NbaSetupSoundInit *s, uint16_t pc, uint8_t bytes, uint8_t idles)
{
...
static bool index_idle(const NbaSetupSoundInit *s, uint16_t base, uint16_t index)
{
    return (P & 0x10u) == 0 || (base & 255u) + index > 255u;
}

static void memory(NbaSetupSoundInit *s, uint16_t pc, uint8_t bytes,
...
static void nz(NbaSetupSoundInit *s, uint16_t value, uint8_t width)
{
    uint16_t mask = width == 1 ? 255u : 65535u;
...
static void change_flags(NbaSetupSoundInit *s, uint16_t old, uint16_t value,
...
static void rmw(NbaSetupSoundInit *s, uint16_t pc, uint8_t bytes, uint32_t address,
...
static void rmw_commit(NbaSetupSoundInit *s)
{
...
static void set_a(NbaSetupSoundInit *s, uint32_t value)
{
...
static void set_x(NbaSetupSoundInit *s, uint32_t value)
{
...
static void set_y(NbaSetupSoundInit *s, uint32_t value)
{
    uint8_t width = (P & 0x10u) != 0 ? 1u : 2u;
...
static void status_set(NbaSetupSoundInit *s, uint32_t value)
{
    P = (uint8_t)value;
    if ((P & 0x10u) != 0) { X &= 255u; Y &= 255u; }
}

static void accumulator_change(NbaSetupSoundInit *s, SoundChange kind)
{
    uint8_t width = (P & 0x20u) != 0 ? 1u : 2u;
    uint16_t old = A;
...
static void compare(NbaSetupSoundInit *s, uint16_t value, uint16_t operand, uint8_t width)
{
    uint16_t mask = width == 1 ? 255u : 65535u;
...
static void push(NbaSetupSoundInit *s, uint16_t pc, uint8_t bytes, uint16_t value, uint8_t width)
{
    uint8_t i;
    instruction(s, pc, bytes);
    idle(s);
...
static void pull(NbaSetupSoundInit *s, uint16_t pc, uint8_t width, bool rts)
{
    uint8_t i;
    instruction(s, pc, 1);
    idle(s); idle(s);
...
static void jsl_call(NbaSetupSoundInit *s, uint16_t pc)
{
    instruction(s, pc, 3);
...
static bool boundary(NbaSetupSoundInit *s, uint32_t pc, NbaSoundInitStop stop)
{
...
static bool unsupported(NbaSetupSoundInit *s)
{
    B.status = NBA_CODEC_WORK_UNSUPPORTED;
    return false;
}
...
/* $9B73 preserves the original M=0/X=0 entry PHP across its DB guard;
 * $9B90..9B97 clears bytes $062A..07ED in descending order. The nested
 * $9BD5 guard decrements $53 again: preserve byte underflow/nesting and
 * the $7E/$7F DB normalization. $9B8E copies the full X-width word from
 * $33/$34 into $5A/$5B, not just the low bank byte. The first idle check at AACD
 * remains a live unresolved read, never an assumed acknowledgement. */
#include "nba_setup_sound_init_program.inc"

static bool prepare(NbaSetupSoundInit *s)
{
...
bool nba_setup_sound_init_begin(NbaSetupSoundInit *s, const NbaCodecWorkEntry *entry,
                               uint64_t instruction_limit)
{
    if (s == NULL || entry == NULL || instruction_limit == 0 ||
        (entry->status & 0x38u) != 0 ||
        ((entry->data_bank & 0x40u) != 0 && entry->data_bank != 0x7eu && entry->data_bank != 0x7fu)) return false;
    memset(s, 0, sizeof(*s));
    B.registers = *entry;
...
bool nba_setup_sound_init_peek(const NbaSetupSoundInit *s, NbaCodecBusCycle *out)
{
...
bool nba_setup_sound_init_accept(NbaSetupSoundInit *s, uint8_t value)
{
    const NbaCodecPendingCycle *p;
    if (s == NULL || B.status != NBA_CODEC_WORK_RUNNING) return false;
    if (B.pending_index >= B.pending_count || s->stop != NBA_SOUND_INIT_NONE) return false;
    ...
    if (B.pending_index < B.pending_count && B.pending[B.pending_index].cycle.kind == NBA_CODEC_READ &&
        ...
        s->stop = NBA_SOUND_INIT_SPC_RESPONSE; s->boundary_pc = B.pending[B.pending_index].cycle.source_pc;
    }
    return true;
}
