...
static void subtract(NbaBootstrapCpu *s,uint16_t value){
 unsigned mask=(P&32)?255u:65535u,sign=(P&32)?128u:32768u;
 unsigned a=A&mask,b=value&mask,borrow=(P&1u)?0u:1u,result=a-b-borrow;
 P=(uint8_t)((P&~0x41u)|(a>=b+borrow?1u:0u)|(((a^b)&(a^result)&sign)?64u:0u));
 set_a(s,result);
}
static void pea(NbaBootstrapCpu *s,uint16_t pc,uint16_t value){
 /* PEA: three fetches and two stack writes; unlike PHA, no idle cycle. */
 instruction(s,pc,3);
 cycle(s,NBA_CODEC_WRITE,B.registers.stack_pointer,(uint8_t)(value>>8),-1,false);
 cycle(s,NBA_CODEC_WRITE,(uint16_t)(B.registers.stack_pointer-1u),(uint8_t)value,-1,false);
}
/* Only the two original byte DEC$53 instructions9B8A/9BED. Carry is kept. */
static void decrement_guard(NbaBootstrapCpu *s,uint16_t pc){
 memory(s,pc,2,0x53,1,false,false,0);idle(s);
 cycle(s,NBA_CODEC_WRITE,0x53,0,0,true);
}
...
#include "nba_bootstrap_nmi_cpu_program.inc"
...
bool nba_bootstrap_nmi_cpu_power_on(NbaBootstrapCpu *s) {
...
/* The only alternate source entry is produced by the actual native-mode NMI
 * vector cycles. Keep the same architectural registers/stack; no begin-from-
 * snapshot API or replacement CPU state is used. The pushed return PC remains
 * authoritative for the eventual RTI owner, which is not implemented here. */
bool nba_bootstrap_nmi_cpu_enter_vector(NbaBootstrapCpu *s,uint16_t vector){
 if(!s||vector!=0x8156||s->program_bank||s->emulation||(P&0x30)!=0x30)return false;
 B.resume=0xffffffffu;return prepare(s);
}
