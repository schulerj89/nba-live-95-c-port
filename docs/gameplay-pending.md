...
| Final player sprite placement | Native projection/culling and human edge indicator implemented; rare presentation branches remain | `$87:A3BB-$A43B` owns exact integer-word projection and visibility. `$87:A846-$A97D` now owns exact controller-arrow resource, attribute and clipped placement across all four controllers/eight edges. Five statically translated high-jump culling starts lack a live witness; 23 other unobserved starts belong to queue/effect/ball setup. |
