...
All 287 declared source/dependency/object/earlier-freeze/artifact/external identities
rehash. No original artifact was missing or modified. Private evidence is
auditor `build/pass-catch-audit-v1`.

The initial inventory report counted 282 entries and omitted the three external
and two private-object identities from that count. They are now explicitly
rehashed in `external-recheck.json` (ROM, Mesen, pack and two original objects).
The earlier inventory is retained; no source/verdict or evidence bytes change.
