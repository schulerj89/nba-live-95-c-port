...
def scope(pc):return 0x800d<=pc<0x814e or 0x8a57<=pc<=0x8a92 or 0xab06<=pc<=0xac1a or 0xac89<=pc<=0xacc1 or 0xda72<=pc<=0xda90 or 0x9b73<=pc<=0x9bb2 or 0x9bd5<=pc<=0x9bf1 or 0xa4fc<=pc<=0xa508 or 0xaacd<=pc<=0xaad0 or 0x8156<=pc<=0x816c or 0x8178<=pc<0x8184
def label(s):return 'source_%04x_m%dx%d'%s
def generate(rom,d):
    todo=deque([(0x800d,1,1),(0x8156,1,1)]); found={}; edges={}; returns=set()
    while todo:
        s=todo.popleft()
        ...
        if pc in (0xab17,0x8a68,0x9b84):post=(0,0)
        ...
        if i.mnem=='JSR':
            es=[(i.operand&65535,*post)];returns.add(nxt)
        if i.mnem=='JSL':
    ...
    out=['/* Generated canonical reset/upload/first-fill/reset-tables/early-sound/early-NMI source. No opcode dispatch. */',
         'static bool advance(NbaBootstrapCpu *s) { switch(B.resume) { case 0: goto source_800d_m1x1; case 0xffffffffu: goto source_8156_m1x1;']
    for state,i in sorted(found.items()):
        ...
        def addr():
            ...
            if i.mode==d.ABS_X:return f'DB(0x{o:x}u+X)',('true'if i.mnem in ('STA','STZ','STX','STY')else f'index_idle(s,0x{o:x},X)')
            if i.mode==d.LONG:return f'0x{o:x}u','false'
            if i.mode==d.LONG_X:return f'((0x{o:x}u+X)&0xffffffu)','false'
            if i.mode==d.ABS_Y:return f'DB(0x{o:x}u+Y)',('true'if i.mnem in ('STA','STZ','STX','STY')else f'index_idle(s,0x{o:x},Y)')
            raise ValueError(str(i))
        if i.mnem in ('LDA','LDX','LDY','CMP','CPX','ADC','SBC','AND'):
            ...
            elif i.mnem=='ADC':b+=[f'add(s,{v});']
            elif i.mnem=='SBC':b+=[f'subtract(s,{v});']
            else:b+=[f'set_a(s,A&{v});']
            ...
            assert i.mode==d.DP and w==1 and o==0x53
            b+=[f'SUSPEND(decrement_guard(s,0x{pc:x}));','nz(s,(uint8_t)(B.read_value-1u),1);']
        elif i.mnem in ('INX','INY','DEX'):
            r=i.mnem[-1];b+=[f'IMP(0x{pc:x});',f'set_{r.lower()}(s,{r}{"-" if i.mnem=="DEX" else "+"}1u);']
        elif i.mnem in ('INC','ROL','LSR'):
            b+=[f'IMP(0x{pc:x});',f'accumulator_change(s,SOUND_{i.mnem});']
        elif i.mnem in ('TAX','TAY'):b+=[f'IMP(0x{pc:x});',f'set_{i.mnem[-1].lower()}(s,A);']
        elif i.mnem in ('TXA','TYA'):b+=[f'IMP(0x{pc:x});',f'set_a(s,{i.mnem[1]});']
        elif i.mnem=='CLI':b+=[f'IMP(0x{pc:x});','P&=0xfbu;']
        elif i.mnem=='SEC':b+=[f'IMP(0x{pc:x});','P|=1;']
        elif i.mnem=='PEA':b+=[f'SUSPEND(pea(s,0x{pc:x},0x{o:x}));','B.registers.stack_pointer-=2u;']
        ...
        elif i.mnem in ('PHP','PHA','PHB','PHK','PHD','PHX','PHY'):
            v,width={'PHP':('P',1),'PHA':('A',w),'PHB':('B.registers.data_bank',1),'PHK':('s->program_bank',1),'PHD':('0',2),'PHX':('X',xw),'PHY':('Y',xw)}[i.mnem]
            ...
            if pc in (0xab17,0x8a68,0x9b84):nxt=(pc+1,0,0)
        elif i.mnem in BR:
            b+=[f'BRANCH(0x{pc:x},({BR[i.mnem]})!=0);','if(B.branch) { '+go((o,m,x))+' }']
        elif i.mnem in ('JML','JMP'):
            b+=[f'IMM(0x{pc:x},{i.length});','s->program_bank=0x80;',go((o&65535,m,x))];nxt=None
        elif i.mnem=='JSR':
            b+=[f'PUSH(0x{pc:x},3,0x{pc+2:x},2);',go((o&65535,m,x))];nxt=None
        elif i.mnem=='JSL':
