...
def main():
 ...
 entries={0x84:[('HumanBPrefix',0xe2ac,0xe2f2)],0x85:[('HumanAcceleration',0xa82c,0xab17)],
          0x87:[('HumanInputGate',0x9138,0x9165),('HumanMovementCaller',0x91c3,0x922e)]}
 script=Path(__file__).parent/'ghidra/DumpHumanDispatch.java'
 ...
 for bank,items in entries.items():
  ...
  (out/f'human_bank{bank:02x}.c').write_text(emit_bank(rom,bank,[BankEntry(n,s,e,0,0)for n,s,e in items]))
  ...
  if run.returncode or not(out/f'human_bank{bank:02x}.txt').is_file():raise RuntimeError('Ghidra failed; retain output')
