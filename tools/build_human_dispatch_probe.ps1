...
$privateBuild=Join-Path $root 'build\human-dispatch'
...
cl /nologo /W4 /O2 /MD /utf-8 /I "$root\include" /Fe"$privateBuild\human_dispatch_probe.exe" /Fo"$privateBuild\human_dispatch_probe.obj" "$root\tools\human_dispatch_probe.c" "$privateBuild\nba_human_dispatch.obj" $(($objects|ForEach-Object{'"'+$_+'"'})-join ' ') user32.lib gdi32.lib winmm.lib
exit /b %ERRORLEVEL%
"@
