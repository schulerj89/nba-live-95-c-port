/* Original OOB entry-state adapter; never reads native expected output. */
...
int main(int argc,char **argv) {
    ...
    for(uint32_t index=0;index<count;++index) {
        ...
        in.latched_event_raw_08f0=w(raw,0x8F0);in.event_actor_raw_492d=w(raw,0x492D);
        in.contact_context_raw_497f=w(raw,0x497F);in.foul_out_state_raw_09ca=w(raw,0x9CA);
        in.injury_state_raw_09cc=w(raw,0x9CC);
        h.violation_characters_published=pc==0x83EBDBu;
        bool ok;
