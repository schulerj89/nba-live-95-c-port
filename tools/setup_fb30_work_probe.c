...
{
    /* Four canonical symbols with equal two-bit codes: output ABC, then termination.
     * These input bytes are synthetic tests, never native evidence. */
    static const uint8_t stream[] = {0x30,0xfb,0,0,3,0xfe,0x88,0x08,0xb2,0x02,0xf8,0x6e,0x40,0,0};
    /* Keep the actual source jump/number tables in ROM. Only this synthetic
     * resource window and fresh scratch memory differ from ordinary input. */
    memset(bus->wram, 0, sizeof(bus->wram));
    bus->wmadd = bus->written = bus->payload_written = 0;
...
static int self_test(ProbeBus *bus)
{
    ...
    for (variant = 0; variant < 3; ++variant) {
        ...
        if (nba_setup_fb30_work_begin(&work, &invalid, 1000)) return 3;
        ++cases;
    }
    if (nba_setup_fb30_work_begin(&work, &entry, 0) ||
        nba_setup_fb30_work_begin(NULL, &entry, 1) ||
        nba_setup_fb30_work_begin(&work, NULL, 1)) return 3;
    ++cases;
    for (variant = 0; variant < 4; ++variant) {
        uint64_t cycles = 0;
        synthetic_bus(bus);
        if (variant == 3) bus->wram[0x562] = 0x0f; /* other empty-queue branch */
        if (!nba_setup_fb30_work_begin(&work, &entry, 10000)) return 3;
        while (nba_setup_fb30_work_peek(&work, &event)) {
            uint8_t value = 0;
            if (variant == 1) {
                ...
                if (!nba_setup_fb30_work_peek(&work, &again) ||
            ...
            if (variant == 2 && event.kind == NBA_CODEC_READ && event.address == 0xae800a)
                bus->rom[0x17000a] ^= 2u; /* resolve data at the pending read */
            ...
            ++cycles;
            if (!bus->valid || !nba_setup_fb30_work_accept(&work, value)) return 3;
        }
        cost[variant] = cycles;
        if (work.bus.status != NBA_CODEC_WORK_DONE || bus->payload_written != 3 || work.bus.output_size != 3 ||
            work.bus.registers.stack_pointer != entry.stack_pointer || work.bus.registers.status != entry.status ||
            memcmp(bus->wram + 0x12000, variant == 2 ? "CBC" : "ABC", 3) != 0) return 3;
        ++cases;
    }
    if (cost[0] != cost[1] || cost[0] != cost[2] || cost[0] != cost[3] + 2u) return 3;
    ++cases;
    synthetic_bus(bus);
    if (!nba_setup_fb30_work_begin(&work, &entry, 1)) return 3;
    while (nba_setup_fb30_work_peek(&work, &event)) {
        if (event.kind == NBA_CODEC_WRITE) bus_write(bus, event.address, event.value);
        if (!nba_setup_fb30_work_accept(&work, event.kind == NBA_CODEC_READ ? bus_read(bus, event.address) : 0)) return 3;
    }
    if (work.bus.status != NBA_CODEC_WORK_LIMIT || work.bus.instructions != 1 || bus->payload_written != 0) return 3;
    ++cases;
    for (variant = 0; variant < 2; ++variant) {
        synthetic_bus(bus);
        if (variant == 0) bus->wram[0x37] = 8; /* unsupported nonempty queue */
        else bus->rom[0x170000] = 0x46; /* FB46 is handled by the separate frozen component */
        if (!nba_setup_fb30_work_begin(&work, &entry, 10000)) return 3;
        while (nba_setup_fb30_work_peek(&work, &event)) {
            if (event.kind == NBA_CODEC_WRITE) bus_write(bus, event.address, event.value);
            if (!nba_setup_fb30_work_accept(&work, event.kind == NBA_CODEC_READ ? bus_read(bus, event.address) : 0)) return 3;
        }
        if (work.bus.status != NBA_CODEC_WORK_UNSUPPORTED || bus->payload_written != 0) return 3;
        ++cases;
    }
    synthetic_bus(bus);
    if (!nba_setup_fb30_work_begin(&work, &entry, 10000)) return 3;
    while (nba_setup_fb30_work_peek(&work, &event)) {
        uint8_t value = event.kind == NBA_CODEC_READ ? bus_read(bus, event.address) : 0;
        /* The effective address must use the pointer bytes accepted at their
         * actual bus reads, including a changed byte between peek and accept. */
        if (event.source_pc == 0x80be75 && event.address == 0x0c) value = 4;
        if (event.source_pc == 0x80be75 && event.address > 0xae0000) {
            if (event.address != 0xae8004) return 3;
            break;
        }
        if (event.kind == NBA_CODEC_WRITE) bus_write(bus, event.address, event.value);
        if (!nba_setup_fb30_work_accept(&work, value)) return 3;
    }
    if (work.bus.status != NBA_CODEC_WORK_RUNNING) return 3;
    ++cases;
    synthetic_bus(bus);
    if (!nba_setup_fb30_work_begin(&work, &entry, 10000)) return 3;
    variant = 0;
    while (nba_setup_fb30_work_peek(&work, &event)) {
        if (event.source_pc == 0x80be79 && event.kind == NBA_CODEC_WRITE) {
            if (event.address != (variant == 0 ? 0x0du : 0x0cu) ||
                event.value != (variant == 0 ? 0x80u : 6u)) return 3;
            ++variant;
        }
        if (event.kind == NBA_CODEC_WRITE) bus_write(bus, event.address, event.value);
        if (!nba_setup_fb30_work_accept(&work, event.kind == NBA_CODEC_READ ? bus_read(bus, event.address) : 0)) return 3;
    ...
    if (variant != 2) return 3;
    ++cases;
    printf("PASS: %u FB30 continuation contract cases\n", cases);
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    if ((argc != 4 && argc != 5 && argc != 6) || !hex_address(argv[2], &source)) {
        fprintf(stderr, "usage: setup_fb30_work_probe ROM SIX_HEX_SOURCE OUTPUT_WRAM [TRACE_JSONL [A,X,Y,SP,DB,PS,QUEUE_CURSOR]]\n");
        return 2;
    }
    f = fopen(argv[1], "rb");
    if (f == NULL) return 2;
    ...
    bus.valid = true;
    if (!nba_setup_fb30_work_begin(&work, &entry, 1000000u)) return 2;
    while (nba_setup_fb30_work_peek(&work, &c)) {
        uint8_t value = 0;
        ...
        if (trace != NULL && work.bus.pending_index == 0) fprintf(trace,
            "{\"kind\":\"instruction\",\"cycle\":%llu,\"master\":%llu,\"pc\":%u,\"a\":%u,\"x\":%u,\"y\":%u,\"ps\":%u,\"db\":%u,\"sp\":%u}\n",
            (unsigned long long)cycles, (unsigned long long)(master - c.master_clocks), c.source_pc, work.bus.registers.value,
            work.bus.registers.symbol, work.bus.registers.stream_cursor, work.bus.registers.status,
            work.bus.registers.data_bank, work.bus.registers.stack_pointer);
        ...
        if (!bus.valid || !nba_setup_fb30_work_accept(&work, value)) break;
    }
    ...
    printf("{\"schema\":1,\"status\":%u,\"bus_valid\":%s,\"source\":%u,\"cycles\":%llu,\"master\":%llu,\"slow\":%llu,\"instructions\":%llu,\"output_bytes\":%u,\"declared_bytes\":%u,\"cursor\":%u,\"sp\":%u,\"wmadd\":%u,\"counts\":{",
           (unsigned)work.bus.status, bus.valid ? "true" : "false", source,
           (unsigned long long)cycles, (unsigned long long)master,
           (unsigned long long)slow, (unsigned long long)instructions,
           bus.payload_written, work.bus.output_size, work.bus.registers.stream_cursor,
