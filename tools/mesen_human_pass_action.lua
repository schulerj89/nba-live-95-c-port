...
local selection=assert(tonumber(os.getenv('NBA95_PASS_ACTION_SELECTION')))
local stop_at=assert(tonumber(os.getenv('NBA95_PASS_ACTION_FRAMES')))
...
hook(0x86ad0e,function()
 if action=='gate'then snap('action.offaxis',0x86ad0e,true);action=false end
end)
...
hook(0x84e09c,function()
 if active then
  check(not action and not upper and not metric and not cancel and(active=='selection'or(active=='child'and prefix and geometry)),'bad pass resume')
