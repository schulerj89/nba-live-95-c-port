-- Controlled `$09D8` witnesses for the normal mode-one parent at $86:F1B0.
...
local labels = assert(io.open(out .. "/cpu-mode-one-role-cases.jsonl", "wb"))
local traces = assert(io.open(out .. "/cpu-mode-one-role-pcs.jsonl", "wb"))
local cases = {
    {name="role_clear_ownerless_record", role_ownerless=0,
     named_owner=false, slot=2},
    {name="role_set_named_owner", role_ownerless=1,
     named_owner=true, slot=7},
}
...
hook(0x879244, function()
    ...
    if slot ~= case.slot then return end
    index = index + 1
...
emu.addMemoryCallback(function(address)
    ...
    if address == 0x86f235 or address == 0x86f23e then
        local parts = {}
...
end, emu.callbackType.exec, 0x86f1b0, 0x86f23e,
    emu.cpuType.snes, emu.memType.snesMemory)
...
local function restore()
    if not pending_case then return end
    assert(word(pending_case.actor + 0x5e) == 1,
        "native mode-one parent changed control mode")
    for address, value in pairs(pending_case.saved) do put_byte(address, value) end
    pending_case = nil
end
hook(0x86f235, restore)
hook(0x86f23e, restore)
