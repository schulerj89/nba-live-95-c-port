...
cl /nologo /W4 /WX /O2 /MD /utf-8 /I "$root\include" /Fe"$output\setup_producer_work_probe.exe" /Fo"$output\\" "$root\tools\setup_producer_work_probe.c" "$root\src\nba_setup_producer_work.c" "$root\src\nba_setup_fb30_work.c" "$root\src\nba_setup_codec_work.c"
exit /b %ERRORLEVEL%
"@
...
$files = @('include/nba_setup_codec_work.h', 'src/nba_setup_codec_work.c', 'include/nba_setup_fb30_work.h', 'src/nba_setup_fb30_work.c', 'src/nba_setup_fb30_program.inc', 'src/nba_setup_producer_program.inc', 'tools/generate_setup_producer_work.py', 'include/nba_setup_producer_work.h', 'src/nba_setup_producer_work.c', 'tools/setup_producer_work_probe.c', 'tools/build_setup_producer_work_probe.ps1')
...
$exe = Join-Path $output 'setup_producer_work_probe.exe'
