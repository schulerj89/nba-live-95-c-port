...
#include <stdlib.h>
#include <string.h>
typedef struct {FILE *trace;NbaBootstrap *state;NbaBootstrapOamPrefix *owner;const char *directory;bool failed,poll_saved;} Output;
...
static bool final_state(const char *directory,const NbaBootstrapOamPrefix *owner){
 char path[2048];FILE *f;const NbaBootstrap *s=&owner->prefix.machine.core;const NbaCodecWorkEntry *r=&s->cpu.work.registers;
 ...
 fprintf(f,"ppu.vramAddress=%u\nppu.vramReadBuffer=%u\ndmaController.channel[1].srcAddress=%u\ndmaController.channel[1].srcBank=%u\ndmaController.channel[1].transferSize=%u\ndmaController.channel[1].transferMode=%u\ndmaController.channel[1].fixedTransfer=%s\ndmaController.channel[1].invertDirection=%s\ndmaController.channel[1].decrement=%s\ndmaController.channel[1].dmaActive=%s\n",
  owner->prefix.machine.vram_address,owner->prefix.machine.vram_read_buffer,s->io[0x2312]|((unsigned)s->io[0x2313]<<8),s->io[0x2314],s->io[0x2315]|((unsigned)s->io[0x2316]<<8),s->io[0x2310]&7,
  s->io[0x2310]&8?"true":"false",s->io[0x2310]&128?"true":"false",s->io[0x2310]&16?"true":"false",owner->prefix.machine.dma.pending?"true":"false");
 fprintf(f,"ppu.enableOamPriority=%s\nppu.oamRamAddress=%u\nppu.internalOamAddress=%u\n",
  owner->oam_priority_rotation?"true":"false",owner->oam_ram_address,owner->oam_internal_address);
 return fclose(f)==0;
}
static bool nmi_state(const char *directory,const char *name,const NbaBootstrapNmi *owner){
 char path[2048];FILE *f;const NbaBootstrapNmiState *n=&owner->nmi;
 if(snprintf(path,sizeof(path),"%s/%s.nmi",directory,name)<0)return false;
 f=fopen(path,"wb");if(!f)return false;
 fprintf(f,"cpu.needNmi=%s\ncpu.nmiFlagCounter=%u\ninternalRegisters.enableNmi=%s\ninternalRegisters.nmiFlag=%s\ninternalRegisters.irqFlag=%s\n",
  n->need?"true":"false",n->delay,n->enabled?"true":"false",n->flag?"true":"false",n->irq_flag?"true":"false");
 fprintf(f,"internalRegisters.enableAutoJoypadRead=%s\ninternalRegisters.autoReadActive=%s\ninternalRegisters.autoReadDisabled=%s\ninternalRegisters.autoReadClockStart=%llu\ninternalRegisters.autoReadNextClock=%llu\ncontrolManager.autoReadStrobe=%s\nmemoryManager.openBus=%u\n",
  n->auto_enabled?"true":"false",n->auto_active?"true":"false",n->auto_disabled?"true":"false",(unsigned long long)n->auto_start,(unsigned long long)n->auto_next,n->auto_strobe?"true":"false",n->open_bus);
 return fclose(f)==0;
}
static void observe(void *context,const NbaBootstrapEvent *e){
 ...
 if(e->kind==NBA_BOOT_EVENT_CPU_ENTRY){
  ...
  switch(e->pc){case 0x808184:name="cpu_oam_request.wram";break;case 0x808188:name="cpu_oam_written.wram";break;case 0x8080c0:name="cpu_fill_return.wram";break;case 0x8080ca:name="cpu_clear_after.wram";break;
   case 0x8080fd:name="cpu_table_after.wram";break;case 0x808101:name="cpu_da72_return.wram";break;
   case 0x808141:name="cpu_ab7e_return.wram";break;case 0x808145:name="cpu_nmi_enable.wram";break;case 0x809b73:name="cpu_sound_entry.wram";break;case 0x809bed:name="cpu_sound_guard_before.wram";break;case 0x809bef:name="cpu_sound_guard_after.wram";break;case 0x8156:name="nmi_vector_entry.wram";break;case 0x80aacd:if(!o->poll_saved){name="cpu_sound_poll.wram";o->poll_saved=true;}break;default:break;}
  if(name){char prefix[128];size_t len=strlen(name)-5;
   if(!save(o->directory,name,s->wram,sizeof(s->wram)))o->failed=true;
   memcpy(prefix,name,len);prefix[len]=0;if(!nmi_state(o->directory,prefix,&o->owner->prefix))o->failed=true;
  }
 }
 if(e->kind==NBA_BOOT_EVENT_CPU_ENTRY&&e->pc==0x8080bc){if(!save(o->directory,"cpu_80bc.wram",s->wram,sizeof(s->wram))||!nmi_state(o->directory,"cpu_80bc",&o->owner->prefix))o->failed=true;}
...
int main(int argc,char **argv){
 NbaRom rom={0};NbaBootstrap *s;NbaBootstrapOamPrefix *owner,*prior;Output out;char path[2048];unsigned long steps;
 if(argc!=3)return 2;if(!nba_rom_load_file(&rom,argv[1]))return 3;
 owner=calloc(1,sizeof(*owner));prior=malloc(sizeof(*owner));if(!owner||!prior)return 4;s=&owner->prefix.machine.core;
 if(!nba_bootstrap_oam_prefix_power_on(owner,rom.data,rom.size,NBA_BOOT_PROFILE_NTSC_ZERO_32040))return 5;
 if(snprintf(path,sizeof(path),"%s/events.jsonl",argv[2])<0)return 6;
 out.trace=fopen(path,"wx");out.state=s;out.owner=owner;out.directory=argv[2];out.failed=false;out.poll_saved=false;if(!out.trace)return 7;
 for(steps=0;steps<2000000&&s->status==NBA_BOOT_RUNNING;steps++)(void)nba_bootstrap_oam_prefix_step(owner,observe,&out);
 if(out.failed||fclose(out.trace))return 8;
 *prior=*owner;if(nba_bootstrap_oam_prefix_step(owner,observe,&out)||memcmp(prior,owner,sizeof(*owner)))return 9;
 if(!save(argv[2],"final.aram",s->spc_bus.aram,65536)||!save(argv[2],"final.wram",s->wram,sizeof(s->wram)))return 10;
 if(!save(argv[2],"final.vram",owner->prefix.machine.vram,sizeof(owner->prefix.machine.vram))||!final_state(argv[2],owner)||!nmi_state(argv[2],"final",&owner->prefix))return 12;
 ...
 printf("\"dma_bytes\":%u,\"vram_address\":%u,\"sync_counter\":%u,\"source_index\":%u}\n",owner->prefix.machine.dma.transferred,owner->prefix.machine.vram_address,owner->prefix.machine.dma.clock_counter,owner->prefix.machine.dma.source_index);
