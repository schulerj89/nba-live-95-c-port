...
def main():
 ...
 entries={0x86:[('SideBackGate',0xac50,0xaca9),('GroundedPass',0xb00b,0xb04c),('BoostedPassReference',0xafc4,0xb00b)],0x87:[('InstallUpper',0xb47a,0xb4db)]}
 script=Path(__file__).parent/'ghidra/DumpHumanPassAction.java'
 ...
 for bank,items in entries.items():
  ...
  (out/f'human_pass_action_bank{bank:02x}.c').write_text(emit_bank(rom,bank,[BankEntry(n,s,e,0,0)for n,s,e in items]))
  ...
  if run.returncode or not(out/f'human_pass_action_bank{bank:02x}.txt').is_file():raise RuntimeError('Ghidra failed; retain output')
