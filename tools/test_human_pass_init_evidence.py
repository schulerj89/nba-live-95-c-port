...
import verify_human_pass_init as verifier
...
def main():
 ...
 for artifact in('boundaries.jsonl','capture.lua','capture_complete.txt','capture_human_pass_init.py','mesen_portable.py',
 ...
 manifest_check('wrong selection environment',lambda m:m['environment'].update(NBA95_PASS_INIT_SELECTION='1'))
 ...
 event_index=next(i for i,row in enumerate(original_rows)if row['tag']=='pass.initialize')
 ...
 event_check('missing ready stage',lambda rows:rows.pop(next(i for i,row in enumerate(rows)if row['tag']=='init.ready')))
 ...
 # Reject partial new-stage results and numeric JSON type substitutions.
 for field in('prefix_words','geometry_words'):
  index=next(i for i,line in enumerate(actual_lines)if json.loads(line).get(field))
  altered=list(actual_lines);item=json.loads(altered[index]);item[field][0]=True;altered[index]=json.dumps(item)
  changes.append(('boolean '+field,'\n'.join(altered)+'\n'))
  altered=list(actual_lines);item=json.loads(altered[index]);item.pop(field);altered[index]=json.dumps(item)
  changes.append(('missing '+field,'\n'.join(altered)+'\n'))
 fine_index=next(i for i,line in enumerate(actual_lines)if 'fine'in json.loads(line))
 altered=list(actual_lines);item=json.loads(altered[fine_index]);item['fine']=float(item['fine']);altered[fine_index]=json.dumps(item)
 changes.append(('floating fine direction','\n'.join(altered)+'\n'))
 changes.append(('missing fine result','\n'.join(x for i,x in enumerate(actual_lines)if i!=fine_index)+'\n'))
 changes.append(('missing all initializer results','\n'.join(x for x in actual_lines if 'prefix_words'not in json.loads(x))+'\n'))
