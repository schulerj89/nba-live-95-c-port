...
local selection=assert(tonumber(os.getenv('NBA95_PASS_ALIGNED_SELECTION')))
local stop_at=assert(tonumber(os.getenv('NBA95_PASS_ALIGNED_FRAMES')))
...
hook(0x84e09c,function()
 if active then
  check(not aligned and not lane and not aligned_upper and not action and not upper and not metric and not cancel and(active=='selection'or(active=='child'and prefix and geometry)),'bad pass resume')
