...
def main():
 ...
 entries={0x84:[('HumanPass',0xdf7a,0xe141)],0x85:[('WeightedDistance',0xf1c1,0xf229),('DirectionZero',0xf347,0xf34f),('DirectionDistance',0xf34f,0xf3bb)]}
 script=Path(__file__).parent/'ghidra/DumpHumanPass.java'
 ...
 for bank,items in entries.items():
  ...
  (out/f'human_pass_bank{bank:02x}.c').write_text(emit_bank(rom,bank,[BankEntry(n,s,e,0,0)for n,s,e in items]))
  ...
  if run.returncode or not(out/f'human_pass_bank{bank:02x}.txt').is_file():raise RuntimeError('Ghidra failed; retain output')
