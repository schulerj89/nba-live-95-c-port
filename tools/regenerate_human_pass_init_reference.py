...
def main():
 ...
 entries={0x85:[('FineZero',0xf3bb,0xf3c3),('FineDirection',0xf3c3,0xf473),('AlignedSelector',0xf473,0xf5e4)],0x86:[('PassInitialize',0xab2d,0xaf66)],0x87:[('CancelUpper',0xb538,0xb555)]}
 script=Path(__file__).parent/'ghidra/DumpHumanPassInit.java'
 ...
 for bank,items in entries.items():
  ...
  (out/f'human_pass_init_bank{bank:02x}.c').write_text(emit_bank(rom,bank,[BankEntry(n,s,e,0,0)for n,s,e in items]))
  ...
  if bank==0x85:
   table=raw[0xf16f-0x8000:0xf18f-0x8000]
   m['ranges']['FineDirectionMap']=dict(start='85:F16F',end_exclusive='85:F18F',bytes=table.hex(),sha256=sha(table))
  ...
  if run.returncode or not(out/f'human_pass_init_bank{bank:02x}.txt').is_file():raise RuntimeError('Ghidra failed; retain output')
