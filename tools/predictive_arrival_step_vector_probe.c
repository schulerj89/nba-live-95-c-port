/* Replays `$85:B402-$B4B8` through the corresponding C-port helpers. */
#include <stdio.h>
...
int main(void) {
    ...
    while (scanf("%x %x %x %x %x %x %x %x %x %x %x %x",
        ...
        uint8_t steering = 8u;
        bool arrived = nba_gameplay_predictive_arrival(
            (int16_t)(uint16_t)x, (int16_t)(uint16_t)y, out_x, out_y,
