...
static void load_actor(NbaTipoffActor *a,const uint8_t *r,unsigned b,uint8_t roster){
    ...
    for(unsigned i=0;i<3;i++){a->animation_upper_queue_raw_1c[i]=word(r,b+0x1c+i*2);a->animation_lower_queue_raw_22[i]=word(r,b+0x22+i*2);}
    a->rom_upper_animation_phase_raw_3a=word(r,b+0x3a);a->rom_lower_animation_phase_raw_3c=word(r,b+0x3c);
    a->upper_animation_accumulator_raw_42=word(r,b+0x42);a->lower_animation_accumulator_raw_44=word(r,b+0x44);
    a->upper_animation_lock_raw_46=word(r,b+0x46);a->lower_animation_lock_raw_48=word(r,b+0x48);
