...
cl /nologo /W4 /WX /O2 /MD /utf-8 /I "$root\include" /Fe"$output\setup_spc_init_probe.exe" /Fo"$output\\" "$root\tools\setup_spc_init_probe.c" "$root\src\nba_setup_spc_init.c"
exit /b %ERRORLEVEL%
"@
...
$files = @('include/nba_setup_spc_resident.h', 'include/nba_setup_spc_init.h', 'src/nba_setup_spc_init.c', 'tools/setup_spc_init_probe.c', 'tools/build_setup_spc_init_probe.ps1')
...
$exe = Join-Path $output 'setup_spc_init_probe.exe'
