...
def run_probe(exe,out,name,data):
    ...
    for row in rows:
        ...
        check(type(row['words'])is list and len(row['words'])==len(mapping())and all(integer(x,0,255 if width==1 else 65535)for x,(_,_,width)in zip(row['words'],mapping())),'role C typed word schema')
    return rows
