"""Observe unpatched NBA95 normal cold power-on through post-OAM-write pre-PPU-status818E."""
...
def main():
    ...
    script=out/'capture.lua';shutil.copyfile(Path(__file__).with_name('mesen_bootstrap_oam_prefix.lua'),script);runner=out/'runner.py';shutil.copyfile(__file__,runner)
    args=[str(exe),'--testrunner','--timeout=180',str(a.rom.resolve()),str(script)]
    m={'schema':1,'kind':'normal reset first NMI post-OAM-write pre-PPU-status observation','state_injection':False,'rom_patch':False,'inputs':False,'accepted':False,'sources':{},'settings':settings,'initial_saves':[],'arguments':args}
    ...
    env={k:v for k,v in os.environ.items()if not k.startswith('NBA95_')};env['NBA95_BOOTSTRAP_OAM_DIR']=str(out)
    ...
    try:
        ...
        if (out/'complete.txt').read_text()!='ok; normal power-on through post-OAM-write pre-PPU-status818E\n':raise ValueError('incomplete capture')
