...
static void bus_write(ProbeBus *b, uint32_t address, uint8_t value)
{
    ...
    else if ((bank & 0x40u) == 0 && low >= 0x2115 && low <= 0x2122) ppu_write(b, low, value);
    else b->valid = false;
}
...
static void initial_bus(ProbeBus *bus, const NbaCodecWorkEntry *entry, uint16_t cursor)
{
    ...
    bus->wram[0x561] = 0xbf; bus->wram[0x562] = 0x8f;
...
static int self_test(ProbeBus *bus)
{
    ...
    NbaSetupProducerWork work,copy;
    NbaCodecBusCycle event,again;
    uint64_t hashes[2]={0},costs[4]={0};
    uint8_t selector_zero[0x18c0];
    unsigned variant,cases=0;
    for (variant=0;variant<3;++variant) {
        NbaCodecWorkEntry bad=entry;bad.status|=(uint8_t)(8u<<variant);
        if (nba_setup_producer_work_begin(&work,&bad,10000)) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
        ++cases;
    }
    if (nba_setup_producer_work_begin(NULL,&entry,1) || nba_setup_producer_work_begin(&work,NULL,1) ||
        nba_setup_producer_work_begin(&work,&entry,0)) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
    ++cases;
    for (variant=0;variant<4;++variant) {
        ...
        if (!nba_setup_producer_work_begin(&work,&entry,1000000)) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
        while (nba_setup_producer_work_peek(&work,&event)) {
            uint8_t value=0;
            if (variant==1) {
                memcpy(&copy,&work,sizeof(work));memset(&work,0xa5,sizeof(work));memcpy(&work,&copy,sizeof(work));
                if (!nba_setup_producer_work_peek(&work,&again) || again.kind!=event.kind ||
                    again.address!=event.address || again.value!=event.value || again.source_pc!=event.source_pc) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
            }
            bus->cpu_cycle=++cpu;
            ...
            if (!bus->valid || !nba_setup_producer_work_accept(&work,value)) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
        }
        if (work.local.bus.status!=NBA_CODEC_WORK_DONE || work.return_pc!=0x81d018 ||
            work.local.bus.registers.stack_pointer!=entry.stack_pointer+3u) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
        if (variant<2) hashes[variant]=hash;
        if (variant==2) memcpy(selector_zero,bus->wram+0x12000,sizeof(selector_zero));
        if (variant==3 && memcmp(selector_zero,bus->wram+0x12000,sizeof(selector_zero))) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
        costs[variant]=cpu;++cases;
    }
    if (hashes[0]!=hashes[1] || costs[0]!=costs[1] || costs[3]!=costs[2]+8u) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
    ++cases;
    for (variant=0;variant<2;++variant) {
        initial_bus(bus,&entry,0);
        if (variant==1) bus->wram[0x562]=15; /* queued fill is explicitly outside this component */
        if (!nba_setup_producer_work_begin(&work,&entry,variant?1000000:1000)) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
        while (nba_setup_producer_work_peek(&work,&event)) {
            if (event.kind==NBA_CODEC_WRITE) bus_write(bus,event.address,event.value);
            if (!nba_setup_producer_work_accept(&work,event.kind==NBA_CODEC_READ?bus_read(bus,event.address):0)) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
        }
        if (work.local.bus.status!=(variant?NBA_CODEC_WORK_UNSUPPORTED:NBA_CODEC_WORK_LIMIT)) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
        if (!variant && work.local.bus.instructions!=1000) { fprintf(stderr,"producer self-test failed at source line %u\n",(unsigned)__LINE__); return 3; }
        ++cases;
    }
    printf("PASS: %u producer continuation contract cases\n",cases);
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    if (argc != 3 && argc != 4 && argc != 5) {
        fprintf(stderr, "usage: setup_producer_work_probe ROM OUTPUT_WRAM [TRACE_JSONL [A,X,Y,SP,DB,PS,QUEUE_CURSOR]]\n");
        return 2;
    }
    f = fopen(argv[1], "rb");
    if (f == NULL) return 2;
    ...
    bus.valid = true;
    if (!nba_setup_producer_work_begin(&work, &entry, 1000000u)) return 2;
    while (nba_setup_producer_work_peek(&work, &c)) {
        const NbaSetupCodecWork *current = work.child_active ?
            (work.codec_kind == 30 ? &work.child.fb30.bus : &work.child.fb46) : &work.local.bus;
        bus.cpu_cycle = cycles + 1;
        uint8_t value = 0;
        /* Peek must be side-effect free, including between a RMW read/write. */
        if (!nba_setup_producer_work_peek(&work, &again) ||
        ...
        if (!bus.valid || !nba_setup_producer_work_accept(&work, value)) break;
    }
    ...
    printf("{\"schema\":1,\"status\":%u,\"bus_valid\":%s,\"return_pc\":%u,\"cycles\":%llu,\"master\":%llu,\"slow\":%llu,\"instructions\":%llu,\"dma_bytes\":%u,\"dma_jobs\":%u,\"cursor\":%u,\"sp\":%u,\"wmadd\":%u,\"counts\":{",
           (unsigned)work.local.bus.status, bus.valid ? "true" : "false", work.return_pc,
           (unsigned long long)cycles, (unsigned long long)master,
           (unsigned long long)slow, (unsigned long long)instructions,
           bus.dma_bytes, bus.dma_jobs, work.local.bus.registers.stream_cursor,
           work.local.bus.registers.stack_pointer, bus.wmadd);
    for (pc = 0; pc < 0x10000u; ++pc) if (per_pc[pc] != 0) {
        printf("%s\"%06X\":%u", first ? "" : ",", pc | 0x800000u, per_pc[pc]);
        first = false;
    }
    printf("}}\n");
    return work.local.bus.status == NBA_CODEC_WORK_DONE && bus.valid ? 0 : 1;
}
