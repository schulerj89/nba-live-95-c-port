...
$script=(Resolve-Path -LiteralPath (Join-Path $PSScriptRoot 'mesen_cpu_mode_two_role.lua')).Path
$variables=@{
    ...
    NBA95_VEC_LABEL='cpu_mode_two_role'; NBA95_VEC_MAX='2';
...
if($process.ExitCode-ne 0-or!(Test-Path -LiteralPath $sentinel)){
    throw "Native CPU mode-two role-flag capture failed: exit=$($process.ExitCode)"
}
$complete=(Get-Content -Raw -LiteralPath $sentinel).Trim()
if($complete-ne 'label=cpu_mode_two_role vectors=2 orphan_exits=0 shared_exit_callbacks=0'){
    throw "Incomplete CPU mode-two role-flag capture: $complete"
}
$complete
