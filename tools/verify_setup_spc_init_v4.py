...
from setup_spc_state_contract_v4 import validate as validate_state, pending_dsp
...
def native(p,rom):
 ...
 for tag in ('entry','post_control','dsp_entry','pending_dsp'):
  ...
  s=state(p/f'spc_init_{tag}.state');validate_state(s);normal_spc_state(s)
 ...
 pending=state(p/'spc_init_pending_dsp.state');pending_dsp(pending,ni[-1],nr[2]);check(pending['spc.dspReg']==state(p/'spc_init_dsp_entry.state')['spc.dspReg'],'pending DSP address latch relation')
 return m,ni,nw,nr
def build(exe):
...
def seed(p,tag):
 s=state(p/f'spc_init_{tag}.state');validate_state(s);regs=[int(s[f'spc.{n}'])for n in ('pc','a','x','y','sp','ps')]
