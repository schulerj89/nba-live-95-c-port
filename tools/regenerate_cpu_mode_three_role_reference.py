"""Generate fresh Ghidra and snesrecomp references for CPU mode three."""
...
FIRST = 0xF23F
END = 0xF2CA
REQUIRED = (0xF23F, 0xF258, 0xF270, 0xF28D, 0xF29A, 0xF29F,
            0xF2AA, 0xF2B0, 0xF2B8, 0xF2C0, 0xF2C1, 0xF2C9)
...
def main():
    ...
    script = Path(__file__).with_name("ghidra") / "DumpCpuModeThreeRole.java"
    ...
    generated = emit_bank(rom, bank, [
        BankEntry("CpuModeThreeRole", FIRST, END, 0, 0)
    ])
    (out / "cpu_mode_three_role_bank86.c").write_text(
    ...
    project = out / "project"
    project.mkdir()
    command = [str(args.ghidra), str(project), "CpuModeThreeRole", "-import",
    ...
    listing = out / "cpu_mode_three_role_bank86.txt"
    ...
    print("bank 86: fresh Ghidra and recomp CPU mode-three role references")
