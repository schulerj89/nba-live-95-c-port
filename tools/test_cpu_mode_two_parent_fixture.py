"""Prove the mode-two parent fixture validator rejects representative mutations."""
...
from verify_cpu_mode_two_parent_vectors import EXPECTED_FIXTURE, validate
...
def main() -> None:
    ...
    add("identity", lambda d: d["calls"][0].__setitem__("native_actor_id", -1))
    add("path", lambda d: d["calls"][0]["executed"].__setitem__(0, "86f6d0"))
    add("child-count", lambda d: d["calls"][0]["child_calls"].__setitem__(
        "target_normal_86e7dc", 2))
    ...
    print(f"[CPU MODE TWO PARENT FIXTURE] PASS: rejected={len(mutations)} before replay")
