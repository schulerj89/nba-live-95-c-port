"""Independent switch evidence mutations; original files remain unchanged."""
...
def main():
    ...
    spec = importlib.util.spec_from_file_location('switch_verifier_audit', a.verifier)
    ...
    index = next(i for i, r in enumerate(original_rows) if r['tag'] == 'switch.entry')
    ...
    for field in ('route', 'actor_words', 'controller_words', 'context_words', 'preserved_words'):
