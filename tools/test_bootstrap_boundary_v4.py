"""Independent parsed-view endpoint and process-output guards. No frozen-file edits."""
 ...
 for case in cases:
  ...
  def rows(path):
   ...
   if Path(path).name=='cpu.jsonl'and case.startswith('native_terminal_'):
    e=next(q for q in d if q['pc']==0x8080bc);assert e['pc']==0x8080bc;key=case[len('native_terminal_'):]
