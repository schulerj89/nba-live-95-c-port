"""Local header trace protocol mutations adapted from the independent codec audit, with unchanged native evidence.
...
def main():
    ...
    nmi_stack_cycle=next(r['cpu_cycles']-9 for r in reader(args.native/'header_work_instructions.jsonl') if r['pc']==0x80815a)
    changes = []
    def backward(rows):
    ...
    def caller_write(rows):
        row=next(r for r in rows if r['cpu_cycles']==nmi_stack_cycle and r['kind']=='write')
        changes.append(dict(event=row['event'],field='value',before=row['value'],after=row['value']^1))
        row['value']^=1
    cases += [('dma_trigger_cycle',dma_cycle),('native_dma_payload',native_dma),('excluded_nmi_stack',caller_write)]
    results = []
    for name, mutation in cases:
        ...
        def altered(path):
            ...
            if name=='dma_trigger_cycle' and path.name in ('native-entry.jsonl.dma.jsonl','default.jsonl.dma.jsonl'):
                mutation(rows)
            elif name in ('native_dma_payload','excluded_nmi_stack') and path.name=='header_work_bus.jsonl':
                mutation(rows)
            elif name not in ('dma_trigger_cycle','native_dma_payload','excluded_nmi_stack') and path.name=='native-entry.jsonl':
                mutation(rows)
            return rows
