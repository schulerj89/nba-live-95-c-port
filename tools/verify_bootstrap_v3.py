...
from bootstrap_trace_protocol_v2 import validate_trace
...
SCRIPT_SHA='aa5d08630983bf394695e036aff2d2bf280338beb10250d1887c2575b4a4a187'
RUNNER_SHA='65f898327b8a7cb110e381c931782434927b763555abb0a0b96f19302d15705c'
DECODER_SHA='b1864664d3ac0abcd439055e88bf7220cf66be7a6ae9dfc5d59b3186f1469a46'
SOURCES=set('tools/bootstrap_probe.c src/nba_bootstrap.c src/nba_bootstrap_cpu.c src/nba_bootstrap_ipl.c src/nba_bootstrap_rom.c src/nba_setup_spc_init.c src/nba_setup_spc_control.c src/nba_rom.c include/nba_bootstrap.h include/nba_bootstrap_internal.h include/nba_setup_codec_work.h include/nba_setup_spc_init.h include/nba_setup_spc_control.h include/nba_setup_spc_resident.h include/nba_rom.h include/nba_types.h src/nba_bootstrap_cpu_program.inc tools/generate_bootstrap_cpu.py tools/build_bootstrap_probe.ps1'.split())
STREAMS=('cpu','spc','cpu_bus','spc_bus','boundaries')
TAGS=('power_entry','cpu_80bc','cpu_dma_request','resident_entry','post_f1')
ARTIFACTS={n+'.jsonl'for n in STREAMS}|{t+'.'+suffix for t in TAGS for suffix in ('state','aram','wram')}|set('stdout.log stderr.log environment.txt complete.txt initial-settings.json capture.lua runner.py'.split())
...
def native(root,rom):
    ...
    check(m['kind']=='normal reset bootstrap observation','native kind')
    ...
    check((root/'complete.txt').read_text()=='ok; normal power-on through actual post-F1 callback\n','completion')
...
def bind_native_boundaries(root,data):
    ...
    known_cpu={'power_entry':0x800d,'cpu_80bc':0x8080bc,'cpu_dma_request':0x808a8d}
...
def main():
    ...
    summary=loads(r.stdout.splitlines()[-1]);check(set(summary)=={'status','boundary_pc','master','spc_ticks','cpu_cycles','spc_pc','spc_phase','upload','resident','f1','refresh'},'C summary schema')
    for k,v in summary.items():check(type(v)is(bool if k in ('resident','f1')else int),'C summary type')
    check(summary['status']==1 and summary['boundary_pc']==0x8080bc and summary['upload']==1264 and summary['resident']and summary['f1'],'actual bounded stop')
    ...
    for t in trace:
        check(integer(t['kind'],0,6)and t['kind']!=4,'C event kind')
    ...
    protocol=validate_trace(trace,summary,raw_rom,decoder)
    ...
    compare(cbus,[t for t in n['cpu_bus']if t['master']<=boundary['master']],('pc','master','address','value','kind'),'CPU data bus')
    ...
    compare([t for t in sbus if ns[0]['cycles']<=t['cycles']<=hi],ns,('pc','cycles','address','value','kind'),'SPC writes and IO')
    check((out/'final.wram').read_bytes()==(root/'cpu_80bc.wram').read_bytes(),'full CPU-boundary WRAM')
    ...
    report={'result':'PASS bounded normal reset/upload/F1; full S1/03DB remains OPEN','cpu_instruction_states':len(ccpu),'cpu_data_accesses':len(cbus),'spc_instruction_states':len(ss),'spc_write_io_accesses':len(ns),'scalar_fields':2*len(scalar_keys),'wram_bytes':131072,'aram_bytes':131072,'profile':'pinned software NTSC zero RAM +40Hz SPC','SPC_master_callback_parity':False,'excluded':'SPC DSP RAM reads/full DSP state, concurrent CPU after80BC, DMA/NMI and03DB','executable_sha256':sha(exe),'native_manifest_sha256':sha(root/'manifest.json'),'trace_sha256':sha(out/'events.jsonl'),'summary':summary,'protocol_v2':protocol,'native_boundary_bindings_v3':len(n['boundaries']),'verifier_sources':{p.name:sha(p) for p in (Path(__file__),Path(__file__).with_name('bootstrap_trace_protocol_v2.py'))},'build':bm}
