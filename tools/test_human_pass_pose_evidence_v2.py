"""Pose/attachment evidence and diagnostic mutation tests, derived from the independent pass audit tool."""
...
def main():
    ...
    metadata('wrong route environment', lambda m: m['environment'].update(NBA95_PASS_POSE_SELECTION='2'))
    ...
    index = next(i for i, r in enumerate(original_rows) if r['tag'] == 'pose.entry')
    ...
    events('missing pose entry', lambda rows: rows.pop(next(i for i,r in enumerate(rows) if r['tag']=='pose.entry')))
    events('missing offset stage', lambda rows: rows.pop(next(i for i,r in enumerate(rows) if r['tag']=='pose.offset.entry')))
    ...
    for field,value in [('cpu_x',65536),('cpu_y',False),('cpu_ps',256),('cpu_ps',32)]:
        events('pose CPU contract '+field+'='+repr(value),lambda rows,k=field,x=value:rows[index].update({k:x}))
    for tag in('pose.entry','pose.resolve.entry','pose.resolve.exit','pose.attach.entry',
               'pose.offset.entry','pose.offset.exit','pose.attach.exit','pose.commit','pose.ready'):
        position=next(i for i,r in enumerate(original_rows)if r['tag']==tag)
        events('binary arithmetic domain '+tag,lambda rows,i=position:rows[i].update(cpu_ps=rows[i]['cpu_ps']|8))
    commit_index=next(i for i,r in enumerate(original_rows)if r['tag']=='pose.commit')
    events('AF30 carried X mismatch',lambda rows:rows[commit_index].update(cpu_x=rows[commit_index]['cpu_x']^256))
    offset_index=next(i for i,r in enumerate(original_rows)if r['tag']=='pose.offset.exit')
    events('B832 X restore mismatch',lambda rows:rows[offset_index].update(cpu_x=rows[offset_index]['cpu_x']^256))
    events('B832 Y restore mismatch',lambda rows:rows[offset_index].update(cpu_y=rows[offset_index]['cpu_y']^1))
    ...
    for field in ('route', 'actor_words', 'controller_words', 'context_words', 'global_words', 'dp_words', 'profile_words', 'order_words'):
