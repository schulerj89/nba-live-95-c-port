/* Native-entry-only AD0E/AE10/AED9 and F473/B47A replay. Each mode reads
 * original pre-call words only; no native expected result enters this probe. */
...
static uint16_t cursor(unsigned address) {
    if(address<0x34d3||address>0x34e7||(address-0x34d1)%2)exit(3);
    return (uint16_t)((address-0x34d1)/2);
}
static NbaHumanPassAlignedState aligned_input(void) {
    NbaHumanPassAlignedState s={0};s.action=action_input();
    unsigned a=word(0x96),b=word(0x8e);
    s.fine_c0=word(0xc0);s.band_b2=word(0xb2);s.scratch_aa=word(0xaa);s.family_ae=word(0xae);
    s.receiver_anchor_8c=word(b+0x8c);s.passer_anchor_8a=word(a+0x8a);
    s.options_07f6=word(0x7f6);s.layout_0956=word(0x956);
    s.lane.source_slot=s.action.passer_slot;s.lane.receiver_slot=s.action.receiver_slot;
    s.lane.source_cursor=cursor(word(a+0x14));s.lane.receiver_cursor=cursor(word(b+0x14));
    for(unsigned i=0;i<11;++i){unsigned p=0x34eb+i*0x100;
        s.lane.actors[i].x=word(p+4);s.lane.actors[i].y=word(p+8);s.lane.actors[i].team=word(p+0x6e);
    }
    for(unsigned i=0;i<13;++i){unsigned p=word(0x34d1+i*2);
        if(p==0)s.lane.order[i]=0xffff;
        else if(p==0x3eeb)s.lane.order[i]=10;
        else {s.lane.order[i]=actor_index(p);if(s.lane.order[i]==0xffff)exit(3);}
    }
    return s;
}
static void project_aligned(const NbaHumanPassAlignedState *s) {
    project_action(&s->action);
    put(0xc0,s->fine_c0);put(0xb2,s->band_b2);put(0xaa,s->scratch_aa);put(0xae,s->family_ae);
    put(0x7f6,s->options_07f6);put(0x956,s->layout_0956);
    put(0x34eb+s->action.passer_slot*0x100+0x8a,s->passer_anchor_8a);
    put(0x34eb+s->action.receiver_slot*0x100+0x8c,s->receiver_anchor_8c);
    for(unsigned i=0;i<13;++i)put(0x34d1+i*2,s->lane.order[i]==0xffff?0:(uint16_t)(0x34eb+s->lane.order[i]*0x100));
}
int main(int argc,char **argv) {
    ...
    while(fgets(line,sizeof(line),stdin)) {
        ...
        load(path);NbaHumanPassAlignedState s=aligned_input();
        NbaHumanPassAlignedRoute route=NBA_HUMAN_PASS_ALIGNED_POSE_AF1D;
        if(!strcmp(mode,"gate"))route=nba_human_pass_aligned_prepare(&assets,&s);
        else if(!strcmp(mode,"choice")){if(!nba_human_pass_aligned_choose(&s))return 3;}
        else if(!strcmp(mode,"install"))route=nba_human_pass_aligned_install(&assets,&s);
        else if(!strcmp(mode,"lane")){if(!nba_human_pass_lane_obstructed(&s.lane,&s.scratch_aa))return 3;}
        else if(!strcmp(mode,"upper")){if(!nba_human_pass_action_upper(&assets,&s.action))return 3;}
        else return 2;
        if(route==NBA_HUMAN_PASS_ALIGNED_INVALID)return 3;
        project_aligned(&s);
        if(!strcmp(mode,"lane")) {
            printf("{\"obstructed\":%u,\"saved_words\":[",s.scratch_aa);
            static const unsigned saved_words[]={0xb6,0xba,0xbe,0xc2,0x9a,0x9e,0xa6};
            for(unsigned i=0;i<sizeof(saved_words)/sizeof(saved_words[0]);++i)printf("%s%u",i?",":"",word(saved_words[i]));
            printf("]");
        }else {
            printf("{\"route\":%u,\"dp_words\":[",(unsigned)route);
            static const unsigned addresses[]={0,0x47,0x49,0x4f,0x51,0xbe,0xc0,0xb2,0xaa,0xae};
            for(unsigned i=0;i<sizeof(addresses)/sizeof(addresses[0]);++i)printf("%s%u",i?",":"",word(addresses[i]));
            printf("]");
        }
        printf(",\"profile_byte\":%u,\"selection_words\":[%u,%u]",s.action.profile_3e,s.options_07f6,s.layout_0956);
        array("order_words",0x34d1,13);state_output();printf("}\n");
    }
