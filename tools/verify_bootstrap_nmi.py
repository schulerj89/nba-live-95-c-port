...
from bootstrap_nmi_trace_protocol import validate as validate_trace
...
SCRIPT_SHA='7953189ac62169f3eedd91a2d2c625265e9502848b29d343ea584e8ec8c0adfd'
RUNNER_SHA='0e2536d6b6ffe8d39ced432a978433162dde6863f19f2f5c56d9cc8cd798524e'
DECODER_SHA='b1864664d3ac0abcd439055e88bf7220cf66be7a6ae9dfc5d59b3186f1469a46'
SOURCES=set(['include/nba_bootstrap.h', 'include/nba_bootstrap_fill.h', 'include/nba_bootstrap_internal.h', 'include/nba_bootstrap_nmi.h', 'include/nba_rom.h', 'include/nba_setup_codec_work.h', 'include/nba_setup_spc_control.h', 'include/nba_setup_spc_init.h', 'include/nba_setup_spc_resident.h', 'include/nba_types.h', 'src/nba_bootstrap.c', 'src/nba_bootstrap_cpu.c', 'src/nba_bootstrap_cpu_program.inc', 'src/nba_bootstrap_ipl.c', 'src/nba_bootstrap_nmi.c', 'src/nba_bootstrap_nmi_cpu.c', 'src/nba_bootstrap_nmi_cpu_program.inc', 'src/nba_bootstrap_rom.c', 'src/nba_rom.c', 'src/nba_setup_spc_control.c', 'src/nba_setup_spc_init.c', 'tools/bootstrap_nmi_probe.c', 'tools/build_bootstrap_nmi_probe.ps1', 'tools/generate_bootstrap_cpu.py', 'tools/generate_bootstrap_nmi_cpu.py'])
STREAMS=('cpu','spc','cpu_bus','spc_bus','boundaries')
TAGS=('power_entry', 'cpu_80bc', 'cpu_dma_request', 'resident_entry', 'post_f1', 'cpu_after_dma', 'cpu_fill_return', 'cpu_clear_after', 'cpu_table_after', 'cpu_da72_return', 'cpu_ab7e_return', 'cpu_nmi_enable', 'cpu_sound_entry', 'cpu_sound_guard_before', 'cpu_sound_guard_after', 'cpu_sound_poll', 'nmi_vector_entry', 'cpu_oam_request')
...
def native(root,rom):
    ...
    check(m['kind']=='normal reset first NMI pre-OAM observation','native kind')
    ...
    check((root/'complete.txt').read_text()=='ok; normal power-on through pre-OAM8184\n','completion')
...
def main():
    ...
    check(summary['status']==1 and summary['boundary_pc']==0x808184 and summary['upload']==1264 and summary['resident']and summary['f1'],'actual bounded stop')
    ...
    check((out/'final.wram').read_bytes()==(root/'cpu_oam_request.wram').read_bytes(),'full CPU-boundary WRAM')
    for tag in ('cpu_fill_return','cpu_clear_after','cpu_table_after','cpu_da72_return','cpu_ab7e_return','cpu_nmi_enable','cpu_sound_entry','cpu_sound_guard_before','cpu_sound_guard_after','cpu_sound_poll','nmi_vector_entry'):
    ...
    check((out/'final.vram').read_bytes()==expected_vram==(root/'cpu_oam_request.vram').read_bytes(),'source and native full first-fill VRAM')
    ...
    final_c=state(out/'final.state');final_n=state(root/'cpu_oam_request.state');check(set(final_c)==final_keys,'final typed field closure')
    for k in final_keys:check(final_c[k]==final_n[k],'final typed field '+k)
    projections={'cpu_80bc':0x8080bc,'cpu_fill_return':0x8080c0,'cpu_clear_after':0x8080ca,'cpu_table_after':0x8080fd,'cpu_da72_return':0x808101,'cpu_ab7e_return':0x808141,'cpu_nmi_enable':0x808145,'cpu_sound_entry':0x809b73,'cpu_sound_guard_before':0x809bed,'cpu_sound_guard_after':0x809bef,'cpu_sound_poll':0x80aacd,'nmi_vector_entry':0x8156,'final':0x808184}
    for name,pc in projections.items():
        cs=state(out/(name+'.nmi'));nsnapshot=state(root/(('cpu_oam_request'if name=='final'else name)+'.state'))
    ...
    report={'result':'PASS bounded normal reset/upload/F1/firstDMA/reset tables/first NMI pre-OAM; full S1/03DB remains OPEN','cpu_instruction_states':len(ccpu),'cpu_and_DMA_data_accesses':len(cbus),'cpu_data_accesses':len(cbus)-protocol['DMA_operations'],'spc_instruction_states':len(ss),'spc_write_io_accesses':len(ns),'scalar_fields':2*len(scalar_keys),'wram_bytes':13*131072,'NMI_control_fields':13*12,'aram_bytes':131072,'vram_bytes':65536,'final_typed_fields':len(final_keys),'profile':'pinned software NTSC zero RAM +40Hz SPC','SPC_master_callback_parity':False,'excluded':'SPC DSP RAM reads/full DSP state, first OAM write8184 onward, RTI, serial controller effects, otherDMA/HDMA and03DB','executable_sha256':sha(exe),'native_manifest_sha256':sha(root/'manifest.json'),'trace_sha256':sha(out/'events.jsonl'),'summary':summary,'protocol_v2':protocol,'native_boundary_bindings':len(n['boundaries']),'verifier_sources':{p.name:sha(p) for p in (Path(__file__),Path(__file__).with_name('bootstrap_boundary_protocol_v2.py'),Path(__file__).with_name('bootstrap_nmi_trace_protocol.py'),Path(__file__).with_name('bootstrap_nmi_profile.py'),Path(__file__).with_name('bootstrap_trace_protocol_v2.py'))},'build':bm}
