...
def main():
 ...
 entries={0x85:[('IrqBand',0xeeee,0xef14)],0x86:[('Normalize',0x9846,0x986d),('Launch',0x99c4,0x9c6f),('AirbornePass',0xa629,0xa6b3),('Mode15',0xa6b3,0xa7a0),('SpecialPass',0xa7a8,0xa7da)],0x87:[('ModeDispatch',0x9244,0x925c),('DispatchTrampoline',0x9bd0,0x9bd3),('Mode15Wrapper',0x9c53,0x9c58)]}
 script=Path(__file__).parent/'ghidra/DumpHumanPassRelease.java'
 ...
 for bank,items in entries.items():
  ...
  (out/f'human_pass_release_bank{bank:02x}.c').write_text(emit_bank(rom,bank,[BankEntry(n,s,e,0,0)for n,s,e in items]))
  ...
  if run.returncode or not(out/f'human_pass_release_bank{bank:02x}.txt').is_file():raise RuntimeError('Ghidra failed; retain output')
 for name,address,size in [('PassLaunchTables',0x869c6f,108),('PassThresholds',0x86a7a0,8),('Mode15DispatchEntry',0x879c0f,4),('ActorPointers',0x879c7b,20)]:
  offset=((address>>16)&0x7f)*0x8000+(address&0x7fff);data=rom[offset:offset+size]
  (out/(name+'.bin')).write_bytes(data);m['ranges'][name]=dict(start=f'{address:06X}',bytes_count=size,sha256=sha(data))
