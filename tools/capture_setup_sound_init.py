"""Natural backdrop NMI source-work capture; read-only instructions and bus inputs."""
import argparse
import hashlib
import json
import os
...
def main():
 ...
 script=out/'capture.lua';shutil.copyfile(Path(__file__).with_name('mesen_setup_sound_init.lua'),script)
 base=out/'scheduler_base.lua';shutil.copyfile(Path(__file__).with_name('mesen_setup_scheduler.lua'),base)
 tail=out/'interrupt_base.lua';shutil.copyfile(Path(__file__).with_name('mesen_setup_interrupt_tail.lua'),tail)
 prefix_script=out/'sound_prefix_base.lua';shutil.copyfile(Path(__file__).with_name('mesen_setup_sound_prefix.lua'),prefix_script)
 ...
 for name,path in [('rom',rom),('mesen',exe),('script',script),('runner',runner),('settings',initial),('base_script',base),('interrupt_script',tail),('sound_prefix_script',prefix_script)]:
 ...
 try:
  ...
  tail_summary=(out/'interrupt_complete.txt').read_text()
  if not tail_summary.startswith('ok; scopes=4; nmi=46; instructions='):raise ValueError('incomplete backdrop interrupt observation')
  manifest['interrupt_summary']=tail_summary
  prefix=(out/'sound_prefix_complete.txt').read_text()
  if not prefix.startswith('ok; calls=46; instructions='):raise ValueError('incomplete sound prefix observation')
  manifest['sound_prefix_summary']=prefix
  init=(out/'sound_init_complete.txt').read_text()
  if not init.startswith('ok; calls='):raise ValueError('incomplete sound initialization observation')
  manifest['sound_init_summary']=init
