...
def main(a):
 ...
 for k,(i,j)in enumerate(scopes):
  ...
  with trace.open('w')as f:run=subprocess.run([str(a.exe.resolve()),str(path),str(endpoint)],stdout=f)
  require(run.returncode==0,'fresh C probe');cr=json_lines(trace);ci=[];cb=[];cycle=0;last=None
