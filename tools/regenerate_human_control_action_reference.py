...
def main():
 ...
 entries={0x84:[('HumanPass',0xdf7a,0xe141),('HumanSwitch',0xe141,0xe2ac)],0x85:[('DirectionZero',0xf347,0xf34f),('DirectionDistance',0xf34f,0xf3bb)]}
 script=Path(__file__).parent/'ghidra/DumpHumanControlAction.java'
 ...
 for bank,items in entries.items():
  ...
  (out/f'control_action_bank{bank:02x}.c').write_text(emit_bank(rom,bank,[BankEntry(n,s,e,0,0)for n,s,e in items]))
  ...
  if bank==0x85:
   table=raw[0xf09a-0x8000:0xf0aa-0x8000]
   m['ranges']['DirectionMap']=dict(start='85:F09A',end_exclusive='85:F0AA',bytes=table.hex(),sha256=sha(table))
  ...
  if run.returncode or not(out/f'control_action_bank{bank:02x}.txt').is_file():raise RuntimeError('Ghidra failed; retain output')
