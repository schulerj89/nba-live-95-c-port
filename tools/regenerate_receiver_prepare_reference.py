...
def main():
 ...
 entries={0x80:[('Random',0xcee7,0xcefd),('QueueEntry32Writer',0xad57,0xad86),('QueueEntryLengthWriter',0xb79e,0xb7c6)],0x85:[('UnsignedDivision',0xf867,0xf8d9),('SignedDivision',0xf8d9,0xf929)],0x86:[('Accuracy',0xaa6a,0xab0d),('ReceiverParent',0xaf66,0xafa6),('SelectorSevenOffset',0xb0e2,0xb0f7),('ReceiverChild',0xb468,0xb625)],0x87:[('PosePoint',0xb7d8,0xb953)]}
 script=Path(__file__).parent/'ghidra/DumpReceiverPrepare.java'
 ...
 for bank,items in entries.items():
  ...
  (out/f'receiver_prepare_bank{bank:02x}.c').write_text(emit_bank(rom,bank,[BankEntry(n,s,e,0,0)for n,s,e in items]))
  ...
  with(out/f'ghidra{bank:02x}.log').open('w')as f:run=subprocess.run(command,env=env,stdout=f,stderr=subprocess.STDOUT,timeout=120,creationflags=subprocess.CREATE_NO_WINDOW)
  m['commands'].append(dict(args=command,exit_code=run.returncode))
  if run.returncode or not(out/f'receiver_prepare_bank{bank:02x}.txt').is_file():raise RuntimeError('Ghidra failed; retain output')
 for name,address,size in [('PoseSelectors',0x86b430,56),('ReceiverBandIncludingOpcode',0x86afa6,36),('AccuracyError',0x86ab0d,32)]:
