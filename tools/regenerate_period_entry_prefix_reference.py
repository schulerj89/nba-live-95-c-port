...
def main():
 ...
 entries={0x86:[('PeriodEntryPrefix',0xdca6,0xdd97)]}
 script=Path(__file__).parent/'ghidra/DumpPeriodEntryPrefix.java'
 ...
 for bank,items in entries.items():
  ...
  (out/f'period_entry_prefix_bank{bank:02x}.c').write_text(emit_bank(rom,bank,[BankEntry(n,s,e,0,0)for n,s,e in items]))
  ...
  if run.returncode or not(out/f'period_entry_prefix_bank{bank:02x}.txt').is_file():raise RuntimeError('Ghidra failed; retain output')
 for name,address,size in [('OvertimeClockTable',0x86e392,8)]:
