...
public class DumpHumanPassAction extends GhidraScript {
 public void run() throws Exception {
  int bank=Integer.parseInt(getScriptArgs()[1],16);
  int start=bank==0x86?0xac50:0xb47a, end=bank==0x86?0xb04b:0xb4da;
  ...
  int[] seeds=bank==0x86?new int[]{0xac50,0xafc4,0xb00b}:new int[]{0xb47a};
  for(int seed:seeds)disassemble(toAddr(seed));
  try(PrintWriter out=new PrintWriter(new File(getScriptArgs()[0],String.format("human_pass_action_bank%02x.txt",bank)),"UTF-8")){
