...
$analysisDirectory = Join-Path $projectRoot '.analysis\team_select_ghidra'
...
foreach ($bank in @('80', '82')) {
    ...
    & $headless $projectDirectory ("NbaLive95TeamSelect{0}" -f $bank) `
        ...
        -postScript 'DumpTeamSelect.java' $analysisDirectory $bank
    if ($LASTEXITCODE -ne 0) { throw "Ghidra failed for bank $bank (exit $LASTEXITCODE)" }
}

Write-Host "Team Select analysis written to: $analysisDirectory" -ForegroundColor Green
