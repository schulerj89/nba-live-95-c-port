...
$outputDirectory = Join-Path $analysisRoot 'gameplay85-closure-ghidra'
...
foreach ($bank in @('80', '85', '86', '87')) {
    ...
    & $headless $projectDirectory ("NbaLive95Gameplay85Closure{0}" -f $bank) `
        ...
        -postScript 'DumpGameplay85Closure.java' $outputDirectory $bank $analysisRoot
    if ($LASTEXITCODE -ne 0) {
        throw "Gameplay-85 Ghidra closure failed for bank $bank"
    }
}

Write-Host "Gameplay-85 closure written to: $outputDirectory" -ForegroundColor Green
