...
| `mesen_setup_menus_capture.lua` | Rules/Options VRAM/CGRAM/OAM, per-frame open/return VRAM traces, full PPU timing, WRAM commits and DSP sounds; set `NBA95_CAPTURE_MENU=rules` or `options`, `NBA95_CAPTURE_VARIANTS=1` for OFF/MONO/CPU, or `NBA95_CAPTURE_CALLS=1` for deduplicated JSR/JSL targets |
