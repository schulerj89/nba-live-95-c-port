"""Retain controlled native `$86:F23F` role-flag witnesses for replay."""
...
def main():
    ...
    if not (len(vectors) == len(cases) == len(paths) == len(CASE_NAMES)):
        raise ValueError("expected exactly two aligned mode-three witnesses")
    ...
    for number, (vector, case, path, name) in enumerate(
        ...
        expected_exit = "86f2c9" if number == 1 else "86f2c0"
        if vector["entry_pc"] != "86f23f" or vector["exit_pc"] != expected_exit:
            raise ValueError(f"case {number} has an unexpected boundary")
        executed = path["executed"]
        if not executed or executed[0] != "86f23f" or executed[-1] != expected_exit:
            raise ValueError(f"case {number} has an incomplete PC path")
        if number == 1 and ("86f29a" not in executed or
                            "86f29f" not in executed or
                            "86f2b0" in executed):
            raise ValueError("role-clear case did not use formation fallback")
        if number == 2 and not all(
                pc in executed for pc in ("86f29a", "86f2b0", "86f2c0")):
        ...
        if word(before, actor + 0x5E) != 3 or word(after, actor + 0x5E) != 3:
            raise ValueError(f"case {number} did not preserve mode three")
    ...
    print("[CPU MODE THREE ROLE NORMALIZE] calls=2 entry=86f23f exits=86f2c0/86f2c9")
