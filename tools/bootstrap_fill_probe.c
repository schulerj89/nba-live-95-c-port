...
#include <stdlib.h>
#include <string.h>
typedef struct {FILE *trace;NbaBootstrap *state;NbaBootstrapFill *owner;const char *directory;bool failed;} Output;
...
static bool final_state(const char *directory,const NbaBootstrapFill *owner){
...
int main(int argc,char **argv){
 NbaRom rom={0};NbaBootstrap *s;NbaBootstrapFill *owner,*prior;Output out;char path[2048];unsigned long steps;
 ...
 if(!nba_bootstrap_fill_power_on(owner,rom.data,rom.size,NBA_BOOT_PROFILE_NTSC_ZERO_32040))return 5;
 ...
 for(steps=0;steps<2000000&&s->status==NBA_BOOT_RUNNING;steps++)(void)nba_bootstrap_fill_step(owner,observe,&out);
 if(out.failed||fclose(out.trace))return 8;
 *prior=*owner;if(nba_bootstrap_fill_step(owner,observe,&out)||memcmp(prior,owner,sizeof(*owner)))return 9;
