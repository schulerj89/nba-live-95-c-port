...
from setup_spc_evidence_contract_v3 import loads, capture_envelope, build_envelope, state_file, normal_spc_state, resident_terminal
...
def json_lines(p):return [loads(s)for s in Path(p).read_text().splitlines()]
def subset(g,w):
 for k,v in w.items():
...
def read_native(p,rom):
 m=loads((p/'manifest.json').read_text());capture_envelope(m,p,rom,'cold-reset resident SPC observation; isolated component witness only');require(m['schema']==1 and type(m['schema'])is int and m['accepted']is True and m['state_injection']is False and m['rom_patch']is False and type(m['exit_code'])is int and m['exit_code']==0 and m['initial_saves']==[],'capture contract')
 ...
 initial=loads((p/'initial-settings.json').read_text());require(initial==m['settings'],'manifest settings differ');subset(loads((p/'portable-mesen/settings.json').read_text(encoding='utf-8-sig')),initial)
 ...
 require(all(r['tag']=='init.source' for r in bounds[1:-2]),'initialization boundary tags')
 require(all((r['address']>>16)in (0,0x80,0x82) and 0x2140<=(r['address']&65535)<=0x2143 for r in rows['cpu_ports']),'CPU port hook address range')
 require(all(times[0]<=r['spc_cycle']<=bounds[-1]['spc_cycle'] for r in rows['cpu_ports']),'CPU port scope chronology')
 ...
 for tag,bound in (('upload_entry',bounds[0]),('poll_entry',bounds[-2]),('end',bounds[-1])):
  ram=(p/f'spc_resident_{tag}.aram').read_bytes();require(len(ram)==65536 and ram[0x380:0x870]==payload,'uploaded source identity')
  state=state_file(p/f'spc_resident_{tag}.state');normal_spc_state(state)
  for key in ('pc','a','x','y','sp','ps'):require(state['spc.'+key]==str(bound[key]),'native state boundary '+key)
  require(state['spc.cycle']==str(bound['spc_cycle']),'native state boundary cycle')
  for name,field in (('cpuRegs','input'),('outputReg','output')):
   for port in range(4):require(state[f'spc.{name}[{port}]']==str(bound[f'{field}{port}']),'native state boundary port')
 require(not any(r['kind']=='write' and r['address']==0xf0 for r in nb),'SPC speed/write preconditions may not change inside capture')
 return m,rows
...
def check_build(exe):
 m=loads((exe.parent/'build-manifest.json').read_text(encoding='utf-8-sig'));build_envelope(m);require(type(m['schema'])is int and m['schema']==1 and type(m['compiler_exit'])is int and m['compiler_exit']==0 and set(m['sources'])==BUILD,'build closure')
...
def main(a):
 ...
 for k,(i,j)in enumerate(scopes):
  ...
  s=entry;dsp=0
  states=state_file(p/'spc_resident_poll_entry.state');dsp=int(states['spc.dspReg'])
  for r in nb:
   if r['spc_cycle']>entry['spc_cycle']:break
