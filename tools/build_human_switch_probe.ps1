...
$privateBuild=Join-Path $root 'build\human-control-action'
...
cl /nologo /W4 /O2 /MD /utf-8 /I "$root\include" /Fe"$privateBuild\human_switch_probe.exe" /Fo"$privateBuild\human_switch_probe.obj" "$root\tools\human_switch_probe.c" "$privateBuild\nba_human_switch.obj" $(($objects|ForEach-Object{'"'+$_+'"'})-join ' ') user32.lib gdi32.lib winmm.lib
exit /b %ERRORLEVEL%
"@
