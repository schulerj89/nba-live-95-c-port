...
cl /nologo /W4 /WX /O2 /MD /utf-8 /I "$root\include" /Fe"$output\setup_codec_work_probe.exe" /Fo"$output\\" "$root\tools\setup_codec_work_probe.c" "$root\src\nba_setup_codec_work.c"
exit /b %ERRORLEVEL%
"@
...
$files = @('include/nba_setup_codec_work.h', 'src/nba_setup_codec_work.c', 'tools/setup_codec_work_probe.c', 'tools/build_setup_codec_work_probe.ps1')
...
$exe = Join-Path $output 'setup_codec_work_probe.exe'
