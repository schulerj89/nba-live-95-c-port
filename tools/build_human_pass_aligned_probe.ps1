...
$privateBuild=Join-Path $root 'build\human-pass-aligned'
...
$batch=Join-Path $privateBuild 'compile_human_pass_aligned.bat'
$command=@"
@echo off
call "$vcvars" >nul
cl /nologo /W4 /O2 /MD /utf-8 /I "$root\include" /c /Fo"$privateBuild\nba_human_pass_aligned.obj" "$root\src\nba_human_pass_aligned.c"
if errorlevel 1 exit /b %ERRORLEVEL%
cl /nologo /W4 /O2 /MD /utf-8 /I "$root\include" /c /Fo"$privateBuild\nba_human_pass_action_dependency.obj" "$root\src\nba_human_pass_action.c"
if errorlevel 1 exit /b %ERRORLEVEL%
cl /nologo /W4 /O2 /MD /utf-8 /I "$root\include" /Fe"$privateBuild\human_pass_aligned_probe.exe" /Fo"$privateBuild\human_pass_aligned_probe.obj" "$root\tools\human_pass_aligned_probe.c" "$privateBuild\nba_human_pass_aligned.obj" "$privateBuild\nba_human_pass_action_dependency.obj" $(($objects|ForEach-Object{'"'+$_+'"'})-join ' ') user32.lib gdi32.lib winmm.lib
exit /b %ERRORLEVEL%
"@
