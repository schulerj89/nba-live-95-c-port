...
cl /nologo /W4 /WX /O2 /MD /utf-8 /I "$root\include" /Fe"$output\period_restart_probe.exe" /Fo"$output\\" "$root\tools\period_restart_probe_v2.c" "$root\src\nba_period_restart_v2.c"
exit /b %ERRORLEVEL%
"@
Set-Content -LiteralPath $batch -Value $command -Encoding ASCII
& cmd /c $batch
if ($LASTEXITCODE -ne 0) { throw 'Fresh period restart compilation failed.' }
$files = @('include/nba_period_restart_v2.h','src/nba_period_restart_v2.c','tools/period_restart_probe_v2.c','tools/period_restart_probe_fields.inc','tools/build_period_restart_probe_v2.ps1')
...
$exe = Join-Path $output 'period_restart_probe.exe'
