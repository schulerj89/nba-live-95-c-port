...
def main():
 ...
 entries={0x80:[('Random',0xcee7,0xcefd)],0x85:[('CatchDirection',0xf02d,0xf09a),('TargetDirectionReference',0xf347,0xf3bb),('BasketLane',0xf5e4,0xf728)],0x86:[('CatchPrefix',0xad3d,0xae10),('CatchReceiverSetup',0xaf66,0xafa6)]}
 script=Path(__file__).parent/'ghidra/DumpHumanPassCatch.java'
 ...
 for bank,items in entries.items():
  ...
  (out/f'human_pass_catch_bank{bank:02x}.c').write_text(emit_bank(rom,bank,[BankEntry(n,s,e,0,0)for n,s,e in items]))
  ...
  if run.returncode or not(out/f'human_pass_catch_bank{bank:02x}.txt').is_file():raise RuntimeError('Ghidra failed; retain output')
 for name,address,size in [('CatchRatingTable',0x80d04a,32),('CatchBandTableIncludingBand30Opcode',0x86afa6,36),('CatchDirectionMap',0x85f09a,16)]:
