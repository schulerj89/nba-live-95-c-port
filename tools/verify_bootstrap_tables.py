...
SCRIPT_SHA='861a383e1de7e11b9342c7fe0a83c175c78a5a8a117592e3db4758eb8daf0c74'
RUNNER_SHA='e369dedb343126f0a07bb12b9718e74dd012d7eec24a8539ca1dd2402f030ff7'
DECODER_SHA='b1864664d3ac0abcd439055e88bf7220cf66be7a6ae9dfc5d59b3186f1469a46'
SOURCES=set(['include/nba_bootstrap.h', 'include/nba_bootstrap_fill.h', 'include/nba_bootstrap_internal.h', 'include/nba_bootstrap_tables.h', 'include/nba_rom.h', 'include/nba_setup_codec_work.h', 'include/nba_setup_spc_control.h', 'include/nba_setup_spc_init.h', 'include/nba_setup_spc_resident.h', 'include/nba_types.h', 'src/nba_bootstrap.c', 'src/nba_bootstrap_cpu.c', 'src/nba_bootstrap_cpu_program.inc', 'src/nba_bootstrap_fill.c', 'src/nba_bootstrap_ipl.c', 'src/nba_bootstrap_rom.c', 'src/nba_bootstrap_tables.c', 'src/nba_bootstrap_tables_cpu.c', 'src/nba_bootstrap_tables_cpu_program.inc', 'src/nba_rom.c', 'src/nba_setup_spc_control.c', 'src/nba_setup_spc_init.c', 'tools/bootstrap_tables_probe.c', 'tools/build_bootstrap_tables_probe.ps1', 'tools/generate_bootstrap_cpu.py', 'tools/generate_bootstrap_tables_cpu.py'])
STREAMS=('cpu','spc','cpu_bus','spc_bus','boundaries')
TAGS=('power_entry','cpu_80bc','cpu_dma_request','resident_entry','post_f1','cpu_after_dma','cpu_fill_return','cpu_clear_after','cpu_table_after','cpu_da72_return','cpu_ab7e_return','cpu_nmi_enable')
...
def native(root,rom):
    ...
    check(m['kind']=='normal reset table initialization observation','native kind')
    ...
    check((root/'complete.txt').read_text()=='ok; normal power-on through pre-NMI-enable8145\n','completion')
    ...
    for name,stream in data.items():
        ...
        for j,r in enumerate(stream):
            ...
            if name.endswith('_bus'):
                ...
                if name=='cpu':check(integer(r['db'],0,255)and r['dp']==0 and type(r['emulation'])is bool,'native CPU domain')
...
def bind_native_boundaries(root,data):
    ...
    known_cpu={'power_entry':0x800d,'cpu_80bc':0x8080bc,'cpu_dma_request':0x808a8d,'cpu_after_dma':0x808a92,'cpu_fill_return':0x8080c0,'cpu_clear_after':0x8080ca,'cpu_table_after':0x8080fd,'cpu_da72_return':0x808101,'cpu_ab7e_return':0x808141,'cpu_nmi_enable':0x808145}
    ...
    for r in data['boundaries']:
        ...
        if r['tag']in known_cpu:
            ...
            check(any((x['pc'],x['master'],x['cycles'])==(pc,r['master'],r['cpu'])for x in data['cpu']),'CPU source hook instruction')
        else:
            pc=known_spc[r['tag']]
...
def main():
    ...
    check(summary['status']==1 and summary['boundary_pc']==0x808145 and summary['upload']==1264 and summary['resident']and summary['f1'],'actual bounded stop')
    ...
    check((out/'final.wram').read_bytes()==(root/'cpu_nmi_enable.wram').read_bytes(),'full CPU-boundary WRAM')
    for tag in ('cpu_fill_return','cpu_clear_after','cpu_table_after','cpu_da72_return','cpu_ab7e_return'):
    ...
    check((out/'final.vram').read_bytes()==expected_vram==(root/'cpu_nmi_enable.vram').read_bytes(),'source and native full first-fill VRAM')
    ...
    final_c=state(out/'final.state');final_n=state(root/'cpu_nmi_enable.state');check(set(final_c)==final_keys,'final typed field closure')
    for k in final_keys:check(final_c[k]==final_n[k],'final typed field '+k)
    report={'result':'PASS bounded normal reset/upload/F1/firstDMA/reset tables; NMI/full S1/03DB remains OPEN','cpu_instruction_states':len(ccpu),'cpu_and_DMA_data_accesses':len(cbus),'cpu_data_accesses':len(cbus)-protocol['DMA_operations'],'spc_instruction_states':len(ss),'spc_write_io_accesses':len(ns),'scalar_fields':2*len(scalar_keys),'wram_bytes':7*131072,'aram_bytes':131072,'vram_bytes':65536,'final_typed_fields':len(final_keys),'profile':'pinned software NTSC zero RAM +40Hz SPC','SPC_master_callback_parity':False,'excluded':'SPC DSP RAM reads/full DSP state, concurrent CPU from8145, otherDMA/HDMA, NMI and03DB','executable_sha256':sha(exe),'native_manifest_sha256':sha(root/'manifest.json'),'trace_sha256':sha(out/'events.jsonl'),'summary':summary,'protocol_v2':protocol,'native_boundary_bindings':len(n['boundaries']),'verifier_sources':{p.name:sha(p) for p in (Path(__file__),Path(__file__).with_name('bootstrap_fill_trace_protocol.py'),Path(__file__).with_name('bootstrap_trace_protocol_v2.py'))},'build':bm}
