...
#include <stdlib.h>
#include <string.h>
typedef struct {FILE *trace;NbaBootstrap *state;NbaBootstrapNmi *owner;const char *directory;bool failed,poll_saved;} Output;
...
static bool final_state(const char *directory,const NbaBootstrapNmi *owner){
 char path[2048];FILE *f;const NbaBootstrap *s=&owner->machine.core;const NbaCodecWorkEntry *r=&s->cpu.work.registers;
 ...
 fprintf(f,"ppu.vramAddress=%u\nppu.vramReadBuffer=%u\ndmaController.channel[1].srcAddress=%u\ndmaController.channel[1].srcBank=%u\ndmaController.channel[1].transferSize=%u\ndmaController.channel[1].transferMode=%u\ndmaController.channel[1].fixedTransfer=%s\ndmaController.channel[1].invertDirection=%s\ndmaController.channel[1].decrement=%s\ndmaController.channel[1].dmaActive=%s\n",
  owner->machine.vram_address,owner->machine.vram_read_buffer,s->io[0x2312]|((unsigned)s->io[0x2313]<<8),s->io[0x2314],s->io[0x2315]|((unsigned)s->io[0x2316]<<8),s->io[0x2310]&7,
  s->io[0x2310]&8?"true":"false",s->io[0x2310]&128?"true":"false",s->io[0x2310]&16?"true":"false",owner->machine.dma.pending?"true":"false");
 return fclose(f)==0;
}
...
static void observe(void *context,const NbaBootstrapEvent *e){
 ...
 if(e->kind==NBA_BOOT_EVENT_CPU_ENTRY){
  ...
  if(name){char prefix[128];size_t len=strlen(name)-5;
   ...
   memcpy(prefix,name,len);prefix[len]=0;if(!nmi_state(o->directory,prefix,o->owner))o->failed=true;
  }
 }
 if(e->kind==NBA_BOOT_EVENT_CPU_ENTRY&&e->pc==0x8080bc){if(!save(o->directory,"cpu_80bc.wram",s->wram,sizeof(s->wram))||!nmi_state(o->directory,"cpu_80bc",o->owner))o->failed=true;}
...
int main(int argc,char **argv){
 NbaRom rom={0};NbaBootstrap *s;NbaBootstrapNmi *owner,*prior;Output out;char path[2048];unsigned long steps;
 if(argc!=3)return 2;if(!nba_rom_load_file(&rom,argv[1]))return 3;
 owner=calloc(1,sizeof(*owner));prior=malloc(sizeof(*owner));if(!owner||!prior)return 4;s=&owner->machine.core;
 if(!nba_bootstrap_nmi_power_on(owner,rom.data,rom.size,NBA_BOOT_PROFILE_NTSC_ZERO_32040))return 5;
 ...
 for(steps=0;steps<2000000&&s->status==NBA_BOOT_RUNNING;steps++)(void)nba_bootstrap_nmi_step(owner,observe,&out);
 if(out.failed||fclose(out.trace))return 8;
 *prior=*owner;if(nba_bootstrap_nmi_step(owner,observe,&out)||memcmp(prior,owner,sizeof(*owner)))return 9;
 if(!save(argv[2],"final.aram",s->spc_bus.aram,65536)||!save(argv[2],"final.wram",s->wram,sizeof(s->wram)))return 10;
 if(!save(argv[2],"final.vram",owner->machine.vram,sizeof(owner->machine.vram))||!final_state(argv[2],owner)||!nmi_state(argv[2],"final",owner))return 12;
 ...
 printf("\"dma_bytes\":%u,\"vram_address\":%u,\"sync_counter\":%u,\"source_index\":%u}\n",owner->machine.dma.transferred,owner->machine.vram_address,owner->machine.dma.clock_counter,owner->machine.dma.source_index);
