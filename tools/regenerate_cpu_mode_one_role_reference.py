"""Generate fresh Ghidra and snesrecomp references for CPU mode one."""
...
FIRST = 0xF1B0
END = 0xF23F
REQUIRED = (0xF1B0, 0xF1C9, 0xF1E1, 0xF1FE, 0xF20B, 0xF210,
            0xF217, 0xF21F, 0xF225, 0xF22D, 0xF235, 0xF236, 0xF23E)
...
def main():
    ...
    script = Path(__file__).with_name("ghidra") / "DumpCpuModeOneRole.java"
    ...
    generated = emit_bank(rom, bank, [
        BankEntry("CpuModeOneRole", FIRST, END, 0, 0)
    ])
    (out / "cpu_mode_one_role_bank86.c").write_text(
    ...
    project = out / "project"
    project.mkdir()
    command = [str(args.ghidra), str(project), "CpuModeOneRole", "-import",
    ...
    listing = out / "cpu_mode_one_role_bank86.txt"
    ...
    print("bank 86: fresh Ghidra and recomp CPU mode-one role references")
