"""Separate source-work and native validation for the bounded backdrop producer continuation.
...
BUILD_SOURCES = {'include/nba_setup_codec_work.h', 'src/nba_setup_fb30_work.c', 'include/nba_setup_fb30_work.h', 'tools/generate_setup_producer_work.py', 'src/nba_setup_producer_program.inc', 'tools/setup_producer_work_probe.c', 'src/nba_setup_codec_work.c', 'src/nba_setup_fb30_program.inc', 'include/nba_setup_producer_work.h', 'tools/build_setup_producer_work_probe.ps1', 'src/nba_setup_producer_work.c'}
CAPTURE_REVISIONS = {'script': 'e1aae6bed286b144f7c938e04412919fde4be899e80871c583d5b3c7910bb067', 'base_script': '1653867f508681901809885a72956e822417abfab0eaa246eda9b854673f5285', 'runner': '47258819cc673f75f0eb18df5d1fea38a5da2823fce7b31dca4ca122ab0223cb', 'codec_script': 'd7bc0dfb9678ccc3ce0aabde1e55ce8ee24b9da61b0cdd6ffdf558bae3597f78'}
...
def validate_capture_manifest(manifest, directory, rom):
    ...
    exact_keys(manifest, fields | {'codec_summary', 'producer_summary'}, 'capture manifest')
...
def check_build(exe):
    ...
    source_root = Path(manifest['sources']['src/nba_setup_producer_work.c']['path']).resolve().parents[1]
...
def native_records(directory):
    return fb46.native_records(directory)


def producer_records(directory):
    ...
    instructions = json_lines(directory/'producer_instructions.jsonl')
    integer_rows(instructions,dict(common,instruction=2**31-1),'instruction')
    require(all(r['scope']==1 and r['dp']==0 for r in instructions), 'producer observation scope differs')
    boundaries = json_lines(directory/'producer_boundaries.jsonl')
    require(len(boundaries)==48, 'four producer scopes with five codec brackets required')
    for index,row in enumerate(boundaries):
        exact_keys(row, {*common,'tag','event'}, 'producer boundary')
        for key,limit in common.items(): integer(row[key],0,limit,key)
        integer(row['event'],index,index,'producer event')
        position=index%12
        tag='producer.entry' if position==0 else 'producer.exit' if position==11 else 'codec.entry' if position%2 else 'codec.exit'
        pc={'producer.entry':0x80ec68,'producer.exit':0x80eec6,'codec.entry':0x80c62b,'codec.exit':0x80c682}[tag]
        require(row['tag']==tag and row['pc']==pc and row['scope']==index//12+1 and
                row['dp']==0 and row['ps']&0x38==0, 'producer bracket source contract differs')
        if index: require(boundaries[index-1]['master_clock']<row['master_clock'], 'producer boundaries not chronological')
    bus=json_lines(directory/'producer_bus.jsonl')
    for index,row in enumerate(bus):
        ...
        exact_keys(row,{'kind',*schema},'producer bus')
        ...
        require(row['kind'] in ('read','write') and row['scope']==1 and row['event']==index and
                row['dma_active'] in (0,2), 'unexpected producer bus source')
    expected=f'ok; scopes=4; boundaries=48; instructions={len(instructions)}; bus={len(bus)}\n'
    require((directory/'producer_complete.txt').read_text()==expected and
            read_json(directory/'manifest.json')['producer_summary']==expected, 'producer completion differs')
...
def run_probe(exe,rom,output,entry=None):
    ...
    require(result.returncode==0 and result.stderr=='','fresh producer probe failed')
    ...
    exact_keys(report,{'schema','status','bus_valid','return_pc','cycles','master','slow','instructions',
        'dma_bytes','dma_jobs','cursor','sp','wmadd','counts'},'producer report')
    ...
    require(report['schema']==1 and report['status']==1 and report['bus_valid'] is True and
            report['return_pc']==0x81d018,'producer did not return normally')
    require((report['cycles'],report['master'],report['instructions'],report['dma_bytes'],report['dma_jobs'])==
            (550552,3493454,155750,27726,73),'independent producer source checksum differs')
    require(report['master']==6*report['cycles']+2*report['slow'],'intrinsic bus accounting differs')
    require(report['sp']==(entry['sp'] if entry else 0x1fef)+3,'source return stack differs')
    require(type(report['counts']) is dict and report['counts'],'source block counts missing')
    for pc,count in report['counts'].items():
        require(re.fullmatch(r'80[0-9A-F]{4}',pc) is not None,'invalid producer PC count')
        integer(count,1,2**31-1,'source count')
    require(sum(report['counts'].values())==report['instructions'],'source counts do not sum')
    scratch=prefix.with_suffix('.wram').read_bytes();require(len(scratch)==0x20000,'WRAM output size differs')
    traces=json_lines(prefix.with_suffix('.jsonl'))
    ci,cw=validate_source_events(traces,report)
...
def main():
    ...
    old=fb46.read_native(previous,rom)
    invariance=check_previous(rows,old)
    for name in ('codec_instructions.jsonl','codec_writes.jsonl','codec_boundaries.jsonl'):
        require((native/name).read_bytes()==(previous/name).read_bytes(),'previous codec observation changed: '+name)
    boundaries,codec_instructions,codec_writes=native_records(native)
    pb,pi,pbus=producer_records(native)
    # Check original observations before sorting source fragments for composition.
    validate_chronology(pb, 'producer boundaries', strict=True)
    validate_chronology(pi, 'producer original instructions', strict=True)
    validate_chronology(codec_instructions, 'codec original instructions', strict=True)
    validate_chronology(codec_writes, 'codec original writes')
    full_instructions = pi + [r for r in codec_instructions if 1 <= r['call'] <= 5]
    full_instructions.sort(key=lambda r: r['cpu_cycles'])
    validate_native_scope(full_instructions, pbus, pb[0], pb[11], 'producer native')
    validate_native_scope(full_instructions, [r for r in codec_writes if 1 <= r['call'] <= 5],
                          pb[0], pb[11], 'producer codec writes')
    output.mkdir(parents=True,exist_ok=False)
    report,scratch,ci,cw,cdma=run_probe(exe,rom,output,pb[0])
    default,default_scratch,_,_,default_dma=run_probe(exe,rom,output)
    require(all(report[k]==default[k] for k in ('cycles','master','counts','dma_bytes','dma_jobs')) and cdma==default_dma,
            'native typed entry changed work or DMA payloads')
    require(scratch[0x12000:0x138c0]==default_scratch[0x12000:0x138c0],'native typed entry changed produced WRAM')
    ni=[r for r in pi if r['pc']!=0x81d018]
    require(len(pi)-len(ni)==1 and all(r['pc']!=0x80815a for r in pi), 'canonical residual interrupt/caller scope differs')
    caller=pi[-1]
    require(caller['pc']==0x81d018,'native caller JSL missing')
    caller_writes=[r for r in pbus if r['kind']=='write' and r['pc']==0x81d018 and not r['dma_active']]
    require(len(caller_writes)==3 and pb[11]['cpu_cycles']-caller['cpu_cycles']==8 and
            pb[11]['sp']==caller['sp']-3,'excluded caller JSL source protocol differs')
    for index,(value,offset)in enumerate(((0x81,4),(0xd0,7),(0x1b,8))):
        row=caller_writes[index]
        require(row['address']==caller['sp']-index and row['value']==value and
                row['cpu_cycles']==caller['cpu_cycles']+offset,'excluded caller JSL stack write differs')
    nw=[r for r in pbus if r['kind']=='write' and r['dma_active']==0 and r['pc']!=0x81d018]
    nmi_count=0
    for call in range(1,6):
        helper=fb30 if call==1 else fb46
        instructions,writes,nmis=helper.separate_native_effects(call,codec_instructions,codec_writes)
        ni+=instructions;nw+=writes;nmi_count+=nmis
    ni.sort(key=lambda r:r['cpu_cycles']);nw.sort(key=lambda r:r['cpu_cycles'])
    require(len(ni)==len(ci) and len(nw)==len(cw),'composed source event lengths differ')
    require(Counter(f'{r["pc"]:06X}'for r in ni)==report['counts'],'source path counts differ')
    for index,(c,n) in enumerate(zip(ci,ni)):
        require(all(c[k]==n[k]for k in ('pc','a','x','y','ps','db','sp')),f'instruction/register mismatch {index}')
    nmi_in=[r for r in rows if r['tag']=='nmi.entry' and pb[0]['master_clock']<r['master_clock']<caller['master_clock']]
    nmi_out=[r for r in rows if r['tag']=='nmi.exit' and pb[0]['master_clock']<r['master_clock']<caller['master_clock']]
    require(len(nmi_in)==len(nmi_out)==nmi_count,'composed NMI brackets differ')
    nn=ni[1:]+[caller];cn=ci[1:]+[dict(cycle=report['cycles']+1,master=report['master'])]
    local_work=Counter();inside=False
    dma_service_cycles={r['cpu_cycles'] for r in pbus if r['dma_active']}
    intrinsic_checked=0
    for index,(c,next_c,n,next_n) in enumerate(zip(ci,cn,ni,nn)):
        interrupts=[(a,b)for a,b in zip(nmi_in,nmi_out)if n['master_clock']<a['master_clock']<next_n['master_clock']]
        cpu=next_n['cpu_cycles']-n['cpu_cycles']-sum(b['cpu_cycles']-a['cpu_cycles']+19 for a,b in interrupts)
        require(cpu==next_c['cycle']-c['cycle'],f'instruction CPU work differs {index}')
        if n['cpu_cycles']+2 not in dma_service_cycles:
            master=next_n['master_clock']-n['master_clock']-sum(b['master_clock']-a['master_clock']+142 for a,b in interrupts)
            residual=master-(next_c['master']-c['master'])
            require(residual>=0 and residual%40==0,f'non-DMA instruction intrinsic work differs {index}')
            intrinsic_checked+=1
        if c['pc']==0x80c62b:inside=True
        if c['pc']==0x80c682:inside=False
        if not inside:
            pc=c['pc']&65535
            group='codec_rtl' if pc==0xc682 else 'queue_helper' if 0x86da<=pc<=0x86ec else 'palette' if 0x8a02<=pc<=0x8a41 else 'fill' if 0x8ad2<=pc<=0x8b34 else 'graphics' if 0x8ba1<=pc<=0x8bcf else 'map_rows' if 0x8cd0<=pc<=0x8d98 else 'transform' if 0xea4b<=pc<=0xea78 else 'routing'
            local_work[group]+=cpu
    require(sum(local_work.values())==26479,'source-derived routing/helper work differs')
    instruction_index=0
    for index,(c,n)in enumerate(zip(cw,nw)):
        require(all(c[k]==n[k]for k in ('pc','address','value')),f'ordered CPU write differs {index}')
        ...
        require(c['cycle']-ci[instruction_index]['cycle']+1==n['cpu_cycles']-ni[instruction_index]['cpu_cycles'],
                f'CPU write bus position differs {index}')
    native_dma=[r for r in pbus if r['dma_active']==2]
    require(len(native_dma)==2*len(cdma)==2*report['dma_bytes'],'DMA effect count differs')
    triggers=[(c,n)for c,n in zip(cw,nw) if c['address']&65535==0x420b]
    require(len(triggers)==73 and all(c['value']==2 for c,n in triggers),'DMA trigger source contract differs')
    job_sizes=Counter()
    for index,(c,r,w)in enumerate(zip(cdma,native_dma[::2],native_dma[1::2])):
        schema=dict(job=72,index=65535,cycle=report['cycles'],source=0xffffff,address=0xffff,value=255)
        exact_keys(c,schema,'C DMA effect')
        for key,limit in schema.items():integer(c[key],0,limit,key)
        require(c['job']==0 if index==0 else c['job'] in (cdma[index-1]['job'],cdma[index-1]['job']+1),'DMA job order differs')
        require(c['index']==job_sizes[c['job']],'DMA byte index discontinuity');job_sizes[c['job']]+=1
        trigger,native_trigger=triggers[c['job']]
        require(c['cycle']==trigger['cycle'] and r['cpu_cycles']==native_trigger['cpu_cycles']+2,
                'DMA request/service instruction boundary differs')
        require(r['kind']=='read' and w['kind']=='write' and w['event']==r['event']+1 and
                ...
                f'ordered DMA source/payload effect differs {index}')
    require(len(job_sizes)==73,'DMA job cardinality differs')
    matches=[]
    for scope in range(1,5):
        start,end=pb[(scope-1)*12],pb[scope*12-1]
        native_scratch=(native/f'producer_{scope:02d}_exit.wram').read_bytes()
        require(native_scratch[0x12000:0x138c0]==scratch[0x12000:0x138c0],'complete produced scratch differs')
        ...
        require(len(ins)==len(outs),'unpaired producer interrupt')
        pure=end['cpu_cycles']-start['cpu_cycles']-sum(b['cpu_cycles']-a['cpu_cycles']+19 for a,b in zip(ins,outs))
        require(pure==report['cycles']+8,'producer plus caller source CPU conservation differs')
        matches.append(dict(scope=scope,pure_cpu=pure,nmis=len(ins)))
    identity={name:dict(path=str(path),sha256=digest(path))for name,path in {
        'rom':rom,'native_manifest':native/'manifest.json','previous_manifest':previous/'manifest.json',
        'verifier':Path(__file__),'strict_scheduler_verifier':Path(__file__).with_name('verify_setup_scheduler.py'),
        'fb30_verifier':Path(fb30.__file__),'fb46_verifier':Path(fb46.__file__),
    ...
    proof=dict(schema=1,scope='EC68 through EDF8 RTL; source caller JSL adds8CPU/54intrinsicmaster',identity=identity,
        build=build,work=report,source_local_cpu=dict(local_work),dma_job_sizes=dict(Counter(job_sizes.values())),
        limitation='No DMA service/alignment, refresh, NMI/audio/SPC or carried phase prediction')
    validation=dict(schema=1,accepted=True,identity=identity,exact_instructions=len(ni),exact_cpu_writes=len(nw),
        non_dma_intrinsic_intervals_checked=intrinsic_checked,observed_dma_service_intervals=len(ni)-intrinsic_checked,
        exact_dma_bytes=len(cdma),exact_native_scratch_matches=4,intervals=matches,previous_invariance=invariance,
        limitation='Typed leaf entry diagnostics; native snapshots/clocks never producer inputs')
    ...
    print(f'PASS: {len(ni)} composed instruction states/CPU durations; {len(nw)} CPU write positions; {len(cdma)} DMA bytes; all four550560CPU intervals')
