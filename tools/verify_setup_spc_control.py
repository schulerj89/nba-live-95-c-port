...
def run(exe,out,name,data):
 ...
 r=subprocess.run([str(exe.resolve()),str(src),str(dst)],capture_output=True);check(type(r.returncode)is int and r.returncode==0,'fresh probe integer exit status');b=dst.read_bytes();check(len(b)==65577,'output length');return b
def main(a):
 ...
 for i,pc,value in ((1,0x384,0x30),(2,0x3ec,1)):
  ...
  for s in (before,after):check(number(s,'source_pc',0,65535)==pc and number(s,'value')==value,'source publication identity')
