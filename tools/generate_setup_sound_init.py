"""Statically translate the bounded original sound initializer source to C continuations.
...
def in_scope(pc):
    return any(a<=pc<=b for a,b in [(0x9b73,0x9bb2),(0x9bd5,0x9bf1),(0xa4fc,0xa508),(0xaacd,0xaacd)])
...
def translate(rom, decoder):
    jumps = {}
    todo, decoded, edges, returns = deque([(0x9b73,0,0)]), {}, {}, set()
    while todo:
        state = todo.popleft()
        ...
        if pc == 0x9b84: post=(0,0) # entry PHP at 9B73
        if pc == 0x9bea: post=(1,1) # 9BD5 entered after 9B99 SEP #10
        next_state=(pc+ins.length,*post)
        successors=[]
        if pc==0xaacd: successors=[] # unresolved SPC read; no response consumed
        elif ins.mnem in ('RTL','RTS'): pass
        elif ins.mnem in ('JSR','JSL'):
            target=ins.operand&65535
            post_call=(1,1) if target == 0x9bd5 else (m,x)
            ...
            if ins.mode!=decoder.ABS:raise ValueError('unexpected indirect sound_init jump')
            successors=[(ins.operand,*post)]
        else:successors=[next_state]
        edges[state]=successors
        todo.extend(s for s in successors if in_scope(s[0]))

    def go(state): return 'goto ' + label(state) + ';' if state in decoded else f'return boundary(s, 0x80{state[0]:04x}u, NBA_SOUND_INIT_UNIMPLEMENTED);'
    def address(ins):
        operand = ins.operand
    ...
    lines = ['/* Generated from canonical ROM only by tools/generate_setup_sound_init_work.py.',
             ...
             'static bool advance(NbaSetupSoundInit *s)', '{', '    switch (B.resume) {',
             '    case 0: goto source_9b73_m0x0;']
    for state, ins in sorted(decoded.items()):
        ...
        body = []
        if pc == 0xaacd:
            body = ['READ(0xaacd, 3, DB(0x2140u), 1, false);',
                    'return boundary(s, 0x80aacdu, NBA_SOUND_INIT_UNIMPLEMENTED);']; nxt = None
        elif pc == 0xa1be:
            body = ['return boundary(s, 0x80a1beu, NBA_SOUND_INIT_RETURN);']; nxt = None
        ...
        if ins.mnem=='PLP' and pc == 0x9b84: nxt=(pc+1,0,0)
        if ins.mnem=='PLP' and pc == 0x9bea: nxt=(pc+1,1,1)
