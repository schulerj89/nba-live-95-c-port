"""Emit native OOB parent reference semantics; never the behavioral oracle."""
import argparse
import sys
...
def main():
    ...
    if len(rom) % 0x8000 == 512:
        rom = rom[512:]
    output.write_text(emit_bank(rom, 0x87, [
        BankEntry('ViolationOobParent', 0x92A5, 0x94A2, 0, 0)]), encoding='utf-8')
    print(output)
