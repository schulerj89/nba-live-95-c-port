...
cl /nologo /W4 /WX /O2 /MD /utf-8 /I "$root\include" /Fe"$output\setup_sound_prefix_probe.exe" /Fo"$output\\" "$root\tools\setup_sound_prefix_probe.c" "$root\src\nba_setup_sound_prefix.c"
exit /b %ERRORLEVEL%
"@
...
$files = @('include/nba_setup_codec_work.h', 'src/nba_setup_sound_prefix_program.inc', 'tools/generate_setup_sound_prefix.py', 'include/nba_setup_sound_prefix.h', 'src/nba_setup_sound_prefix.c', 'tools/setup_sound_prefix_probe.c', 'tools/build_setup_sound_prefix_probe.ps1')
...
$exe = Join-Path $output 'setup_sound_prefix_probe.exe'
