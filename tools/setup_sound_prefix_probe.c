...
static bool run(Bus *bus, NbaSetupSoundPrefix *s, const NbaCodecWorkEntry *entry, FILE *trace,
                uint64_t limit, uint64_t *cpu, uint64_t *master)
{
    NbaCodecBusCycle c; uint64_t prior = 0;
    *cpu = *master = 0;
    if (!nba_setup_sound_prefix_begin(s, entry, limit)) return false;
    while (nba_setup_sound_prefix_peek(s, &c)) {
        ...
        if (s->stop == NBA_SOUND_PREFIX_SPC_RESPONSE) break;
        ...
        if (!nba_setup_sound_prefix_accept(s,c.kind==NBA_CODEC_READ?read_bus(bus,c.address):0)) return false;
    }
    return bus->valid;
}
...
{
    NbaSetupSoundPrefix s, saved; NbaCodecBusCycle pending; uint64_t cpu,master,previous=0; unsigned channel;
    NbaCodecWorkEntry entry={0x5500,0x80,5,0x1fe0,0x80,0x13};
    for(channel=0;channel<8;++channel) {
        memset(bus->wram,0,sizeof(bus->wram));bus->valid=true;
        bus->wram[0x5a]=0x82;bus->wram[0x7cd]=bus->wram[0x7ce]=bus->wram[0x7cf]=255;
        bus->wram[0x73a+channel]=10;bus->wram[0x742+channel]=9;
        bus->wram[0x78a+channel*2]=0xb2;bus->wram[0x78b+channel*2]=0x0c;
        bus->wram[0x7ba+channel*2]=bus->wram[0x7bb+channel*2]=255;
        if(!run(bus,&s,&entry,NULL,10000,&cpu,&master)||s.stop!=NBA_SOUND_PREFIX_SPC_RESPONSE||s.boundary_pc!=0x80aae6||s.bus.registers.data_bank!=0x82||s.bus.registers.stream_cursor!=channel||bus->wram[0x7ba+channel*2]!=0||bus->wram[0x7bb+channel*2]!=0) return 1;
        if(channel&&cpu-previous!=35) return 2; previous=cpu;
        saved=s;
        if(!nba_setup_sound_prefix_peek(&s,&pending)||pending.address!=0x822140||!pending.instruction_end) return 3;
        if(nba_setup_sound_prefix_accept(&s,0)||nba_setup_sound_prefix_accept(&s,255)||memcmp(&s,&saved,sizeof(s))) return 4;
    }
    {
        const uint8_t banks[]={0,0x80,0x82,0xbf}; unsigned i;
        for(i=0;i<sizeof(banks);++i) {
            bus->wram[0x5a]=banks[i];bus->valid=true;
            if(!run(bus,&s,&entry,NULL,10000,&cpu,&master)||s.stop!=NBA_SOUND_PREFIX_SPC_RESPONSE||
               !nba_setup_sound_prefix_peek(&s,&pending)||pending.address!=((uint32_t)banks[i]<<16|0x2140u))return 9;
        }
        bus->wram[0x5a]=0x7e;bus->valid=true;
        if(!run(bus,&s,&entry,NULL,10000,&cpu,&master)||s.stop!=NBA_SOUND_PREFIX_UNIMPLEMENTED||s.boundary_pc!=0x80aae6)return 10;
    }
    bus->wram[0x5a]=0x82;bus->valid=true;
    if(!run(bus,&s,&entry,NULL,5,&cpu,&master)||s.bus.status!=NBA_CODEC_WORK_LIMIT||s.bus.instructions!=5)return 11;
    entry.status=0x1b;if(nba_setup_sound_prefix_begin(&s,&entry,1000))return 5;entry.status=0x13;
    if(nba_setup_sound_prefix_begin(NULL,&entry,1000)||nba_setup_sound_prefix_begin(&s,NULL,1000)||nba_setup_sound_prefix_begin(&s,&entry,0))return 6;
    memset(bus->wram,0,sizeof(bus->wram));bus->wram[0x53]=1;bus->valid=true;
    if(!run(bus,&s,&entry,NULL,10000,&cpu,&master)||s.stop!=NBA_SOUND_PREFIX_RETURN||s.boundary_pc!=0x80a1be)return 7;
    bus->wram[0x53]=0;bus->wram[0x637]=1;
    if(!run(bus,&s,&entry,NULL,10000,&cpu,&master)||s.stop!=NBA_SOUND_PREFIX_UNIMPLEMENTED||s.boundary_pc!=0x809fb6)return 8;
    puts("PASS: 8 channel prefixes, immutable unresolved SPC read, mirrors/WRAM distinction, instruction limit, return/stream branches, invalid entry contracts"); return 0;
}

int main(int argc,char **argv)
{
    static Bus bus; NbaSetupSoundPrefix s;NbaCodecWorkEntry entry;FILE *trace,*out;uint64_t cpu,master;
