...
local selection=assert(tonumber(os.getenv('NBA95_PASS_INIT_SELECTION')))
local stop_at=assert(tonumber(os.getenv('NBA95_PASS_INIT_FRAMES')))
...
hook(0x86ac50,function()
 if active=='child'then
  check(prefix,'missing prefix');snap(geometry and'init.revisit'or'init.ready',0x86ac50,not geometry);geometry=true
 end
end)
hook(0x84e09c,function()
 if active then
  check(not metric and not cancel and(active=='selection'or(active=='child'and prefix and geometry)),'bad pass resume')
