...
def verify(capture,probe,rom,diagnostics,assets=DEFAULT_ASSETS):
 ...
 for row in rows:
  ...
  if tag.startswith('pose.'):
   ...
   # AEC3/B832 and caller ADC/SBC assume binary arithmetic; no native CLD.
   # D=0 bounds this C translation and remains required through the route.
   need((row['cpu_ps']&0x38)==0,'pose requires native 16-bit A/X and decimal mode off')
   if tag=='pose.entry':
