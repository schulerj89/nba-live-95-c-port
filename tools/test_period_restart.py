...
def main():
    ...
    raw=a.rom.read_bytes();tables=[[struct.unpack_from('<hhHH',raw,base+i*8)[:3]for i in range(5)]for base in (0x506a,0x5092,0x50ba)]
    # Explicit source branch destinations from DDFD..DEA4, independent of C's
    # boolean expressions: (first table, second table, mirror both).
    choices={(1,0):(1,2,False),(1,5):(2,1,True),(2,0):(1,2,True),(2,5):(2,1,False),(3,0):(2,1,False),(3,5):(1,2,True)}
        ...
        for tip in (0,5):
            for anchor in (-336,336):
                ...
                for actor in range(10):
                    ...
                    if 0<period<4:
                        first,second,flip=choices[(period,tip)];x,y,d=tables[second if side else first][pair]
                        if flip:x,y,d=-x,-y,(d+4)%8
                    else:
                        x,y,d=tables[0][pair]
                        if anchor>=0:x,d=-x,(d+4)%8
                        if side:x,y,d=-x,-y,(d+4)%8
