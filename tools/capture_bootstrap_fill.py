"""Observe unpatched NBA95 normal cold power-on through first reset fill return80C0."""
...
def main():
    ...
    script=out/'capture.lua';shutil.copyfile(Path(__file__).with_name('mesen_bootstrap_fill.lua'),script);runner=out/'runner.py';shutil.copyfile(__file__,runner)
    args=[str(exe),'--testrunner','--timeout=180',str(a.rom.resolve()),str(script)]
    m={'schema':1,'kind':'normal reset first DMA fill observation','state_injection':False,'rom_patch':False,'inputs':False,'accepted':False,'sources':{},'settings':settings,'initial_saves':[],'arguments':args}
    ...
    try:
        ...
        if (out/'complete.txt').read_text()!='ok; normal power-on through first reset fill return80C0\n':raise ValueError('incomplete capture')
