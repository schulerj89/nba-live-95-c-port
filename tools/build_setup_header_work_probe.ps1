...
cl /nologo /W4 /WX /O2 /MD /utf-8 /I "$root\include" /Fe"$output\setup_header_work_probe.exe" /Fo"$output\\" "$root\tools\setup_header_work_probe.c" "$root\src\nba_setup_header_work.c"
exit /b %ERRORLEVEL%
"@
...
$files = @('include/nba_setup_codec_work.h', 'src/nba_setup_header_program.inc', 'tools/generate_setup_header_work.py', 'include/nba_setup_header_work.h', 'src/nba_setup_header_work.c', 'tools/setup_header_work_probe.c', 'tools/build_setup_header_work_probe.ps1')
...
$exe = Join-Path $output 'setup_header_work_probe.exe'
