...
$script=(Resolve-Path -LiteralPath (Join-Path $PSScriptRoot 'mesen_cpu_mode_one_role.lua')).Path
$variables=@{
    NBA95_CAPTURE_DIR=$capture; NBA95_TOOL_DIR=$PSScriptRoot;
    NBA95_VEC_ENTRY='86F1B0'; NBA95_VEC_EXITS='86F235,86F23E';
    NBA95_VEC_READS='0000-4AFF'; NBA95_VEC_WRITES='0000-4AFF';
    NBA95_VEC_LABEL='cpu_mode_one_role'; NBA95_VEC_MAX='2';
...
if($process.ExitCode-ne 0-or!(Test-Path -LiteralPath $sentinel)){
    throw "Native CPU mode-one role capture failed: exit=$($process.ExitCode)"
}
$complete=(Get-Content -Raw -LiteralPath $sentinel).Trim()
if($complete-ne 'label=cpu_mode_one_role vectors=2 orphan_exits=0 shared_exit_callbacks=0'){
    throw "Incomplete CPU mode-one role capture: $complete"
}
$complete
