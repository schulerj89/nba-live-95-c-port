"""Strict first-NMI pre-OAM checkpoint. Does not accept full S1/03DB.
...
from bootstrap_oam_prefix_trace_protocol import validate as validate_trace
...
SCRIPT_SHA='e515922adcfe4bd415803704b452ddda87bb18bec68d1f0f882e7ffd5b2bd4fa'
RUNNER_SHA='04a88fc65af0e9a3f08761dd0a7498eb53603660de5174a36723bdef22bae9a0'
DECODER_SHA='b1864664d3ac0abcd439055e88bf7220cf66be7a6ae9dfc5d59b3186f1469a46'
SOURCES=set(['include/nba_bootstrap.h', 'include/nba_bootstrap_fill.h', 'include/nba_bootstrap_internal.h', 'include/nba_bootstrap_nmi.h', 'include/nba_bootstrap_oam_prefix.h', 'include/nba_rom.h', 'include/nba_setup_codec_work.h', 'include/nba_setup_spc_control.h', 'include/nba_setup_spc_init.h', 'include/nba_setup_spc_resident.h', 'include/nba_types.h', 'src/nba_bootstrap.c', 'src/nba_bootstrap_cpu.c', 'src/nba_bootstrap_cpu_program.inc', 'src/nba_bootstrap_ipl.c', 'src/nba_bootstrap_nmi.c', 'src/nba_bootstrap_oam_prefix.c', 'src/nba_bootstrap_nmi_cpu.c', 'src/nba_bootstrap_nmi_cpu_program.inc', 'src/nba_bootstrap_rom.c', 'src/nba_rom.c', 'src/nba_setup_spc_control.c', 'src/nba_setup_spc_init.c', 'tools/bootstrap_oam_prefix_probe.c', 'tools/build_bootstrap_oam_prefix_probe.ps1', 'tools/generate_bootstrap_cpu.py', 'tools/generate_bootstrap_nmi_cpu.py'])
STREAMS=('cpu','spc','cpu_bus','spc_bus','boundaries')
TAGS=('power_entry', 'cpu_80bc', 'cpu_dma_request', 'resident_entry', 'post_f1', 'cpu_after_dma', 'cpu_fill_return', 'cpu_clear_after', 'cpu_table_after', 'cpu_da72_return', 'cpu_ab7e_return', 'cpu_nmi_enable', 'cpu_sound_entry', 'cpu_sound_guard_before', 'cpu_sound_guard_after', 'cpu_sound_poll', 'nmi_vector_entry', 'cpu_oam_request', 'cpu_oam_written', 'cpu_ppu_status_request')
...
def native(root,rom):
    ...
    check(m['kind']=='normal reset first NMI post-OAM-write pre-PPU-status observation','native kind')
    ...
    check((root/'complete.txt').read_text()=='ok; normal power-on through post-OAM-write pre-PPU-status818E\n','completion')
...
def bind_native_boundaries(root,data):
    ...
    known_cpu={'power_entry':0x800d,'cpu_80bc':0x8080bc,'cpu_dma_request':0x808a8d,'cpu_after_dma':0x808a92,'cpu_fill_return':0x8080c0,'cpu_clear_after':0x8080ca,'cpu_table_after':0x8080fd,'cpu_da72_return':0x808101,'cpu_ab7e_return':0x808141,'cpu_nmi_enable':0x808145,'cpu_sound_entry':0x809b73,'cpu_sound_guard_before':0x809bed,'cpu_sound_guard_after':0x809bef,'cpu_sound_poll':0x80aacd,'nmi_vector_entry':0x8156,'cpu_oam_request':0x808184,'cpu_oam_written':0x808188,'cpu_ppu_status_request':0x80818e}
...
def main():
    ...
    check(summary['status']==1 and summary['boundary_pc']==0x80818e and summary['upload']==1264 and summary['resident']and summary['f1'],'actual bounded stop')
    ...
    for t in trace:
        check(integer(t['kind'],0,10)and t['kind']!=4,'C event kind')
        ...
        if kind==9:
            check(end_cpu and current is not None and t['pc']==current['pc'] and t['bus'] in (0,1,2),'NMI bus association')
            if t['bus']==2:check(t['address']==0 and t['value']==0,'NMI canonical idle')
        elif kind==10:
            check(end_cpu and (t['pc'],t['address'],t['value'],t['bus'],t['end'])==(0x8156,0,0,2,False),'NMI vector marker')
        elif kind==0:
    ...
    check(end_cpu,'final complete CPU instruction')
    protocol,expected_vram,control_states=validate_trace(trace,summary,raw_rom,decoder)
    ...
    for t in trace:
        ...
        if t['kind']in(0,9):
            cpu_count+=1
            if t['bus']==2 or(t['kind']==0 and t['bus']==0 and current['pc']<=t['address']<fetch_end):continue
    ...
    check((out/'final.wram').read_bytes()==(root/'cpu_ppu_status_request.wram').read_bytes(),'full CPU-boundary WRAM')
    for tag in ('cpu_oam_request','cpu_oam_written','cpu_fill_return','cpu_clear_after','cpu_table_after','cpu_da72_return','cpu_ab7e_return','cpu_nmi_enable','cpu_sound_entry','cpu_sound_guard_before','cpu_sound_guard_after','cpu_sound_poll','nmi_vector_entry'):
        check((out/(tag+'.wram')).read_bytes()==(root/(tag+'.wram')).read_bytes(),'full intermediate WRAM '+tag)
    ...
    check((out/'final.vram').read_bytes()==expected_vram==(root/'cpu_ppu_status_request.vram').read_bytes(),'source and native full first-fill VRAM')
    final_keys=set('masterClock cpu.cycleCount cpu.a cpu.x cpu.y cpu.sp cpu.ps cpu.k cpu.pc cpu.dbr cpu.d cpu.emulationMode ppu.vramAddress ppu.vramReadBuffer dmaController.channel[1].srcAddress dmaController.channel[1].srcBank dmaController.channel[1].transferSize dmaController.channel[1].transferMode dmaController.channel[1].fixedTransfer dmaController.channel[1].invertDirection dmaController.channel[1].decrement dmaController.channel[1].dmaActive ppu.enableOamPriority ppu.oamRamAddress ppu.internalOamAddress'.split())
    final_c=state(out/'final.state');final_n=state(root/'cpu_ppu_status_request.state');check(set(final_c)==final_keys,'final typed field closure')
    for k in final_keys:check(final_c[k]==final_n[k],'final typed field '+k)
    projections={'cpu_80bc':0x8080bc,'cpu_fill_return':0x8080c0,'cpu_clear_after':0x8080ca,'cpu_table_after':0x8080fd,'cpu_da72_return':0x808101,'cpu_ab7e_return':0x808141,'cpu_nmi_enable':0x808145,'cpu_sound_entry':0x809b73,'cpu_sound_guard_before':0x809bed,'cpu_sound_guard_after':0x809bef,'cpu_sound_poll':0x80aacd,'nmi_vector_entry':0x8156,'cpu_oam_request':0x808184,'cpu_oam_written':0x808188,'final':0x80818e}
    for name,pc in projections.items():
        cs=state(out/(name+'.nmi'));nsnapshot=state(root/(('cpu_ppu_status_request'if name=='final'else name)+'.state'))
        expected=control_states[pc]
        check(set(cs)==set(expected),'NMI control state closure')
        for k,v in expected.items():
            want=str(v).lower() if type(v)is bool else str(v)
            check(cs[k]==want and nsnapshot[k]==want,'source/native NMI field '+name+'.'+k)
    report={'result':'PASS bounded normal reset/upload/F1/firstDMA/reset tables/first NMI OAM-write prefix; full S1/03DB remains OPEN','cpu_instruction_states':len(ccpu),'cpu_and_DMA_data_accesses':len(cbus),'cpu_data_accesses':len(cbus)-protocol['DMA_operations'],'spc_instruction_states':len(ss),'spc_write_io_accesses':len(ns),'scalar_fields':2*len(scalar_keys),'wram_bytes':15*131072,'NMI_control_fields':15*12,'aram_bytes':131072,'vram_bytes':65536,'final_typed_fields':len(final_keys),'profile':'pinned software NTSC zero RAM +40Hz SPC','SPC_master_callback_parity':False,'excluded':'SPC DSP RAM reads/full DSP state, PPU status213E read and OAM data/DMA onward, RTI, serial controller effects, otherDMA/HDMA and03DB','executable_sha256':sha(exe),'native_manifest_sha256':sha(root/'manifest.json'),'trace_sha256':sha(out/'events.jsonl'),'summary':summary,'protocol_v2':protocol,'native_boundary_bindings':len(n['boundaries']),'verifier_sources':{p.name:sha(p) for p in (Path(__file__),Path(__file__).with_name('bootstrap_boundary_protocol_v2.py'),Path(__file__).with_name('bootstrap_oam_prefix_trace_protocol.py'),Path(__file__).with_name('bootstrap_nmi_profile.py'),Path(__file__).with_name('bootstrap_trace_protocol_v2.py'))},'build':bm}
