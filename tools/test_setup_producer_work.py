...
import verify_setup_producer_work as verify
...
class IntegrityTests(unittest.TestCase):
    ...
    def test_capture_source_revisions_are_pinned(self):
        for name in ('script', 'base_script', 'runner', 'codec_script'):
            with self.subTest(name=name):
    ...
    def test_missing_trace_script_runner_or_snapshot_identity(self):
        for name in ('scheduler.jsonl', 'codec_instructions.jsonl', 'codec_writes.jsonl',
                     ...
                     'scheduler_base.lua', 'codec_base.lua', 'producer_instructions.jsonl', 'producer_bus.jsonl', 'producer_boundaries.jsonl', 'producer_04_exit.wram', 'codec_20_exit.wram', 'header_01_before_wait.wram'):
            with self.subTest(name=name):
    ...
    def test_source_continuation_self_tests(self):
        ...
        self.assertEqual(p.stdout, 'PASS: 11 producer continuation contract cases\n')
