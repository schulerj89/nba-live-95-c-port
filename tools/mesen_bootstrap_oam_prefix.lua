...
local out=assert(os.getenv('NBA95_BOOTSTRAP_OAM_DIR'))
local files={};local counts={};local cpu_pc=0;local spc_pc=0;local done=false;local poll_seen=false
...
local function snapshot(tag)
 ...
 for _,spec in ipairs({{'aram',emu.memType.spcRam,65536},{'wram',emu.memType.snesWorkRam,131072},{'vram',emu.memType.snesVideoRam,65536}})do
...
local function finish()
 ...
 local f=assert(io.open(out..'/complete.txt','wb'));f:write('ok; normal power-on through post-OAM-write pre-PPU-status818E\n');f:close();emu.stop(0)
end
emu.addMemoryCallback(function(pc)
 ...
 if pc==0x808a92 then snapshot('cpu_after_dma')end
 if pc==0x8080c0 then snapshot('cpu_fill_return')end
 if pc==0x8080ca then snapshot('cpu_clear_after')end
 if pc==0x8080fd then snapshot('cpu_table_after')end
 if pc==0x808101 then snapshot('cpu_da72_return')end
 if pc==0x808141 then snapshot('cpu_ab7e_return')end
 if pc==0x808145 then snapshot('cpu_nmi_enable')end
 if pc==0x809b73 then snapshot('cpu_sound_entry')end
 if pc==0x809bed then snapshot('cpu_sound_guard_before')end
 if pc==0x809bef then snapshot('cpu_sound_guard_after')end
 if pc==0x80aacd and not poll_seen then poll_seen=true;snapshot('cpu_sound_poll')end
 if pc==0x8156 then snapshot('nmi_vector_entry')end
 if pc==0x808184 then snapshot('cpu_oam_request')end
 if pc==0x808188 then snapshot('cpu_oam_written')end
 if pc==0x80818e then snapshot('cpu_ppu_status_request');finish();return end
 if pc==0x808a8d then
  snapshot('cpu_dma_request')
 end
 assert(counts.cpu<100000,'CPU bound')
...
emu.addMemoryCallback(function(pc)
 ...
 if pc==0x387 then snapshot('post_f1')end
 assert(counts.spc<100000,'SPC bound')
