"""Compare AD0E early gates, AE10 choice, F473 lane and AED9 upper installation; continuations explicit."""
...
SOURCE_VERSION={'capture':'e80d7e0e8901182b9932e7219a4aed67617b873c49deabe231d833f26012b2f7',
 'runner':'8feac2b09f398b7735c8345ebc13e557bcdbfbfef8f5a5f7ba0a5af968c5245b',
...
def attest(capture,rom):
 ...
 need(integer(m.get('schema'),1,1)and m.get('kind')=='native human pass aligned boundaries','wrong capture schema')
 ...
 private={'mesen':'portable-mesen/Mesen.exe','capture':'capture.lua','runner':'capture_human_pass_aligned.py','isolation_helper':'mesen_portable.py'}
 ...
 need(m.get('environment')=={'NBA95_CAPTURE_DIR':capture.as_posix(),'NBA95_PASS_ALIGNED_SELECTION':str(m['selection']),
                            'NBA95_PASS_ALIGNED_FRAMES':str(m['requested_frames'])},'changed executed environment')
 ...
 required={'boundaries.jsonl','capture.lua','capture_complete.txt','capture_human_pass_aligned.py','mesen_portable.py',
 ...
 for row in rows:
  ...
  if tag in('pass.initialize','pass.no_receiver'):
   ...
   if route:
    ...
    append('chain',entry,row,init_expected(end,'chain',0));stages['chain']+=1
   continue
  if tag.startswith('init.'):
   ...
   if tag=='init.cancel.entry':
    ...
    append('cancel',pending['cancel_entry'],row,init_expected(raw(capture,row),'cancel'));stages['cancel']+=1
    pending['init_step']='prefix'
   elif tag=='init.prefix':
    need(step=='prefix','misordered prefix')
    append('prefix',pending['init_entry'],row,init_expected(raw(capture,row),'prefix'));stages['prefix']+=1
    ...
    append('fine',pending['fine_entry'],row,dict(distance=word(end,0xaa),fine=word(end,0xb2)));stages['fine']+=1
    pending['init_step']='ready'
   elif tag=='init.ready':
    need(step=='ready','misordered first AC50');end=raw(capture,row)
    fine=raw(capture,pending['fine_entry'])
    append('geometry',pending['prefix_entry'],row,init_expected(end,'geometry',fine=fine));stages['geometry']+=1
    append('chain',pending['entry'],row,init_expected(end,'chain',fine=fine));stages['chain']+=1
  ...
  if tag.startswith('action.'):
   ...
   if tag in('action.offaxis','action.normal','action.boost'):
    ...
    append('observed_gate',pending['action_entry'],row,action_expected(pending['action_entry'],raw(capture,row),route))
    ...
    append('observed_upper',pending['upper_entry'],row,action_expected(pending['upper_entry'],raw(capture,row),3));action_stages['upper']+=1
    ...
    append('observed_ground',pending['ground_entry'],row,action_expected(pending['ground_entry'],raw(capture,row),3));action_stages['ground']+=1
    ...
    append('observed_gate',pending['action_entry'],row,action_expected(pending['action_entry'],raw(capture,row),3));action_stages['gate']+=1;action_routes[3]+=1
  ...
  if tag.startswith('aligned.'):
   ...
   if tag=='aligned.entry':
    ...
    append('lane',pending['lane_entry'],row,aligned_expected(pending['lane_entry'],end,'lane'))
    ...
    append('choice',pending['choice_entry'],row,aligned_expected(pending['choice_entry'],end,'choice'))
    ...
    append('upper',pending['aligned_upper_entry'],row,aligned_expected(pending['aligned_upper_entry'],raw(capture,row),'upper'))
    ...
    if tag!='aligned.catch':append('install',pending['install_entry'],row,aligned_expected(pending['install_entry'],end,'install',route))
    append('gate',pending['aligned_entry'],row,aligned_expected(pending['aligned_entry'],end,'gate',route))
  ...
  if tag=='pass.resume':
   ...
   stop=pending.get('aligned_stop',pending['action_stop'])
