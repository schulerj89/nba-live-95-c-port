...
#include "../build/fb30-audit-v1/source/tools/setup_fb30_work_probe.c"
#undef main
...
int main(int argc, char **argv)
{
    ...
    bus.valid = true;
    if (!nba_setup_fb30_work_begin(&work, &entry, 1000000)) return 2;
    while (nba_setup_fb30_work_peek(&work, &cycle)) {
        uint8_t value = 0;
    ...
    if (work.bus.status != NBA_CODEC_WORK_DONE) return 3;
    printf("{\"a\":%u,\"x\":%u,\"y\":%u,\"sp\":%u,\"db\":%u,\"ps\":%u}\n",
           work.bus.registers.value, work.bus.registers.symbol, work.bus.registers.stream_cursor,
           work.bus.registers.stack_pointer, work.bus.registers.data_bank, work.bus.registers.status);
    return 0;
}
