...
def main():
 ...
 entries={0x85:[('SignedMultiply',0xf78b,0xf821),('UnsignedDivide',0xf867,0xf8d9),('SignedDivide',0xf8d9,0xf929)],0x86:[('Normalize',0x9846,0x986d),('Launch',0x99c4,0x9c6f)]}
 script=Path(__file__).parent/'ghidra/DumpHumanPassLaunch.java'
 ...
 for bank,items in entries.items():
  ...
  (out/f'human_pass_launch_bank{bank:02x}.c').write_text(emit_bank(rom,bank,[BankEntry(n,s,e,0,0)for n,s,e in items]))
  ...
  if run.returncode or not(out/f'human_pass_launch_bank{bank:02x}.txt').is_file():raise RuntimeError('Ghidra failed; retain output')
 for name,address,size in [('PassLaunchTables',0x869c6f,108)]:
