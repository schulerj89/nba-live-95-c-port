param([Parameter(Mandatory=$true)][string]$OutputDir,
      [ValidateSet('core','wrapper','stream')][string]$Kind='stream',
      [switch]$Controlled,[int]$Frames=3000)
...
$p=Start-Process -FilePath (Get-Command Mesen.exe).Source -ArgumentList @('--testrunner','--timeout=300',
    '"F:/Games/SNES/NBA Live 95 (USA).sfc"',('"'+$PSScriptRoot+'/mesen_camera_presentation.lua"')) -PassThru -WindowStyle Hidden
$p.WaitForExit()
if($p.ExitCode -ne 0 -or !(Test-Path "$OutputDir/capture_complete.txt")){throw 'Camera ROM capture failed'}
Get-Content "$OutputDir/camera_progress.txt"
