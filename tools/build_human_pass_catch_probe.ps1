...
$privateBuild=Join-Path $root 'build\human-pass-catch'
...
$batch=Join-Path $privateBuild 'compile_human_pass_catch.bat'
$command=@"
@echo off
call "$vcvars" >nul
cl /nologo /W4 /WX /O2 /MD /utf-8 /I "$root\include" /c /Fo"$privateBuild\nba_human_pass_catch.obj" "$root\src\nba_human_pass_catch.c"
if errorlevel 1 exit /b %ERRORLEVEL%
cl /nologo /W4 /WX /O2 /MD /utf-8 /I "$root\include" /Fe"$privateBuild\human_pass_catch_probe.exe" /Fo"$privateBuild\human_pass_catch_probe.obj" "$root\tools\human_pass_catch_probe.c" "$privateBuild\nba_human_pass_catch.obj" $(($objects|ForEach-Object{'"'+$_+'"'})-join ' ') user32.lib gdi32.lib winmm.lib
exit /b %ERRORLEVEL%
"@
