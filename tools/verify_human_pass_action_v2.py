"""Compare AC50 side/back gate and complete B00B child; earlier/later AB2D observed only."""
...
SOURCE_VERSION={'capture':'d80d13e59f07b29f7d4ebb1a16817cf6ee3edfd01c30bf58304727a2a8bc0b5b',
 'runner':'eac7abb0b9bbe59c0cfe04ff62c71fa2f3db0da6a47e75bfd1d2980f1952f53e',
...
def attest(capture,rom):
 ...
 need(integer(m.get('schema'),1,1)and m.get('kind')=='native human pass action boundaries','wrong capture schema')
 ...
 private={'mesen':'portable-mesen/Mesen.exe','capture':'capture.lua','runner':'capture_human_pass_action.py','isolation_helper':'mesen_portable.py'}
 ...
 need(m.get('environment')=={'NBA95_CAPTURE_DIR':capture.as_posix(),'NBA95_PASS_ACTION_SELECTION':str(m['selection']),
                            'NBA95_PASS_ACTION_FRAMES':str(m['requested_frames'])},'changed executed environment')
 ...
 required={'boundaries.jsonl','capture.lua','capture_complete.txt','capture_human_pass_action.py','mesen_portable.py',
...
def verify(capture,probe,rom,diagnostics,assets=DEFAULT_ASSETS):
 ...
 def append(mode,entry,end,want):
  ...
  if mode not in('gate','ground','upper'):return
 ...
 for row in rows:
  ...
  if tag.startswith('action.'):
   ...
   if tag in('action.offaxis','action.normal','action.boost'):
    ...
    append('gate',pending['action_entry'],row,action_expected(pending['action_entry'],raw(capture,row),route))
    ...
    append('upper',pending['upper_entry'],row,action_expected(pending['upper_entry'],raw(capture,row),3));action_stages['upper']+=1
    ...
    append('ground',pending['ground_entry'],row,action_expected(pending['ground_entry'],raw(capture,row),3));action_stages['ground']+=1
    ...
    append('gate',pending['action_entry'],row,action_expected(pending['action_entry'],raw(capture,row),3));action_stages['gate']+=1;action_routes[3]+=1
  ...
  if tag=='pass.resume':
   ...
   child_boundaries.append(dict(entry=pending['action_stop']['index'],pc=pending['action_stop']['pc'],exit=row['index'],replayed=False))
 ...
 return dict(kind='AC50 side/back gate and B00B/B47A child; other continuations unexecuted',passed=not failures,
  manifest_sha256=sha(capture/'manifest.json'),probe_sha256=sha(probe),rom_sha256=sha(rom),verifier_sha256=sha(__file__),
  calls=native_calls,action_stages=dict(action_stages),action_routes=dict(action_routes),native_upper_requests=upper_requests,native_ground_calls=ground_calls,assets_sha256=sha(assets),probe_arguments=[str(probe),str(assets),str(rom)],earlier_initializer_stages_observed_not_replayed=dict(stages),earlier_cancel_locks_observed=dict(cancel_locks),earlier_bands_observed=dict(bands),unreplayed_AC50_revisits=revisits,earlier_metric_calls_observed=sum(metric_exits.values()),earlier_metric_exits_observed=dict(metric_exits),compared_values=compared,
