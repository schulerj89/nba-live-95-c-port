...
# Versioned read-only capture route; update only with new independently audited evidence.
...
def main():
    ...
    identity = {name: dict(path=str(path), sha256=digest(path)) for name, path in {
        ...
        'verifier':Path(__file__), 'trace_contract':Path(__file__).with_name('setup_codec_trace_contract.py'), 'strict_scheduler_verifier':Path(__file__).with_name('verify_setup_scheduler.py')}.items()}
