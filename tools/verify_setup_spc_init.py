...
def state(p):return dict(line.split('=',1)for line in p.read_text().splitlines())
def subset(g,w):
 for k,v in w.items():
  if isinstance(v,dict):subset(g[k],v)
  else:check(type(g[k])is type(v)and g[k]==v,'persisted settings')
def native(p,rom):
 m=json.loads((p/'manifest.json').read_text());check(type(m['schema'])is int and m['schema']==1 and m['accepted']is True and m['state_injection']is False and m['rom_patch']is False and type(m['exit_code'])is int and m['exit_code']==0 and m['initial_saves']==[],'capture contract')
 ...
 check(sha(rom)==ROM_SHA,'ROM identity');initial=json.loads((p/'initial-settings.json').read_text());check(m['settings']==initial,'manifest settings')
 payload=rom.read_bytes()[0x4687:0x4b77]
 for tag in ('entry','post_control','dsp_entry','pending_dsp'):check((p/f'spc_init_{tag}.aram').read_bytes()[0x380:0x870]==payload,'native ROM to ARAM source identity')
 ...
 check(sha(p/'portable-mesen/settings.json')==m['post_settings_sha256'],'post settings identity');subset(json.loads((p/'portable-mesen/settings.json').read_text(encoding='utf-8-sig')),initial)
 ...
 return m,ni,nw,nr
def build(exe):
 m=json.loads((exe.parent/'build-manifest.json').read_text(encoding='utf-8-sig'));check(type(m['schema'])is int and m['schema']==1 and type(m['compiler_exit'])is int and m['compiler_exit']==0 and set(m['sources'])==BUILD,'build closure')
...
def probe(exe,out,name,data):
 ...
 r=subprocess.run([str(exe.resolve()),str(src),str(ip),str(wp),str(ep)],capture_output=True,text=True);check(r.returncode==0,'probe failed '+r.stderr)
 result=json.loads(r.stdout);check(set(result)=={'pc','phase','cycles','instructions','writes','control'}and type(result['control'])is bool and all(type(v)is int and 0<=v<=2**63-1 for k,v in result.items()if k!='control'),'probe numeric schema')
...
def main(a):
 ...
 report={'passed':True,'scope':'isolated 0380/control and post-control RAM-clear slices; no normal hardware initialization','source_states':len(pcs),'native_witnessed_states':len({n[0]for n in ni}),'cases':results,'native_manifest_sha256':sha(a.native/'manifest.json'),'verifier_sha256':sha(__file__),'build_manifest_sha256':sha(a.exe.parent/'build-manifest.json')};(out/'report.json').write_text(json.dumps(report,indent=2)+'\n');print('PASS',results);return report
