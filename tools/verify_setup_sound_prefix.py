...
def main():
    ...
    for call in range(1,47):
        ...
        require(result.returncode==0 and result.stderr=='','fresh isolated prefix probe failed')
        ...
        expected_bus=[]
        for r in cb:
            # The Mesen data observer excludes opcode/operand fetches and idle.
            if r['access']!=2 and not(r['access']==0 and r['pc']<=r['address']<r['pc']+4):expected_bus.append(r)
    ...
    identity={k:dict(path=str(path),sha256=digest(path))for k,path in {'rom':rom,'native_manifest':native/'manifest.json','previous_manifest':previous/'manifest.json','verifier':Path(__file__),'native_contract':Path(__file__).with_name('setup_native_trace_contract.py'),'scheduler_verifier':Path(previous_verifier.__file__)}.items()}
