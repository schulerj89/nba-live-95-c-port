"""Statically translate the bounded original sound-driver prefix source to C continuations.
...
def in_scope(pc):
    return any(a<=pc<=b for a,b in [(0xa137,0xa144),(0xa154,0xa16b),(0xa1ad,0xa1c2),(0xa1fa,0xa1fd),(0xa235,0xa238),(0xa278,0xa28e),(0xa293,0xa2a4),(0xa2bf,0xa2cd),(0x9fa1,0x9fb3),(0xa0b2,0xa0b3),(0xaae3,0xaae6)])
...
def translate(rom, decoder):
    jumps = {}
    todo, decoded, edges, returns = deque([(0xa137,0,1)]), {}, {}, set()
    while todo:
        state = todo.popleft()
        ...
        if pc == 0xa0b2: post=(1,1) # matching 9FA1 PHP after A137 SEP: M=1/X=1
        next_state=(pc+ins.length,*post)
        successors=[]
        if pc==0xaae6: successors=[] # unresolved SPC read; no response consumed
        elif ins.mnem in ('RTL','RTS'): pass
        elif ins.mnem in ('JSR','JSL'):
            target=ins.operand&65535
            post_call=(1,1) if target in (0x9fa1,0xa1bf) else (m,x)
            successors=[(target,m,x),(pc+ins.length,*post_call)]
            returns.add((pc+ins.length,*post_call))
        elif ins.mnem in BRANCH:
            successors=[(ins.operand,*post)]
            if ins.mnem!='BRA':successors.append(next_state)
        elif ins.mnem=='JMP':
            if ins.mode!=decoder.ABS:raise ValueError('unexpected indirect sound_prefix jump')
            successors=[(ins.operand,*post)]
        else:successors=[next_state]
        edges[state]=successors
        todo.extend(s for s in successors if in_scope(s[0]))

    def go(state): return 'goto ' + label(state) + ';' if state in decoded else f'return boundary(s, 0x80{state[0]:04x}u, NBA_SOUND_PREFIX_UNIMPLEMENTED);'
    def address(ins):
        operand = ins.operand
    ...
    lines = ['/* Generated from canonical ROM only by tools/generate_setup_sound_prefix_work.py.',
             ...
             'static bool advance(NbaSetupSoundPrefix *s)', '{', '    switch (B.resume) {',
             '    case 0: goto source_a137_m0x1;']
    for state, ins in sorted(decoded.items()):
        ...
        body = []
        if pc == 0xaae6:
            body = ['READ(0xaae6, 3, DB(0x2140u), 1, false);',
                    'return boundary(s, 0x80aae6u, NBA_SOUND_PREFIX_UNIMPLEMENTED);']; nxt = None
        elif pc == 0xa1be:
            body = ['return boundary(s, 0x80a1beu, NBA_SOUND_PREFIX_RETURN);']; nxt = None
            ...
            if ins.mode in (decoder.ACC, decoder.IMP):
                body += [f'IMP(0x{pc:04x});', f'accumulator_change(s, SOUND_{ins.mnem});']
            else:
                addr, indexed = address(ins)
                if ins.mode in (decoder.ABS_X, decoder.ABS_Y): indexed = 'true'
                body.append(f'CHANGE(0x{pc:04x}, {ins.length}, {addr}, {width}, {indexed}, SOUND_{ins.mnem});')
            ...
            if pc == 0xedf8:
                body += ['s->return_pc = ((uint32_t)s->read_bank << 16) | (uint16_t)(B.read_value + 1u);',
                         'B.status = NBA_CODEC_WORK_DONE;', 'return false;']
            else:
                body += ['if (s->read_bank != 0x80) return unsupported(s);',
                         'switch ((uint16_t)(B.read_value + 1u)) {']
                body += [f'case 0x{target[0]:04x}: '+go(target) for target in sorted(returns)]
                body += ['default: return unsupported(s);','}']
            nxt = None
        else: raise ValueError(f'unsupported source operation {ins}')
        if ins.mnem=='PLP' and pc == 0xa0b2: nxt=(pc+1,1,1)
