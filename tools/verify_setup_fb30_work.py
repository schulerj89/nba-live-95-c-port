"""Separate source-work and native validation for the bounded FB30 continuation.
...
from analyze_setup_fb30_semantics import decode as semantic_decode
import hashlib
import json
...
BUILD_SOURCES = {'include/nba_setup_codec_work.h', 'include/nba_setup_fb30_work.h', 'src/nba_setup_fb30_work.c',
                 'src/nba_setup_fb30_program.inc', 'tools/generate_setup_fb30_work.py',
                 'tools/setup_fb30_work_probe.c', 'tools/build_setup_fb30_work_probe.ps1'}
RESOURCES = {0xAEA0AF: (960, 115554, 736896)}
# Versioned, read-only codec capture route; updated only with a new audited capture.
CAPTURE_REVISIONS = {'script': 'd7bc0dfb9678ccc3ce0aabde1e55ce8ee24b9da61b0cdd6ffdf558bae3597f78', 'base_script': '1653867f508681901809885a72956e822417abfab0eaa246eda9b854673f5285', 'runner': 'a7aa5ae8b3ef27adcf30f9d5723cd2b9f327c98b17ed7ebbdb3587c0724c2dd2'}
...
def check_build(exe):
    ...
    source_root = Path(manifest['sources']['src/nba_setup_fb30_work.c']['path']).resolve().parents[1]
...
def native_records(directory):
    ...
    for index, row in enumerate(boundaries):
        ...
        if index % 2 == 0:
            source = [0xAEA0AF, 0xAEC446, 0xAED153, 0xA6C5FC, 0xAF97AA][(index // 2) % 5]
...
def separate_native_effects(call, instructions, writes):
    ...
    for index, (write, effect) in enumerate(zip(ports, ram)):
        require(effect['address'] == (0x7e0100 + index if index < 152 else 0x7f2000 + index - 152) and
...
def check_semantic_scratch(wram, semantic):
    require(wram[0x100:0x100 + len(semantic['symbols'])] == semantic['symbols'], 'canonical symbols differ')
    for address, key in [(0x500, 'counts'), (0x520, 'offsets'), (0x540, 'thresholds')]:
        packed = b''.join(v.to_bytes(2, 'little') for v in semantic[key])
        require(wram[address:address + len(packed)] == packed, 'canonical table differs: ' + key)
    require(wram[0x300:0x400] == semantic['fast_symbols'] and
            wram[0x400:0x500] == semantic['fast_lengths'], 'fast table differs')
    require(int.from_bytes(wram[0x0c:0x0e], 'little') == 0xa0af + semantic['prefetch_offset'] and
            int.from_bytes(wram[0x1e:0x20], 'little') == semantic['bit_buffer'] and
            wram[8] == semantic['last'], 'unconsumed terminator/cursor/prefetch differs')
...
def main():
    ...
    proofs, validations = [], []
    semantic = semantic_decode(blob[0x1720af:0x178000])
    for source in RESOURCES:
        report, payload, default_prefix = run_probe(exe, rom, source, output)
        require(payload == semantic['payload'], 'independent semantic payload differs')
        for scratch in [default_prefix.with_suffix('.wram')]:
            check_semantic_scratch(scratch.read_bytes(), semantic)
        ...
        check_semantic_scratch(prefix.with_suffix('.wram').read_bytes(), semantic)
        ...
        for start, end in zip(boundaries[::2], boundaries[1::2]):
            ...
            check_semantic_scratch((native / f'codec_{start["call"]:02d}_exit.wram').read_bytes(), semantic)
        ...
        input_end = int.from_bytes(prefix.with_suffix('.wram').read_bytes()[0x0c:0x0e], 'little')
        input_bytes = blob[input_start:input_start + input_end - (source & 65535)]
    ...
    identity = {name: dict(path=str(path), sha256=digest(path)) for name, path in {
        ...
        'verifier':Path(__file__), 'trace_contract':Path(__file__).with_name('setup_codec_trace_contract.py'), 'strict_scheduler_verifier':Path(__file__).with_name('verify_setup_scheduler.py'),
        'semantic_witness':Path(__file__).with_name('analyze_setup_fb30_semantics.py'),
        'semantic_native_reader':Path(__file__).with_name('verify_setup_codec_work.py')}.items()}
    work_proof = dict(schema=1, scope='C62B entry through C682 entry; FB30 only; no final RTL',
    ...
    print(f'PASS: FB30 work checksum; {len(ni)} instruction/register states; {len(nw)} CPU writes; four native payloads')
