...
if (!$OutputDirectory) { $OutputDirectory = Join-Path $root 'build\scheduler-probe' }
...
cl /nologo /W4 /WX /O2 /MD /utf-8 /I "$root\include" /Fe"$output\setup_scheduler_probe.exe" /Fo"$output\\" "$root\tools\setup_scheduler_probe.c" "$root\src\nba_setup_scheduler.c"
exit /b %ERRORLEVEL%
"@
Set-Content -LiteralPath $batch -Value $command -Encoding ASCII
& cmd /c $batch
if ($LASTEXITCODE -ne 0) { throw 'Fresh scheduler probe compilation failed.' }
$files = @('include/nba_setup_scheduler.h', 'src/nba_setup_scheduler.c', 'tools/setup_scheduler_probe.c', 'tools/build_setup_scheduler_probe.ps1')
...
$exe = Join-Path $output 'setup_scheduler_probe.exe'
