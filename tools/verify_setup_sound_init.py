...
import verify_setup_sound_prefix as previous_verifier
import json
...
def main():
    ...
    for call in range(1,6):
        ...
        require(result.returncode==0 and result.stderr=='','fresh isolated initializer probe failed')
        ...
        expected_bus=[r for r in cb if r['access']!=2 and not(r['access']==0 and r['pc']<=r['address']<r['pc']+4)]
    ...
    identity={k:dict(path=str(path),sha256=digest(path))for k,path in {'rom':rom,'native_manifest':native/'manifest.json','previous_manifest':previous/'manifest.json','verifier':Path(__file__),'native_contract':Path(__file__).with_name('setup_native_trace_contract.py'),'prefix_verifier':Path(previous_verifier.__file__),'scheduler_verifier':Path(__file__).with_name('verify_setup_scheduler.py')}.items()}
