...
def main():
    ...
    spec=importlib.util.spec_from_file_location('sound_init_mutation_target',a.verifier)
    ...
    def data_read(rows):
        ...
        row['value']^=1
    def unresolved(rows):
        row=next(r for r in rows if r['kind']=='read' and r['address']==0x802140);row['value']=255
    cases=[('native_bus_clock','sound_init_bus.jsonl',lambda r:change(r,0,'master_clock',0),True),
           ('native_bus_pc','sound_init_bus.jsonl',lambda r:change(r,0,'pc',0),True),
           ('native_instruction_reorder','sound_init_instructions.jsonl',lambda r:reverse(r,'event'),True),
           ('native_bus_reorder','sound_init_bus.jsonl',lambda r:reverse(r,'event'),True),
           ('native_wrong_call','sound_init_bus.jsonl',lambda r:change(r,0,'call',2),True),
           ...
           ('unresolved_value_not_input','sound_init_bus.jsonl',unresolved,False)]
    result=[]
