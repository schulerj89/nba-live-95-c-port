"""Cold reset to first resident initialization DSP read, observation only."""
...
def main():
 ...
 script=out/'capture.lua';shutil.copyfile(Path(__file__).with_name('mesen_setup_spc_init.lua'),script)
 runner=out/'runner.py';shutil.copyfile(__file__,runner)
 manifest={'schema':1,'kind':'cold-reset SPC initializer observation; isolated component witness only','state_injection':False,'rom_patch':False,'accepted':False,'settings':settings,'sources':{},'initial_saves':[]}
 ...
 args=[str(exe),'--testrunner','--timeout=300',str(a.rom.resolve()),str(script)];manifest['arguments']=args
 env={k:v for k,v in os.environ.items()if not k.startswith('NBA95_')};env['NBA95_SPC_INIT_DIR']=str(out)
 startup=subprocess.STARTUPINFO();startup.dwFlags|=subprocess.STARTF_USESHOWWINDOW;startup.wShowWindow=0
 try:
  with (out/'mesen.log').open('w')as f:r=subprocess.run(args,env=env,stdout=f,stderr=subprocess.STDOUT,startupinfo=startup,timeout=320)
  manifest['exit_code']=r.returncode
  assert r.returncode==0 and (out/'spc_init_complete.txt').read_text().startswith('ok; instructions=')
