...
from setup_spc_control_contract_v5 import validate as validate_state, control_boundary
...
def main(a):
 ...
 report={'passed':True,'scope':'post-cycle F1 commit only; hidden staged inputs synthetic, not native-observed; no clock advancement','cases':results,'native_manifest_sha256':sha(p/'manifest.json'),'build_manifest_sha256':sha(a.exe.parent/'build-manifest.json'),'verifier_sha256':sha(__file__),'evidence_contract_sha256':sha(ROOT/'tools/setup_spc_evidence_contract_v3.py'),'state_contract_sha256':sha(ROOT/'tools/setup_spc_state_contract_v4.py'),'control_contract_sha256':sha(ROOT/'tools/setup_spc_control_contract_v5.py')};(out/'report.json').write_text(json.dumps(report,indent=2)+'\n');print('PASS 2 publications,70 visible fields,2 fullARAM endpoints');return report
