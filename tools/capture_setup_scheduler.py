"""Bounded natural Setup scheduler capture with a per-process portable Mesen."""
import argparse
import hashlib
import json
import os
...
def main():
 ...
 script=out/'capture.lua';shutil.copyfile(Path(__file__).with_name('mesen_setup_scheduler.lua'),script)
 ...
 for name,path in [('rom',rom),('mesen',exe),('script',script),('runner',runner),('settings',initial)]:
