...
import verify_setup_codec_work_v2 as verify
...
class IntegrityTests(unittest.TestCase):
    ...
    def test_nmi_exclusion_requires_exact_protocol(self):
        instructions = [r for r in self.instructions if r['call'] == 2]
        writes = [r for r in self.writes if r['call'] == 2]
        ...
        for field in ('value', 'address', 'pc'):
            ...
            row[field] ^= 1
            with self.assertRaises(ValueError): verify.separate_native_effects(2, instructions, bad)

    def test_induced_wram_write_must_match_cpu_write(self):
        bad = copy.deepcopy(self.writes)
        row = next(r for r in bad if r['call'] == 2 and r['address'] == 0x7f2000)
        row['value'] ^= 1
        with self.assertRaises(ValueError): verify.separate_native_effects(2, self.instructions, bad)
    ...
    def test_source_continuation_self_tests(self):
        p = subprocess.run([str(self.exe), '--self-test'], text=True, capture_output=True)
        self.assertEqual(p.returncode, 0, p.stdout + p.stderr)
        self.assertEqual(p.stdout, 'PASS: 12 codec continuation contract cases\n')
    ...
    def test_bad_numeric_tokens_reject_before_output(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for state in ('-0,0,0,8175,128,0,0', '+0,0,0,8175,128,0,0',
                ...
                p = subprocess.run([str(self.exe), str(self.rom), 'AEC446', str(out), str(trace), state], capture_output=True)
