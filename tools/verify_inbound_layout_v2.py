"""Revision2: exact C37D evidence protocol; never use capture timing in C."""
import argparse
import copy
import json
...
def verify(capture, probe):
    ...
    require(set(m) == {'schema','kind','case','controlled','cpu_writes','rom_patch','controlled_words',
                       'arguments','environment','isolation','sources','exit_code','completion','artifacts'}, 'manifest schema')
    case = m['case']
    ...
    require(type(m['controlled_words']) is list and all(type(x) is int for x in m['controlled_words']) and
            m['controlled_words'] == (ADDRESSES if case else []), 'controlled words')
    ...
    paths = {'mesen':capture/'portable-mesen'/'Mesen.exe','capture':capture/'capture.lua',
             'runner':capture/'capture_inbound_layout.py','isolation_helper':capture/'mesen_portable.py'}
    for key, expected in SOURCE.items():
        item = m['sources'][key]
        require(set(item) == {'path','sha256'} and item['sha256'] == expected, 'source metadata '+key)
        if key in paths:
            require(item['path'] == str(paths[key]), 'executed source path '+key)
    ...
    iso = m['isolation']
    require(set(iso) == {'method','home','save_folder','initial_saves','settings','initial_settings_sha256',
                         'post_settings_verified','observed_script_data_folder','post_settings_sha256','final_saves'} and
            iso['method'] == 'private portable executable/settings', 'isolation schema/method')
    settings = {
    ...
    branches = {0:0x85c548, 1:0x85c50b, 2:0x85c49e, 3:0x85c4d4, 4:0x85c450, 5:0x85c3e4, 65535:0x85c50b}
    layout = word(entry,0x956)
    require(layout in branches and [pc for pc in pcs if pc in branches.values()] == [branches[layout]], 'coherent original dispatcher branch')
    ...
    require(type(run.returncode) is int and run.returncode == 0 and len(run.stdout.splitlines()) == 1 and run.stderr == '', 'C response count/status/stderr')
    ...
    require(run.stdout == ' '.join(f'{x:04x}' for x in actual)+'\n', 'canonical C response')
