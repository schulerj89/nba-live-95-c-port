...
def scope(pc):return 0x800d<=pc<0x80bc or 0xab06<=pc<=0xab7d
...
def generate(rom,d):
    ...
    out=['/* Generated canonical reset/upload source. No opcode dispatch. */',
    ...
    for state,i in sorted(found.items()):
        ...
        elif i.mnem in ('STA','STZ'):
            a,index=addr();b+=[f'WRITE(0x{pc:x},{i.length},{a},{w},{index},{"A" if i.mnem=="STA" else "0"});']
