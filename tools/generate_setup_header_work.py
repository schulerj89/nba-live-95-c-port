"""Statically translate the bounded original backdrop header source to C continuations.
...
def in_scope(pc):
    return any(a<=pc<=b for a,b in [(0xeec6,0xef1a),(0xef8e,0xef93),(0x8a02,0x8a41),(0x8ad2,0x8b34)])
...
def translate(rom, decoder):
    jumps = {}
    todo, decoded, edges, returns = deque([(0xeec6,0,0)]), {}, {}, set()
    while todo:
        state = todo.popleft()
        ...
        successors=[]
        if pc==0xef1a: successors=[] # before source JSL wait
            ...
            if ins.mode!=decoder.ABS:raise ValueError('unexpected indirect header jump')
            successors=[(ins.operand,*post)]
    ...
    lines = ['/* Generated from canonical ROM only by tools/generate_setup_header_work.py.',
             ...
             'static bool advance(NbaSetupHeaderWork *s)', '{', '    switch (B.resume) {',
             '    case 0: goto source_eec6_m0x0;']
    for state, ins in sorted(decoded.items()):
        ...
        body = []
        if pc == 0xef1a:
            body = ['B.status = NBA_CODEC_WORK_DONE;', 'return false;']; nxt = None
            ...
            if ins.mode in (decoder.ACC, decoder.IMP):
                body += [f'IMP(0x{pc:04x});', f'accumulator_change(s, HEADER_{ins.mnem});']
            else:
                addr, indexed = address(ins)
                if ins.mode in (decoder.ABS_X, decoder.ABS_Y): indexed = 'true'
                body.append(f'CHANGE(0x{pc:04x}, {ins.length}, {addr}, {width}, {indexed}, HEADER_{ins.mnem});')
