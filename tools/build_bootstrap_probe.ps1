...
$sources=@('tools/bootstrap_probe.c','src/nba_bootstrap.c','src/nba_bootstrap_cpu.c','src/nba_bootstrap_ipl.c','src/nba_bootstrap_rom.c','src/nba_setup_spc_init.c','src/nba_setup_spc_control.c','src/nba_rom.c')
$headers=@('include/nba_bootstrap.h','include/nba_bootstrap_internal.h','include/nba_setup_codec_work.h','include/nba_setup_spc_init.h','include/nba_setup_spc_control.h','include/nba_setup_spc_resident.h','include/nba_rom.h','include/nba_types.h','src/nba_bootstrap_cpu_program.inc','tools/generate_bootstrap_cpu.py','tools/build_bootstrap_probe.ps1')
...
cl /nologo /W4 /WX /O2 /MD /utf-8 /I "$root\include" /Fe"$output\bootstrap_probe.exe" /Fo"$output\\" $arguments
exit /b %ERRORLEVEL%
...
$exe=Join-Path $output 'bootstrap_probe.exe'
