/* Actual AF1D/AEC3/B649/B832/AF30 prestate only. No expected output input. */
...
int main(int argc,char **argv) {
 ...
 while(fgets(line,sizeof(line),stdin)) {
  ...
  load(path);NbaHumanPassPoseState s=input();bool ok=false;
  if(!strcmp(mode,"resolve"))ok=nba_human_pass_pose_resolve(&assets,&s);
  else if(!strcmp(mode,"offset"))ok=nba_human_pass_pose_offset(&assets,&s);
  else if(!strcmp(mode,"attach"))ok=nba_human_pass_pose_attach(&assets,&s);
  else if(!strcmp(mode,"prefix"))ok=nba_human_pass_pose_prefix(&assets,&s);
  else if(!strcmp(mode,"commit"))ok=nba_human_pass_pose_commit(&s);
  else if(!strcmp(mode,"chain"))ok=nba_human_pass_pose_prepare(&assets,&s);
  else return 2;
  if(!ok)return 3;project(&s);
  printf("{\"route\":1,\"dp_words\":[");
  static const unsigned dp[]={0,2,4,6,0x47,0x49,0xac};
  ...
  static const unsigned globals[]={0x90c,0x90e,0x910,0x922,0x936,0x93a,0x93e,0x942,0x944,0x946,0x978,0x9b8,0x9c4,0x9da};
