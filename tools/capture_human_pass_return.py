...
def main():
 ...
 script=out/'capture.lua';shutil.copyfile(Path(__file__).with_name('mesen_human_pass_return.lua'),script)
 shutil.copyfile(__file__,out/Path(__file__).name)
 shutil.copyfile(mesen_portable.__file__,out/'mesen_portable.py')
 env=os.environ.copy();env.update(NBA95_CAPTURE_DIR=out.as_posix(),NBA95_PASS_RETURN_SELECTION=str(a.selection),NBA95_PASS_RETURN_FRAMES=str(a.frames))
 args=[str(exe),'--testrunner','--timeout=240',str(a.rom.resolve()),str(script)]
 m=dict(schema=1,kind='native human pass return boundaries',selection=a.selection,state_injection=False,rom_patch=False,
  sparse_ranges=[[0,0x2000],[0x3400,0x1600]],requested_frames=a.frames,
  arguments=args,environment={k:v for k,v in env.items()if k.startswith('NBA95_PASS_RETURN')or k=='NBA95_CAPTURE_DIR'},isolation=isolation,
