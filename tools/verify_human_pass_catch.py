"""Compare AD3D catch geometry/RNG/F02D/F5E4 to AE10 or unexecuted B468 child."""
...
SOURCE_VERSION={'capture':'e54cfa99ff1828384990d60e423a49b0a786ca8d11897d851e2f4a5c8da505dd',
 'runner':'0a7d4c191a37dae4a5285d684b861e42b9acb2a5689f72903b332f3f08152a13',
...
PCS.update({'catch.entry':(0x86ad3d,), 'catch.rating':(0x86ad83,), 'catch.geometry':(0x86ad98,),
 'catch.attempt':(0x86ada3,), 'catch.rng.entry':(0x80cee7,), 'catch.rng.exit':(0x80cef5,0x80cefc),
 'catch.direction.entry':(0x85f02d,), 'catch.direction.exit':(0x85f092,0x85f099),
 'catch.lane.entry':(0x85f5e4,), 'catch.lane.exit':(0x85f727,),
 'catch.receiver':(0x86af66,), 'catch.receiver.child':(0x86b468,), 'catch.ready':(0x86ae10,)})
RAW_TAGS.update(k for k in PCS if k.startswith('catch.'))
...
def bus_word(memory,pc,rom):
 def byte(address):
  bank=(address>>16)&255;low=address&65535
  if (bank&127)<0x40 and low<0x2000:return memory[low]
  if bank in(0x7e,0x7f):return memory[(bank-0x7e)*0x10000+low]
  if low>=0x8000:
   offset=(bank&127)*0x8000+(low&0x7fff);need(offset<len(rom),'unmapped source ROM pointer');return rom[offset]
  raise ValueError('unsupported source bus pointer')
 return byte(pc)|(byte((pc+1)&0xffffff)<<8)
def long_pointer(memory,dp):return word(memory,dp)|((word(memory,dp+2)&255)<<16)
...
def attest(capture,rom):
 ...
 need(integer(m.get('schema'),1,1)and m.get('kind')=='native human pass catch boundaries','wrong capture schema')
 ...
 private={'mesen':'portable-mesen/Mesen.exe','capture':'capture.lua','runner':'capture_human_pass_catch.py','isolation_helper':'mesen_portable.py'}
 ...
 need(m.get('environment')=={'NBA95_CAPTURE_DIR':capture.as_posix(),'NBA95_PASS_CATCH_SELECTION':str(m['selection']),
                            'NBA95_PASS_CATCH_FRAMES':str(m['requested_frames'])},'changed executed environment')
 ...
 required={'boundaries.jsonl','capture.lua','capture_complete.txt','capture_human_pass_catch.py','mesen_portable.py',
 ...
 used_raw=set();fields={'index','tag','pc','frame','court','raw','indirect_addr','indirect_word','cpu_d','cpu_x','cpu_y','cpu_ps','actor','owner','live','offense','direction','candidate','score'}
 ...
 for index,row in enumerate(rows,1):
  ...
  bounds={'index':(1,len(rows)),'pc':(0,0xffffff),'frame':(0,17999),
          'court':(-1,int(complete['frames'])-1),'cpu_ps':(0,255),'indirect_addr':(0,0xffffff)}
  ...
  if row['tag']in RAW_TAGS:
   ...
   if row['tag']=='catch.rating':
    address=(long_pointer(state,0)+0x42)&0xffffff
    need(row['cpu_y']==0x42 and row['indirect_addr']==address and row['indirect_word']==bus_word(state,address,rom.read_bytes()),'actual indirect pointer/read mismatch')
  else:need(row['raw']=='','unexpected raw boundary')
  if row['tag']!='catch.rating':need(row['indirect_addr']==0 and row['indirect_word']==0,'spurious indirect read metadata')
...
def verify(capture,probe,rom,diagnostics,assets=DEFAULT_ASSETS,observe_only=False):
 need(type(observe_only)is bool,'invalid observation mode')
 m,rows,native_calls=attest(capture,rom)
 need(sha(assets)==ASSETS_SHA,'wrong immutable asset pack');rom_data=rom.read_bytes()
 catch_stages=collections.Counter();catch_calls=[];catch_children=[];
 ...
 def append_pose(mode,entry,row):
  ...
  pose_stages[mode]+=1 # The later pose checkpoint is observed, not replayed here.
 def append_catch(mode,entry,row,route=0):
  pre=raw(capture,entry);end=raw(capture,row)
  want=dict(input_words=[bus_word(pre,(long_pointer(pre,0)+0x42)&0xffffff,rom_data),bus_word(pre,(long_pointer(pre,0xe0)+0x39)&0xffffff,rom_data),word(pre,word(pre,0x9e)+10)],
   dp_words=[word(end,a)for a in(0,2,0x51,0x8e,0x92,0x96,0x9a,0x9e,0xaa,0xac,0xae,0xb2,0xb6,0xba,0xbe,0xc2,0xa6)],
   actor_words=words(end,0x34eb,11*128),controller_words=words(end,0x47eb,160),context_words=words(end,0x46eb,128),profile_words=words(end,0x3449,20),order_words=words(end,0x34d1,13),
   global_words=[word(end,a)for a in(0x7f6,0x904,0x90c,0x90e,0x910,0x922,0x936,0x93a,0x93e,0x942,0x944,0x946,0x978,0x9b8,0x9c4,0x9da)])
  if mode in('prefix','attempt','receiver'):want['route']=route
  else:want['result']=1
  inputs.append(mode+' '+str(capture/entry['raw']));expected.append(want);catch_stages[mode]+=1
 ...
 for row in rows:
  ...
  if tag.startswith('pose.'):
   ...
   continue
  if tag.startswith('catch.'):
   need(phase=='child'and pending['init_step']=='continuation','catch outside actual pass child')
   step=pending.get('catch_step');state=raw(capture,row)
   # The translated ADC/SBC operations are binary. The native callees do
   # not clear decimal mode; D=0 is an explicit supported entry domain.
   need((row['cpu_ps']&0x38)==0,'catch requires native 16-bit A/X and decimal mode off')
   if tag=='catch.entry':
    need(step is None and pending.get('aligned_stop',{}).get('tag')=='aligned.catch','catch missing original AD3D gate')
    previous=pending['aligned_stop']
    need(row['index']==previous['index']+1 and state==raw(capture,previous)and row['cpu_y']==word(state,0x8e),'changed shared AD3D/carry-Y input')
    pending['catch_entry']=row;pending['catch_step']='rating';pending['catch_child']=None;pending['catch_attempt']=False
   elif tag=='catch.rating':
    need(step=='rating','misordered indirect rating read');pending['catch_step']='geometry';pending['catch_rating']=row
   elif tag=='catch.geometry':
    need(step=='geometry','misordered catch geometry')
    rating=pending['catch_rating']
    need(row['cpu_x']==((rating['indirect_word']&255)>>4)*2,'AD8D rating-table X mismatch')
    append_catch('geometry',pending['catch_entry'],row);pending['catch_geometry']=row;pending['catch_step']='attempt'
    # Require every source-called child, not merely balanced pairs among
    # whichever events remain. This is a call-order guard, not a C oracle.
    distance=word(state,0xaa);threshold=word(state,0xae)
    active=not(((threshold-distance)&0x8000)or((distance-16)&0x8000))
    pending['catch_expected_attempt']=active;children=[]
    if active:
     profile=bus_word(state,(long_pointer(state,0xe0)+0x39)&0xffffff,rom_data)&255
     axis_gate=profile<76
     if 76<=profile<92:
      children.append('rng');seed=word(state,0x7f6)
      next_seed=(((seed<<1)^(0x1d87 if seed&0x8000 else 0))&65535)if seed else 0x9146
      axis_gate=(next_seed&(1 if profile<84 else 3))==0
     if not axis_gate or word(state,word(state,0x96)+0x88)>>1 not in(0,4):children+=['direction','lane']
    pending['catch_expected_children']=children;pending['catch_finished_children']=[]
   elif tag=='catch.attempt':
    need(step=='attempt'and pending['catch_expected_attempt']and not pending['catch_attempt'],'misordered catch attempt gate');pending['catch_attempt']=True
   elif tag in('catch.rng.entry','catch.direction.entry','catch.lane.entry'):
    child=tag.split('.')[1]
    need(step=='attempt'and pending['catch_attempt']and pending['catch_child']is None,'misordered catch child')
    finished=pending['catch_finished_children'];required=pending['catch_expected_children']
    need(len(finished)<len(required)and required[len(finished)]==child,'missing, duplicate or reordered source child')
    if child=='lane':need(word(state,0x96)==word(state,0x8e),'F5E4 did not receive receiver in96')
    pending['catch_child']=row
   elif tag in('catch.rng.exit','catch.direction.exit','catch.lane.exit'):
    entry=pending['catch_child'];child=tag.split('.')[1]
    need(step=='attempt'and entry is not None and entry['tag']=='catch.'+child+'.entry','unpaired catch child')
    if child=='direction':need(row['cpu_x']==entry['cpu_x'],'F02D did not restore carried X')
    if child=='lane':need(row['cpu_x']==word(state,0x96),'F5E4 lost subject X')
    append_catch(child,entry,row);catch_children.append(dict(kind=child,entry=entry['index'],exit=row['index'],court=row['court']))
    pending['catch_child']=None;pending['catch_finished_children'].append(child)
   elif tag in('catch.ready','catch.receiver'):
    need(step=='attempt'and pending['catch_child']is None,'unfinished catch child at continuation')
    need(pending['catch_attempt']==pending['catch_expected_attempt']and pending['catch_finished_children']==pending['catch_expected_children'],'missing required catch attempt/children')
    route=0 if tag=='catch.ready'else 1
    append_catch('attempt',pending['catch_geometry'],row,route)
    if route:
     pending['catch_receiver']=row;pending['catch_step']='receiver'
    else:
     append_catch('prefix',pending['catch_entry'],row,0);pending['catch_step']='done'
     catch_calls.append(dict(entry=pending['catch_entry']['index'],exit=row['index'],court=row['court'],route=0))
   else:
    need(tag=='catch.receiver.child'and step=='receiver','misordered B468 continuation')
    append_catch('receiver',pending['catch_receiver'],row,2);append_catch('prefix',pending['catch_entry'],row,2);pending['catch_step']='done'
    catch_calls.append(dict(entry=pending['catch_entry']['index'],exit=row['index'],court=row['court'],route=2))
   continue
  if tag=='pass.resume':
   ...
   if pending.get('aligned_stop',{}).get('tag')=='aligned.catch':need(pending.get('catch_step')=='done','missing AD3D catch closure')
 ...
 if observe_only:
  need(not inputs and not catch_calls,'observation mode cannot skip existing catch comparisons')
  return dict(kind='attested route with no AD3D catch calls; no C parity result',capture_attested=True,comparison_performed=False,native_catch_calls=0,pass_calls_observed=native_calls,manifest_sha256=sha(capture/'manifest.json'),rom_sha256=sha(rom),verifier_sha256=sha(__file__),compared_values=0)
 need(catch_stages['prefix']>0 and catch_stages['geometry']==catch_stages['attempt']==catch_stages['prefix'],'no complete catch comparisons; use observation-only mode for a route with none')
...
def main():
 ...
 try:report=verify(a.capture.resolve(),a.probe.resolve(),a.rom.resolve(),a.output,a.assets.resolve(),a.observe_only)
 ...
 if report.get('failures'):print(json.dumps(report['failures'][:10]));raise SystemExit(1)
