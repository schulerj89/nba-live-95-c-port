"""Action v2 evidence and diagnostic mutation tests, derived from the independent pass audit tool."""
...
def main():
    ...
    index = next(i for i, r in enumerate(original_rows) if r['tag'] == 'init.ready')
    ...
    events('missing grounded stage', lambda rows: rows.pop(next(i for i,r in enumerate(rows) if r['tag']=='action.ground.entry')))
    ...
    for field in ('route', 'actor_words', 'controller_words', 'context_words', 'global_words', 'dp_words', 'profile_words', 'profile_byte'):
