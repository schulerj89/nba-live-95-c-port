"""Cold reset to eight resident command05 completions, observation only."""
...
def main():
 ...
 script=out/'capture.lua';shutil.copyfile(Path(__file__).with_name('mesen_setup_spc_resident.lua'),script)
 runner=out/'runner.py';shutil.copyfile(__file__,runner)
 manifest={'schema':1,'kind':'cold-reset resident SPC observation; isolated component witness only','state_injection':False,'rom_patch':False,'accepted':False,'settings':settings,'sources':{},'initial_saves':[]}
 ...
 env={k:v for k,v in os.environ.items()if not k.startswith('NBA95_')};env['NBA95_SPC_RESIDENT_DIR']=str(out)
 ...
 try:
  ...
  assert r.returncode==0 and (out/'spc_resident_complete.txt').read_text()=='ok; commands=8\n'
