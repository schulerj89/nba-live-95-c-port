...
local selection=assert(tonumber(os.getenv('NBA95_PASS_SELECTION')))
local stop_at=assert(tonumber(os.getenv('NBA95_PASS_FRAMES')))
...
hook(0x86ab2d,function()
 if active=='waitingchild'then snap('pass.initialize',0x86ab2d,true);active='child'end
end)
hook(0x84e09c,function()
 if active then
  check(not metric and(active=='selection'or active=='child'),'bad pass resume')
