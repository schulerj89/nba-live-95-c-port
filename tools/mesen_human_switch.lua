...
local selection=assert(tonumber(os.getenv('NBA95_SWITCH_SELECTION')))
local stop_at=assert(tonumber(os.getenv('NBA95_SWITCH_FRAMES')))
...
local index,active,calls=0,false,0
...
local function snap(tag,pc,raw)
 ...
 log:write(string.format('{"index":%d,"tag":"%s","pc":%d,"frame":%d,"court":%d,"raw":"%s","cpu_d":%d,"actor":%d,"owner":%d,"live":%d,"offense":%d,"direction":%d,"candidate":%d,"score":%d}\n',
  index,tag,pc,frame,court,file,s['cpu.d']or 0,w(0xc2),w(0x93e),w(0x936),w(0x93a),w(0x90c)>=0x47eb and w(w(0x90c)+6)or 65535,w(0x92),w(0xaa)))
 log:flush()
end
...
hook(0x84e141,function()
 if court<0 then return end
 check(not active,'nested switch');active=true;calls=calls+1;snap('switch.entry',0x84e141,true)
end)
for _,t in ipairs({{0x84e14b,'switch.full'},{0x84e230,'switch.exit'}})do
 hook(t[1],function()if active then snap(t[2],t[1],true);active=false end end)
end
for _,t in ipairs({{0x84e175,'scan.directional'},{0x84e231,'scan.neutral'},
 {0x84e1d9,'candidate.directional'},{0x84e28c,'candidate.neutral'},
 {0x84e1fc,'switch.transfer'},{0x84e21e,'switch.restore'}})do
 hook(t[1],function()if active then snap(t[2],t[1],false)end end)
end
-- Pass child is observed for route coverage only; its body is not replayed.
hook(0x84df7a,function()if court>=0 then snap('pass.observed',0x84df7a,false)end end)
