"""Strict evidence/build/probe contracts for an isolated sound-init component."""
import argparse
import copy
import subprocess
import tempfile
...
import verify_setup_sound_init as v
...
class Contracts(unittest.TestCase):
    ...
    def test_invalid_numeric_tokens_reject_without_outputs(self):
        with tempfile.TemporaryDirectory(prefix='sound-init-contract-')as temp:
            ...
            for state in ('-0,0,0,8158,128,19','4294967296,0,0,8158,128,19','0g,0,0,8158,128,19',
                ...
                r=subprocess.run([str(self.exe),str(self.rom),str(self.native/'sound_init_01_entry.wram'),state,str(trace),str(wram),'isolated-component-differential'],capture_output=True)
                self.assertEqual(r.returncode,2,state);self.assertFalse(trace.exists());self.assertFalse(wram.exists())

    def test_uploaded_program_bytes_are_required(self):
        original=Path.read_bytes
        def altered(path):
            data=original(path)
            if path.name=='sound_init_uploaded.spc':
                changed=bytearray(data);changed[0x380]^=1;return bytes(changed)
            return data
        with patch.object(Path,'read_bytes',altered):
            with self.assertRaises(ValueError):v.upload_provenance(self.native,self.rom)

    def test_upload_entry_and_source_observations_required(self):
        original=v.json_lines
        for field,value in [('source',0),('pc',0),('y',0)]:
            def altered(path):
                rows=original(path)
                if path.name=='sound_init_upload.jsonl':rows[1 if field=='y'else 0][field]=value
                return rows
