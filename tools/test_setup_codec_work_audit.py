"""Independent codec trace protocol mutations, with unchanged native evidence.
...
def main():
    ...
    run('baseline')
    reader = v.json_lines
    baseline = reader(args.output / 'baseline/AEC446-native-entry.jsonl')
    ci = [r for r in baseline if r['kind'] == 'instruction']
    _, instructions, writes = v.native_records(args.native.resolve())
    ni, _, _ = v.separate_native_effects(2, instructions, writes)
    ...
    for name, mutation in cases:
        ...
        def altered(path):
            rows = reader(path)
            if path.name == 'AEC446-native-entry.jsonl':
                mutation(rows)
            return rows
