...
# Exact frame origins for this pinned normal-input capture revision/settings.
# Both retained routes observe Player Setup at2697; left uses the later Start.
# These validate recorded clocks; they are never production timing inputs.
CLOCK_ANCHORS={0:(2697,4590),2:(2697,4390)}
...
def attest(capture,rom):
 ...
 for key,source in m['sources'].items():
  need(set(source)=={'path','sha256'},'invalid source fields '+key)
 ...
 player_frame,court_frame=CLOCK_ANCHORS[m['selection']]
 for index,row in enumerate(rows,1):
  need(set(row)==fields,'invalid native event fields')
  need(type(row['tag'])is str and row['tag']in PCS,'unknown native event')
  for field in fields-{'tag','raw','index','pc','frame','court'}:
   need(integer(row[field],0,0xffff),'invalid native word '+field)
  need(integer(row['index'],1,len(rows))and integer(row['pc'],0,0xffffff),'invalid event index/PC type')
  need(integer(row['frame'],0,17999)and integer(row['court'],-1,int(complete['frames'])-1),'event clocks exceed source/completion bound')
  ...
  if index==1:
   need(row['tag']=='player.entry'and row['court']==-1 and row['frame']==player_frame,'wrong versioned Player Setup clock')
  else:
   need(row['court']>=0 and row['frame']==court_frame+row['court'],'wrong versioned court clock relation')
   if index==2:need(row['tag']=='court.entry'and row['court']==0,'missing court clock origin')
  if row['tag']in RAW_TAGS:
   ...
   memory=raw(capture,row)
   # The Lua diagnostic actor is DP C2, not the switch's DP96 actor operand.
   for field,address in {'actor':0xc2,'owner':0x93e,'live':0x936,'offense':0x93a,'candidate':0x92,'score':0xaa}.items():
    need(row[field]==word(memory,address),'native row/raw disagreement '+field)
   controller=word(memory,0x90c)
   if controller>=0x47eb:
    need(controller+6 in memory and controller+7 in memory,'unobserved controller direction address')
    direction=word(memory,controller+6)
   else:direction=0xffff
   need(row['direction']==direction,'native row/raw disagreement direction')
