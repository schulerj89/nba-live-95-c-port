"""Strict native mode15 origin/word/CPU/clock and complete asset C protocol mutations."""
...
def main():
 ...
 spec=importlib.util.spec_from_file_location('release_verifier',a.verifier);v=importlib.util.module_from_spec(spec);spec.loader.exec_module(v)
 ...
 stdout=(a.output/'baseline.probe-stdout.txt').read_text();stderr=(a.output/'baseline.probe-stderr.txt').read_text();assert stderr==f"[ASSETS] Loaded asset pack: '{v.DEFAULT_ASSETS}' ({v.DEFAULT_ASSETS.stat().st_size} bytes, 263 assets)\n"
 checks=[]
 ...
 metadata('wrong environment route',lambda m:m['environment'].update(NBA95_PASS_RELEASE_SELECTION='2'))
 ...
 entry=next(i for i,r in enumerate(rows)if r['tag']=='mode.entry')
 for key,value in(('actor',True),('actor_pointer',65536),('origin',True),('call',65536),('owner',65536),('live',65536),('offense',65536),('cpu_a',65536),('cpu_x',65536),('cpu_y',False),('cpu_sp',8192),('cpu_k',256),('cpu_dbr',256),('cpu_pc',65536),('cpu_ps',256)):
 ...
 for tag in('dispatch.call','wrapper.entry','mode.entry','offset.entry','offset.exit.point0','attach.exit','wrapper.exit','mode.return'):
  events('missing boundary '+tag,lambda rs,t=tag:rs.pop(next(i for i,r in enumerate(rs)if r['tag']==t)))
 for tag in('dispatch.call','wrapper.entry','mode.entry','wrapper.exit','mode.return','launch.exit','launch.return'):
  index=next(i for i,r in enumerate(rows)if r['tag']==tag)
  for key in('cpu_a','cpu_x','cpu_y','cpu_ps'):
   events('CPU call contract '+tag+' '+key,lambda rs,i=index,k=key:rs[i].update({k:rs[i][k]^1}))
 for tag in('mode.entry','mode.special','special.exit','normalize.entry','launch.return','offset.entry'):
  index=next(i for i,r in enumerate(rows)if r['tag']==tag)
  events('binary arithmetic domain '+tag,lambda rs,i=index:rs[i].update(cpu_ps=rs[i]['cpu_ps']|8))
 for key in('origin','call'):
  events('forged mode '+key,lambda rs,k=key:rs[entry].update({k:rs[entry][k]+1}))
 for label,before,after,bank in(('bad predecessor',0xffff,0xef14,0x85),('bad successor',0xeeee,0xef15,0x85),('wrong bank',0xeeee,0xef14,0x84)):
  def altered_irq(capture,row,a=before,b=after,k=bank):
   memory=original_raw(capture,row)
   if row['call']==1 and row['tag']in('dispatch.entry','dispatch.call'):
    value=a if row['tag']=='dispatch.entry'else b
    memory[0x5c8]=value&255;memory[0x5c9]=value>>8;memory[0x5ca]=k;memory[0x5cb]=0
   return memory
  run('IRQ scope '+label,patch.object(v,'raw',side_effect=altered_irq))
 ...
 for text in('',stderr+'ERROR\n',stderr.replace(str(v.DEFAULT_ASSETS),'forged-other.pak')):
