param([Parameter(Mandatory=$true)][string]$OutputDir,[switch]$Controlled,[switch]$InitActor,[int]$Frames=4000)
...
$env:NBA95_CAMERA_CONTROL=if($Controlled){'1'}else{'0'}
$env:NBA95_CAMERA_INIT_ACTOR=if($InitActor){'1'}else{'0'}
...
$p=Start-Process -FilePath (Get-Command Mesen.exe).Source -ArgumentList @('--testrunner','--timeout=300',
    '"F:/Games/SNES/NBA Live 95 (USA).sfc"',('"'+$PSScriptRoot+'/mesen_camera_handoff.lua"')) -PassThru -WindowStyle Hidden
$p.WaitForExit()
if($p.ExitCode -ne 0 -or !(Test-Path "$OutputDir/capture_complete.txt")){throw 'Camera handoff capture failed'}
Write-Output "Native camera capture completed: $OutputDir"
