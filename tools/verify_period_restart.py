...
BUILD={'include/nba_period_restart.h','src/nba_period_restart.c','tools/period_restart_probe.c','tools/period_restart_probe_fields.inc','tools/build_period_restart_probe.ps1'}
...
def check_source(rom):
    ...
    body=(ROOT/'src/nba_period_restart.c').read_text().split('formation[3][5]={',1)[1].split('};',1)[0]
...
def read_native(p,rom):
    ...
    for row in rows[start['index']-1:]:
        check(row['d']==0 and not(row['ps']&0x30),'native M/X/DP preconditions')
...
def main(a):
    ...
    report={'passed':True,'cases':cases,'formation_rom_sha256':table,'source_sha256':sha(ROOT/'src/nba_period_restart.c'),'verifier_sha256':sha(__file__),'build_manifest_sha256':sha(a.exe.parent/'build-manifest.json')}
