...
$script=Join-Path $out 'capture.lua';Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'mesen_gameplay_hud.lua') -Destination $script
$arguments=@('--testrunner','--timeout=600',('"'+$rom+'"'),('"'+$script+'"'))
$manifest=[ordered]@{schema=1;kind='natural neutral-controller CPU match first HUD publisher';
    ...
    input_schedule='Normal title/setup/team Start presses; on PlayerSetup wait350, left400 iftarget<2, left460 iftarget<1, Start700. All gameplay controllers remain neutral; no court button input. All times are phase-local native frames.';
