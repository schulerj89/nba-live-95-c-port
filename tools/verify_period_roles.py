"""Typed before-only native role-prefix differential; not a full BC07 oracle."""
...
BUILD={'include/nba_period_roles.h','src/nba_period_roles.c','tools/period_roles_probe.c','tools/period_roles_probe_fields.inc','tools/build_period_roles_probe.ps1'}
...
def mapping():
    fields=re.findall(r'^(ACTOR|CONTEXT|GLOBAL)\((\w+),(0x[0-9a-f]+)\)$',(ROOT/'tools/period_roles_probe_fields.inc').read_text(),re.M)
    check(len(fields)==35,'role projection closure');result=[]
    for i in range(10):result.extend((f'actor{i}.{n}',0x34eb+256*i+int(o,16))for k,n,o in fields if k=='ACTOR')
    for i in range(2):result.extend((f'context{i}.{n}',0x46eb+128*i+int(o,16))for k,n,o in fields if k=='CONTEXT')
    result.extend((n,int(o,16))for k,n,o in fields if k=='GLOBAL')
    check(len(result)==91,'role typed word count');return result
...
def source(rom):
    ...
    intervals={}
    for lo,hi in ((0x85bc07,0x85bd0c),(0x85f347,0x85f3ba),(0x86e1e5,0x86e1f6)):
        off=((lo>>16)&127)*32768+(lo&32767);intervals[f'{lo:06x}-{hi:06x}']=hashlib.sha256(raw[off:off+hi-lo+1]).hexdigest()
    return intervals

def binary_input(words):return struct.pack('<'+('H'*(len(words)+2)),0x5252,1,*words)
...
def run_probe(exe,out,name,data):
    ...
    for row in rows:
        check(set(row)=={'kind','pc','completed_calls','words'}and integer(row['kind'],1,4)and integer(row['pc'],0,0xffffff)and integer(row['completed_calls'],0,2),'role C boundary schema')
...
def verify_case(p,rom,exe,out):
    ...
    names=mapping();initial=[word(before['memory'],a)for _,a in names]
    ...
    typed([{k:r[k]for k in ('kind','pc','completed_calls')}for r in got],[{'kind':1,'pc':0x86e1ee,'completed_calls':1},{'kind':2,'pc':0x86e1f7,'completed_calls':2}])
    expected=[word(after['memory'],a)for _,a in names]
    differences=[(n,g,w)for (n,_),g,w in zip(names,got[-1]['words'],expected)if g!=w]
    check(not differences,'role parent fields: '+str(differences[:15]))
    return {'capture':str(p),'manifest_sha256':sha(p/'manifest.json'),'words_compared':len(names),'source_boundary':0x86e1f7,'scope':'two early-return BC07 prefixes including F34F, typed before-state only','initial_cadence':word(before['memory'],0x9d2),'final_cadence':word(after['memory'],0x9d2),'delta':word(before['memory'],0xc6),'rebuild':word(before['memory'],0x9d6),'camera':word(before['memory'],0x93a)}
...
def main(a):
    ...
    report={'passed':True,'cases':cases,'source_intervals':intervals,'verifier_sha256':sha(__file__),'capture_contract_sha256':sha(capture_contract.__file__),'source_sha256':sha(ROOT/'src/nba_period_roles.c'),'build_manifest_sha256':sha(a.exe.parent/'build-manifest.json')}
