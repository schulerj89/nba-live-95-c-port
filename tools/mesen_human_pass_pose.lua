...
local selection=assert(tonumber(os.getenv('NBA95_PASS_POSE_SELECTION')))
local stop_at=assert(tonumber(os.getenv('NBA95_PASS_POSE_FRAMES')))
...
local function snap(tag,pc,raw)
 ...
 log:write(string.format('{"index":%d,"tag":"%s","pc":%d,"frame":%d,"court":%d,"raw":"%s","cpu_d":%d,"cpu_x":%d,"cpu_y":%d,"cpu_ps":%d,"actor":%d,"owner":%d,"live":%d,"offense":%d,"direction":%d,"candidate":%d,"score":%d}\n',
  index,tag,pc,frame,court,file,s['cpu.d']or 0,s['cpu.x']or 0,s['cpu.y']or 0,s['cpu.ps']or 0,w(0xc2),w(0x93e),w(0x936),w(0x93a),w(0x90c)>=0x47eb and w(w(0x90c)+6)or 65535,w(0x92),w(0xaa)))
 log:flush()
end
...
hook(0x84e09c,function()
 if active then
  check(not pose and not aligned and not lane and not aligned_upper and not action and not upper and not metric and not cancel and(active=='selection'or(active=='child'and prefix and geometry)),'bad pass resume')
